...
function formatTimeValue(value) {
    ...
    if (value instanceof Date) {
        ...
        if (parts.second && parts.second !== '00') {
    ...
    if (parsedParts) {
        ...
        return parsedParts.seconds === null || parsedParts.seconds === 0
            ? normalized
...
function isSuspendedAttendanceUser(user) {
...
function buildAbsentEntry(user, dateKey) {
    ...
    const isSuspended = isSuspendedAttendanceUser(user);
...
function getDailyAttendanceSnapshot(dateKey = getDateKey()) {
    ...
    return employees.map((user) => {
        const scheduleForDate = getUserScheduleForDate(user, dateKey);
        const isSuspendedAccount = isSuspendedAttendanceUser(user);
