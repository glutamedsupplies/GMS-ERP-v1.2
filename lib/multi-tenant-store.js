...
function isDeviceBlocked({ deviceId = '', ipAddress = '' } = {}) {
    ensureSystemReady();
    const normalizedDeviceId = normalizeText(deviceId);
    const ipCandidates = buildIpMatchCandidates(ipAddress);
    const db = getMasterDb();
...
function blockDevice({
    ...
    if (!existing && ipCandidates.length) {
        const placeholders = ipCandidates.map(() => '?').join(', ');
        existing = db.prepare(`
            SELECT *
            FROM blocked_devices
    ...
    if (existing) {
        db.prepare(`
            ...
            WHERE id = ?
        `).run(
            normalizedDeviceId || String(existing.device_id || ''),
            normalizedIp || String(existing.ip_address || ''),
