...
function calculateWorkedHours(timeIn, timeOut, scheduledTimeIn = '', scheduledTimeOut = '', options = {}) {
    ...
    const effectiveStartMinutes = !attendancePolicy.countFromScheduledStart || scheduledMinutes === null
        ? timeInMinutes
        : Math.max(timeInMinutes, scheduledMinutes);

    if (timeOutMinutes <= effectiveStartMinutes) {
        return '0.00';
    }

    let workedMinutes = timeOutMinutes - effectiveStartMinutes;
    ...
    const lunchBreakEndMinutes = Number.isFinite(attendancePolicy.lunchBreakEndMinutes)
        ...
        : null;
    const lunchOverlapMinutes = lunchBreakStartMinutes === null || lunchBreakEndMinutes === null
        ? 0
        : Math.max(
            0,
            Math.min(timeOutMinutes, lunchBreakEndMinutes) - Math.max(effectiveStartMinutes, lunchBreakStartMinutes)
        );
    workedMinutes = Math.max(0, workedMinutes - lunchOverlapMinutes);
...
function updateEmployee(payload = {}) {
    ...
    if (!wasActive && nextIsActive && isEmployeeLikeRole(nextRole)) {
        materializeSuspendedAttendanceHistory(user, {
            fromDateKey: getUserSuspendedOn(user),
            untilDateKeyExclusive: getDateKey(),
            branchId: resolvedBranchId || user.branch_id || ''
        });
    }
...
function normalizeAttendanceRow(row, userMap) {
    ...
    const workedHours = calculateWorkedHours(timeIn, timeOut, scheduledTimeIn, scheduledTimeOut) || String(row.worked_hours || '').trim() || '0.00';
