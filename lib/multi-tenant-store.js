...
const Database = require('./postgres-sync');
...
function getDateKey(input = new Date()) {
    const date = input instanceof Date ? input : (parseDateValue(input) || new Date(input));
...
function formatTimeValue(value) {
    ...
    if (parsedMinutes !== null) {
        return `${pad(Math.floor(parsedMinutes / 60))}:${pad(parsedMinutes % 60)}`;
    }
...
function getDayKeyForDate(input = new Date()) {
    const parsedDate = parseDateValue(input);
    if (!parsedDate) {
        return '';
    }
 
    return WEEKLY_SCHEDULE_DAYS[parsedDate.getDay()] || '';
}
...
function normalizeStatus(remarks, lateMinutes, hasTimeIn) {
    ...
    if (text) {
        ...
        if (['late', 'absent', 'excuse', 'day off'].includes(lowered)) {
...
function getDailyAttendanceSnapshot(dateKey = getDateKey()) {
    ...
    return employees.map((user) => {
        ...
        return {
            ...
            displayRemarks: isScheduledDayOffWithoutLogs
                ? 'Scheduled day off'
...
function copySharedColumns(sourceDb, targetDb, sourceTable, targetTable, overrides = {}) {
    ...
    const insert = targetDb.prepare(`
        INSERT INTO ${targetTable} (${allTargetColumns.join(', ')})
        VALUES (${placeholders})
    `);
    ...
    const insertCompanyWithSubscription = db.transaction(() => {
        db.prepare(`
            INSERT INTO companies (
                ...
                logo_path, primary_color, app_name, db_schema
            )
            VALUES (?, ?, 'active', ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
            companyId,
...
function migrateLegacySingleCompany() {
    ...
    const legacyUsersDb = fs.existsSync(legacyPaths.users)
        ? new SqliteDatabase(legacyPaths.users, { readonly: true, fileMustExist: true })
        : null;
    const legacyAttendanceDb = fs.existsSync(legacyPaths.attendance)
        ? new SqliteDatabase(legacyPaths.attendance, { readonly: true, fileMustExist: true })
        : null;
