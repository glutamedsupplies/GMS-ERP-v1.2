...
function resolveUserAccountStatusPayload(payload = {}, currentStatus = USER_ACCOUNT_STATUS_ACTIVE) {
    ...
    if (hasAccountStatusOverride) {
        const requestedStatus = normalizeUserAccountStatusValue(payload.account_status ?? payload.accountStatus);
...
function normalizeStatus(remarks, lateMinutes, hasTimeIn) {
    ...
    if (text) {
        if (lowered === 'present' || lowered === 'on time' || lowered === 'late') {
        ...
        entries.forEach((row) => {
            ...
            if (hasClockActivity) {
                ...
                nextWorkedHours = timeOut
                    ...
                    : '';
                nextRemarks = normalizeStatus(row.remarks, lateMinutes, Boolean(timeIn));
...
function addEmployee({
    ...
    getMasterDb().prepare(`
        INSERT INTO users (
            ...
            account_status, suspended_on, inactive_on, display_name, profile_picture, time_in, time_out, schedule_json, feature_access_json
        )
        VALUES (?, ?, ?, ?, ?, 'employee', ?, ?, ?, ?, '', ?, ?, ?, ?)
    `).run(
...
function normalizeAttendanceRow(row, userMap) {
    ...
    const unavailableStatus = getAttendanceAccountStatusLabel(user, dateKey);
    const isUnavailableWithoutLogs = Boolean(unavailableStatus) && !hasClockActivity;
    const scheduledTimeIn = scheduleForDate.is_day_off || isUnavailableWithoutLogs ? '' : scheduleForDate.time_in;
    const scheduledTimeOut = scheduleForDate.is_day_off || isUnavailableWithoutLogs ? '' : scheduleForDate.time_out;
    const lateMinutes = calculateLateMinutes(timeIn, scheduledTimeIn);
...
function buildAbsentEntry(user, dateKey) {
    ...
    const unavailableStatus = getAttendanceAccountStatusLabel(user, dateKey);
    const status = unavailableStatus || (scheduleForDate.is_day_off ? 'Day Off' : 'Absent');
    ...
    return {
        ...
        lateMinutes: 0,
        remarksRaw: status,
        scheduledTimeIn: scheduleForDate.is_day_off || unavailableStatus ? '' : formatTimeValue(scheduleForDate.time_in || ''),
        scheduledTimeOut: scheduleForDate.is_day_off || unavailableStatus ? '' : formatTimeValue(scheduleForDate.time_out || ''),
        isDayOff: scheduleForDate.is_day_off,
...
function getDailyAttendanceSnapshot(dateKey = getDateKey()) {
    ...
    return employees.map((user) => {
        ...
        const scheduledTimeIn = scheduleForDate.is_day_off || isUnavailableAccount ? '' : formatTimeValue(scheduleForDate.time_in || '');
        const scheduledTimeOut = scheduleForDate.is_day_off || isUnavailableAccount ? '' : formatTimeValue(scheduleForDate.time_out || '');
        const entry = attendanceMap.get(user.id) || null;
        if (!entry) {
            const inferredStatus = unavailableStatus || (scheduleForDate.is_day_off ? 'Day Off' : 'Absent');
            return {
                id: user.id,
                name: user.name,
                dateKey,
                ...
                canEditStatus: !isUnavailableAccount && !scheduleForDate.is_day_off,
                isDayOff: scheduleForDate.is_day_off,
                branch_id: user.branch_id || ''
            };
        }
        ...
        const status = isUnavailableWithoutLogs ? unavailableStatus : entry.status;
        return {
            id: user.id,
            name: user.name,
            ...
            canEditStatus: !isUnavailableAccount && !scheduleForDate.is_day_off && ['Absent', 'Excuse'].includes(entry.status),
            isDayOff: scheduleForDate.is_day_off,
