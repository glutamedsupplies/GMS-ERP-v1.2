...
function normalizeStatus(remarks, lateMinutes, hasTimeIn) {
    ...
    if (text) {
        if (lowered === 'present' || lowered === 'on time') {
            return lateMinutes > 0 ? 'Late' : 'On Time';
        }
        if (['late', 'absent', 'excuse', 'day off', 'suspended'].includes(lowered)) {
...
function getAccountDeletionVerificationPurpose() {
    return 'delete_account';
}

function findReplacementCompanyAdminUser(companyId = '', excludeUsername = '') {
    const normalizedCompanyId = normalizeText(companyId);
    const normalizedExcludeUsername = normalizeText(excludeUsername);
    if (!normalizedCompanyId) {
        return null;
    }
    ...
    return getMasterDb().prepare(`
        ...
        ORDER BY
            ...
            username COLLATE NOCASE
        LIMIT 1
    `).get(normalizedCompanyId, normalizedExcludeUsername);
}

function getVerifiedUserDeletionEmail(user, preferredEmail = '') {
    const normalizedPreferredEmail = normalizeEmail(preferredEmail);
    const loginEmail = normalizeEmail(user?.login_email);
    const googleEmail = normalizeEmail(user?.google_email);
    const loginEmailVerified = Boolean(Number(user?.login_email_verified || 0));
    const googleEmailVerified = Boolean(Number(user?.google_email_verified || 0));

    if (normalizedPreferredEmail) {
        if (loginEmailVerified && loginEmail === normalizedPreferredEmail) {
            return normalizedPreferredEmail;
        }
        if (googleEmailVerified && googleEmail === normalizedPreferredEmail) {
            return normalizedPreferredEmail;
        }
        throw new Error('Use a verified login or Google email for account deletion.');
    }

    if (loginEmailVerified && loginEmail) {
        return loginEmail;
    }
    ...
    throw new Error('Connect and verify an email or Google account before requesting account deletion.');
}

function findUserByVerifiedDeletionEmail(companyId = '', email = '') {
    ensureSystemReady();
    const normalizedCompanyId = normalizeText(companyId);
    const normalizedEmail = normalizeEmail(email);
    if (!normalizedCompanyId || !normalizedEmail) {
        return null;
    }
    ...
    return getMasterDb().prepare(`
        ...
        WHERE company_id = ?
          AND COALESCE(is_active, 0) = 1
          AND (
              (COALESCE(login_email_verified, 0) = 1 AND LOWER(COALESCE(login_email, '')) = LOWER(?))
              OR (COALESCE(google_email_verified, 0) = 1 AND LOWER(COALESCE(google_email, '')) = LOWER(?))
          )
        LIMIT 1
    `).get(normalizedCompanyId, normalizedEmail, normalizedEmail);
}

function assertUserCanDeleteOwnAccount(user) {
    ...
    if (normalizedRole === ROLE_SUPER_ADMIN) {
        throw new Error('Super admin account deletion must be handled separately.');
    }

    if (normalizedRole === ROLE_COMPANY_ADMIN) {
        const replacementAdmin = findReplacementCompanyAdminUser(user.company_id, user.username);
        if (!replacementAdmin) {
            throw new Error('Assign another active company admin before deleting this account.');
        }
    }
}

function findUserAccountDeletionCandidate({ companyCode = '', email = '', host = '' } = {}) {
    ensureSystemReady();
    const company = getCompanyByCodeOrHost(companyCode, host);
    if (!company || String(company.status || '').toLowerCase() !== COMPANY_ACTIVE_STATUS) {
        return null;
    }

    const user = findUserByVerifiedDeletionEmail(company.id, email);
    if (!user) {
        return null;
    }
...
function requestUserAccountDeletion({ userId = '', email = '', ttlMinutes = 15 } = {}) {
    const context = requireTenantContext();
    const user = getScopedUser(context.companyId, userId);
    if (!user) {
        throw new Error('User not found.');
    ...
    assertUserCanDeleteOwnAccount(user);

    const targetEmail = getVerifiedUserDeletionEmail(user, email);
    const verificationPurpose = getAccountDeletionVerificationPurpose();
    const code = buildVerificationCode();
    const expiresAt = new Date(Date.now() + Math.max(5, Number(ttlMinutes) || 15) * 60 * 1000).toISOString();
    const db = getMasterDb();
    ...
    db.prepare(`
        INSERT INTO user_email_verifications (user_id, company_id, email, purpose, code, expires_at)
        VALUES (?, ?, ?, ?, ?, ?)
    `).run(user.id, context.companyId, targetEmail, verificationPurpose, code, expiresAt);
...
function confirmUserAccountDeletion({ userId = '', code = '', email = '' } = {}) {
    const context = requireTenantContext();
    const user = getScopedUser(context.companyId, userId);
    if (!user) {
        throw new Error('User not found.');
    ...
    if (!verification) {
        throw new Error('No account deletion code requested.');
    }

    if (normalizeEmail(verification.email) !== targetEmail) {
        throw new Error('Linked email changed. Request a new deletion code.');
    }

    const normalizedCode = String(code || '').trim();
    if (!normalizedCode || normalizedCode !== String(verification.code || '').trim()) {
        throw new Error('Invalid verification code.');
    }

    const expiresAt = new Date(verification.expires_at || 0).getTime();
    if (!expiresAt || expiresAt < Date.now()) {
        throw new Error('Verification code expired.');
    }

    const normalizedRole = normalizeRole(user.role);
    const replacementAdmin = normalizedRole === ROLE_COMPANY_ADMIN
        ? findReplacementCompanyAdminUser(context.companyId, user.username)
        : null;

    if (normalizedRole === ROLE_COMPANY_ADMIN && !replacementAdmin) {
        throw new Error('Assign another active company admin before deleting this account.');
    }

    const companyAdminCredential = getCompanyAdminCredential(context.companyId);
    if (companyAdminCredential && String(companyAdminCredential.username || '').toLowerCase() === String(user.username || '').toLowerCase()) {
        if (replacementAdmin) {
            upsertCompanyAdminCredential(context.companyId, replacementAdmin.username, '');
        } else {
            db.prepare('DELETE FROM company_admin_credentials WHERE company_id = ?').run(context.companyId);
        }
    }
    ...
    logAudit('user.delete_self', 'user', user.username, {
        role: normalizedRole,
...
function listUsers(role = '', filter = '', { limit = 500, offset = 0 } = {}) {
    const context = requireTenantContext();
...
function updateUser(id, payload = {}) {
    ...
    const nextSuspendedOn = nextIsActive
        ? ''
        : (wasActive ? getDateKey() : getUserSuspendedOn(user));
...
function listClients(filter = '', { limit = 500, offset = 0 } = {}) {
    const db = getTenantDb();
    const normalizedFilter = normalizeText(filter);
    const normalizedOffset = Math.max(0, Number(offset) || 0);
    const normalizedLimit = Math.min(2500, Math.max(1, Number(limit) || 500));
    ...
    if (!normalizedFilter) {
        ...
        rows = db.prepare(`
            ...
            ORDER BY name COLLATE NOCASE, contact_number
            LIMIT ? OFFSET ?
        `).all(`%${normalizedFilter}%`, `%${normalizedFilter}%`, contactPattern, `%${normalizedFilter}%`, normalizedLimit, normalizedOffset);
    }
...
function normalizeAttendanceRow(row, userMap) {
    ...
    const isSuspendedWithoutLogs = isSuspendedAttendanceUser(user, dateKey) && !hasClockActivity;
    const scheduledTimeIn = scheduleForDate.is_day_off || isSuspendedWithoutLogs ? '' : scheduleForDate.time_in;
    const scheduledTimeOut = scheduleForDate.is_day_off || isSuspendedWithoutLogs ? '' : scheduleForDate.time_out;
    const lateMinutes = calculateLateMinutes(timeIn, scheduledTimeIn);
    const status = isSuspendedWithoutLogs
        ? 'Suspended'
...
function buildAbsentEntry(user, dateKey) {
    ...
    const isSuspended = isSuspendedAttendanceUser(user, dateKey);
    const status = isSuspended
        ? 'Suspended'
        : (scheduleForDate.is_day_off ? 'Day Off' : 'Absent');
    ...
    return {
        ...
        lateMinutes: 0,
        remarksRaw: status,
        scheduledTimeIn: scheduleForDate.is_day_off || isSuspended ? '' : formatTimeValue(scheduleForDate.time_in || ''),
        scheduledTimeOut: scheduleForDate.is_day_off || isSuspended ? '' : formatTimeValue(scheduleForDate.time_out || ''),
        isDayOff: scheduleForDate.is_day_off,
...
function getDailyAttendanceSnapshot(dateKey = getDateKey()) {
    ...
    return employees.map((user) => {
        const scheduleForDate = getUserScheduleForDate(user, dateKey);
        const isSuspendedAccount = isSuspendedAttendanceUser(user, dateKey);
        const scheduledTimeIn = scheduleForDate.is_day_off || isSuspendedAccount ? '' : formatTimeValue(scheduleForDate.time_in || '');
        const scheduledTimeOut = scheduleForDate.is_day_off || isSuspendedAccount ? '' : formatTimeValue(scheduleForDate.time_out || '');
        const entry = attendanceMap.get(user.id) || null;
        if (!entry) {
            const inferredStatus = isSuspendedAccount
                ? 'Suspended'
                : (scheduleForDate.is_day_off ? 'Day Off' : 'Absent');
            return {
                id: user.id,
                ...
                remarksRaw: '',
                displayRemarks: isSuspendedAccount
                    ? 'Account suspended'
                    : (scheduleForDate.is_day_off ? 'Scheduled day off' : 'No attendance record'),
                canEditStatus: !isSuspendedAccount && !scheduleForDate.is_day_off,
                isDayOff: scheduleForDate.is_day_off,
                branch_id: user.branch_id || ''
            };
        }
        ...
        const isSuspendedWithoutLogs = isSuspendedAccount && !hasClockActivity;
        const status = isSuspendedWithoutLogs ? 'Suspended' : entry.status;
        return {
            id: user.id,
            name: user.name,
            ...
            displayRemarks: isSuspendedWithoutLogs
                ? 'Account suspended'
                : (isScheduledDayOffWithoutLogs
                    ? 'Scheduled day off'
                    : (normalizeStatus(entry.remarksRaw, entry.lateMinutes, Boolean(entry.timeIn)) || entry.status)),
            canEditStatus: !isSuspendedAccount && !scheduleForDate.is_day_off && ['Absent', 'Excuse'].includes(entry.status),
            isDayOff: scheduleForDate.is_day_off,
...
function getAttendanceRecordForDate(userId, dateKey = getDateKey()) {
    ...
    const user = getUserById(userId);
    if (!user || !isEmployeeLikeRole(user.role) || !isSuspendedAttendanceUser(user, targetDateKey)) {
        return null;
    }
...
function setDailyAttendanceStatus(userId, status, dateKey = getDateKey()) {
    ...
    if (isSuspendedAttendanceUser(user)) {
        throw new Error('This account is suspended. Reactivate it before editing attendance.');
    }
...
function recordTimeIn(userId, at = new Date()) {
    ...
    if (isSuspendedAttendanceUser(user)) {
        throw new Error('This account is suspended. Reactivate it before recording attendance.');
    }
    ...
    const scheduleForDate = getUserScheduleForDate(user, dateKey);
    const remarks = calculateLateMinutes(timeNow, scheduleForDate.time_in) > 0 ? 'Late' : 'On Time';
    const db = getTenantDb();
...
function recordTimeOut(userId, at = new Date()) {
    ...
    `).run(
        ...
        timeNow,
        workedHours,
        normalizeStatus(existing.remarksRaw, existing.lateMinutes, Boolean(existing.timeIn)) || existing.status || 'On Time',
        existing.rowId
    );
...
function createSuperAdminUser(payload = {}) {
    ...
    getMasterDb().prepare(`
        INSERT INTO users (
            id, company_id, branch_id, username, password_hash, role, is_active,
            display_name, profile_picture, login_email, login_email_verified,
            google_email, google_email_verified, time_in, time_out, schedule_json, feature_access_json
        )
        VALUES (?, NULL, NULL, ?, ?, 'super_admin', ?, ?, '', ?, 0, '', 0, '08:00', '17:00', '{}', '{}')
    `).run(
        createUserInternalId('', username),
        username,
        passwordHash,
...
function updateSuperAdminUser(username = '', payload = {}) {
    ...
    const nextSuspendedOn = nextIsActive
        ? ''
        : (Boolean(Number(user.is_active || 0)) ? getDateKey() : getUserSuspendedOn(user));
...
function deleteSuperAdminUser(username = '') {
    ...
    getMasterDb().prepare('DELETE FROM users WHERE id = ?').run(user.id);
    logAudit('customer_service.account_delete', 'user', username, {
        role: ROLE_SUPER_ADMIN
    });
...
module.exports = {
    ...
    findUserAccountDeletionCandidate,
