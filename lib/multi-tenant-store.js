...
function getTenantDb(companyId = '') {
    ...
    return openDb(cacheKey, getTenantSchema(targetCompanyId), initTenantSchema);
}
...
function initMasterSchema(db) {
    db.exec(`
        ...
        CREATE INDEX IF NOT EXISTS idx_blocked_devices_active
        ON blocked_devices (is_active);

        CREATE INDEX IF NOT EXISTS idx_blocked_devices_device_id
        ON blocked_devices (device_id);
...
function getUserByLoginHandle({ companyCode = '', username = '', host = '' } = {}) {
    ensureSystemReady();
    const normalizedUsername = normalizeText(username);
    if (!normalizedUsername) {
        return null;
...
function listActiveCompanyCodesByUsername(username = '') {
    ...
    `).all(normalizedUsername, COMPANY_ACTIVE_STATUS);
...
function listAccessLogs({ limit = 200, offset = 0, filter = '' } = {}) {
    ...
    if (normalizedFilter) {
        ...
        whereClause = `
            WHERE LOWER(access_logs.device_id) LIKE LOWER(?)
               OR LOWER(access_logs.ip_address) LIKE LOWER(?)
               OR LOWER(access_logs.host) LIKE LOWER(?)
               OR LOWER(access_logs.path) LIKE LOWER(?)
               OR LOWER(access_logs.user_agent) LIKE LOWER(?)
        `;
    ...
    return getMasterDb().prepare(`
        ...
        LIMIT ?
        OFFSET ?
    `).all(...params, Number(limit || 200), Number(offset || 0));
}

function listBlockedDevices({ limit = 200, offset = 0, filter = '' } = {}) {
    ensureSystemReady();
    const normalizedFilter = normalizeText(filter);
    const params = [];
    let whereClause = '';
    ...
    if (normalizedFilter) {
        ...
        whereClause = `
            WHERE LOWER(device_id) LIKE LOWER(?)
               OR LOWER(ip_address) LIKE LOWER(?)
               OR LOWER(host) LIKE LOWER(?)
               OR LOWER(user_agent) LIKE LOWER(?)
...
function isDeviceBlocked({ deviceId = '', ipAddress = '' } = {}) {
    ensureSystemReady();
    const normalizedDeviceId = normalizeText(deviceId);
    const normalizedIp = normalizeText(ipAddress);
    const db = getMasterDb();
...
function blockDevice({
    ...
    const normalizedIp = normalizeText(ipAddress);
    if (!normalizedDeviceId && !normalizedIp) {
        throw new Error('Device ID or IP address is required.');
    }
    ...
    if (existing) {
        ...
        `).run(
            ...
            String(userAgent || existing.user_agent || ''),
            String(host || existing.host || ''),
            String(blockedBy || existing.blocked_by || ''),
            existing.id
        );

        return db.prepare('SELECT * FROM blocked_devices WHERE id = ?').get(existing.id);
    }

    const result = db.prepare(`
        INSERT INTO blocked_devices (
            device_id, ip_address, user_agent, host, reason, blocked_by, is_active
        )
        VALUES (?, ?, ?, ?, ?, ?, 1)
    `).run(
        normalizedDeviceId || '',
    ...
    return db.prepare('SELECT * FROM blocked_devices WHERE id = ?').get(result.lastInsertRowid);
}
...
function unblockDevice(id) {
    ...
    return { unblocked: true, id: normalizedId };
}
