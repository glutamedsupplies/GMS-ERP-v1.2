...
const GMS_GWD_LATE_GRACE_START_DATE_KEY = '2026-05-08';
const GMS_GWD_LATE_GRACE_REMARK = 'Grace period (15 minutes)';
...
function getAttendancePolicyDateKey(options = {}) {
    const explicitDateKey = normalizeDateKey(
        options.dateKey
        || options.attendanceDateKey
    ...
    return getDateKey(options.at || new Date());
}

function getLateGraceMinutes(options = {}) {
    const attendancePolicy = resolveAttendancePolicy(options);
    const lateGraceMinutes = Number(attendancePolicy?.lateGraceMinutes);
    if (!Number.isFinite(lateGraceMinutes)) {
        return 0;
    }

    if (
        attendancePolicy === GMS_GWD_ATTENDANCE_POLICY
        && getAttendancePolicyDateKey(options) < GMS_GWD_LATE_GRACE_START_DATE_KEY
    ) {
        return 0;
    }

    return Math.max(0, Math.round(lateGraceMinutes));
}
...
function isWithinLateGracePeriod(timeIn, scheduledTimeIn, options = {}) {
    const scheduledLateMinutes = calculateScheduledLateMinutes(timeIn, scheduledTimeIn);
    const lateGraceMinutes = getLateGraceMinutes(options);
    return scheduledLateMinutes > 0
        && lateGraceMinutes > 0
        && scheduledLateMinutes <= lateGraceMinutes;
}
...
function buildClockAttendanceRemark(timeIn, scheduledTimeIn, options = {}) {
    ...
    if (isWithinLateGracePeriod(timeIn, scheduledTimeIn, options)) {
        return GMS_GWD_LATE_GRACE_REMARK;
    }
...
function buildAttendanceDisplayRemark(rawRemarks = '', timeIn = '', scheduledTimeIn = '', options = {}) {
    ...
    if (['on time', 'late', 'present'].includes(loweredRemarks)) {
...
function getUserCutoffPayrollStatus(userId, input = new Date()) {
    ...
    const row = getTenantDb().prepare(`
        SELECT user_id, cutoff_start_date, cutoff_end_date, payout_status, paid_at, updated_at, updated_by
