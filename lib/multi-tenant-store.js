...
function buildAbsentEntry(user, dateKey) {
    ...
    return {
        ...
        lateMinutes: 0,
        remarksRaw: status,
        scheduledTimeIn: scheduleForDate.is_day_off || unavailableStatus || holidayStatus ? '' : formatTimeValue(scheduleForDate.time_in || ''),
        scheduledTimeOut: scheduleForDate.is_day_off || unavailableStatus || holidayStatus ? '' : formatTimeValue(scheduleForDate.time_out || ''),
        isHoliday: Boolean(holidayStatus),
        isDayOff: scheduleForDate.is_day_off,
...
function getUserSemiMonthlyTimeCard(userId, input = new Date()) {
    ...
    return getSemiMonthlyDateKeys(input)
        .map((dateKey) => attendanceMap.get(dateKey) || buildAbsentEntry(user, dateKey))
        .sort(compareAttendance);
}
