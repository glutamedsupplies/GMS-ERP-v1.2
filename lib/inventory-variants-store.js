...
function hasColumn(db, tableName, columnName) {
    ...
    return columns.some((column) => normalizeText(column.name).toLowerCase() === normalizeText(columnName).toLowerCase());
}
...
function ensureSchema(db) {
    ...
    db.exec(`
        ...
        CREATE INDEX IF NOT EXISTS idx_inventory_variants_item_code
        ON inventory_variants (item_code);
    ...
    if (!hasColumn(db, 'inventory_variants', 'cost_price')) {
        db.exec('ALTER TABLE inventory_variants ADD COLUMN cost_price REAL NOT NULL DEFAULT 0');
    }
...
function findHeaderRow(rows) {
    const aliases = {
        ...
        cost_price: new Set(['cost', 'costprice', 'cost_price', 'kuha', 'kuhaprice', 'purchaseprice', 'buyingprice'])
    };
    ...
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
        ...
        const costPriceIndex = tokens.findIndex((token) => aliases.cost_price.has(token));
...
function readInventoryVariantsFromCsv(filePath) {
    ...
    rows.slice(headerRow.rowIndex + 1).forEach((columns) => {
        ...
        const rawCostPrice = headerRow.indexes.cost_price !== -1
            ? columns[headerRow.indexes.cost_price]
            : '';
        const hasCostPrice = headerRow.indexes.cost_price !== -1 && normalizeText(rawCostPrice) !== '';
        const costPrice = hasCostPrice ? parseNumber(rawCostPrice) : 0;
...
function findVariantById(db, id) {
    return db.prepare(`
        SELECT id, product_name, item_code, set_name, price, cost_price, source, created_at, updated_at
...
function importInventoryVariantsFromCsv({ filePath = '', suppressMissingFile = false } = {}) {
    ...
    const existingRows = db.prepare(`
        SELECT id, product_name, item_code, set_name, price, cost_price
        FROM inventory_variants
    `).all();
    ...
    const insertVariant = db.prepare(`
        INSERT INTO inventory_variants (product_name, item_code, set_name, price, cost_price, source)
        VALUES (?, ?, ?, ?, ?, 'seed')
    `);
    const updateVariant = db.prepare(`
        ...
        WHERE id = ?
    `);
    const updateVariantWithCost = db.prepare(`
        UPDATE inventory_variants
        SET item_code = ?,
            price = ?,
            cost_price = ?,
        ...
        entries.forEach((entry) => {
            ...
            if (!existingRow) {
                insertVariant.run(
                    ...
                    entry.setName,
                    entry.price,
                    entry.hasCostPrice ? entry.costPrice : 0
                );
                stats.inserted += 1;
            } else if (
                normalizeText(existingRow.item_code) !== entry.itemCode
                || Number(existingRow.price) !== Number(entry.price)
                || (entry.hasCostPrice && Number(existingRow.cost_price || 0) !== Number(entry.costPrice || 0))
            ) {
                if (entry.hasCostPrice) {
                    updateVariantWithCost.run(entry.itemCode, entry.price, entry.costPrice, existingRow.id);
                } else {
...
function listInventoryVariants({ productName = '', setName = '', search = '', limit = 500, offset = 0 } = {}) {
    const db = getDb();
    const clauses = [];
    const params = [];
...
function createInventoryVariant(payload) {
    ...
    const result = db.transaction(() => {
        const insertResult = db.prepare(`
            INSERT INTO inventory_variants (product_name, item_code, set_name, price, cost_price, source)
            VALUES (?, ?, ?, ?, ?, 'manual')
        `).run(variant.productName, variant.itemCode, variant.setName, variant.price, variant.costPrice);
...
function ensureInventoryVariant(payload, { source = 'manual' } = {}) {
    const db = getDb();
    ...
    if (existingRow) {
        const existingSource = normalizeText(existingRow.source).toLowerCase();
        if (existingSource !== 'composite') {
            return findVariantById(db, existingRow.id);
        }

        if (normalizeText(existingRow.item_code) === variant.itemCode) {
            return findVariantById(db, existingRow.id);
        }
        ...
        db.transaction(() => {
            db.prepare(`
                ...
                SET item_code = ?,
                    updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
            `).run(variant.itemCode, Number(existingRow.id));

            syncLegacyProductCatalogRow(db, {
                productName: variant.productName,
                itemCode: variant.itemCode,
                setName: variant.setName,
                price: Number(existingRow.price || 0)
            }, existingRow.source || normalizedSource);
        })();
    ...
    const result = db.transaction(() => {
        ...
        syncLegacyProductCatalogRow(db, variant, normalizedSource);
        return insertResult.lastInsertRowid;
    })();
    ...
    db.transaction(() => {
        db.prepare(`
            ...
            SET product_name = ?,
                ...
                updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        `).run(variant.productName, variant.itemCode, variant.setName, variant.price, variant.costPrice, Number(id));
