...
function collectCompanyMonthlyData({ companyId, dateFrom, dateTo }) {
    return store.runWithTenantContextByCompany(companyId, () => {
...
async function buildCompanyWorkbook({ company, monthKey, dateFrom, dateTo }) {
    const workbook = new ExcelJS.Workbook();
    workbook.creator = 'GMS ERP';
    workbook.created = new Date();
    workbook.modified = new Date();

    const data = await Promise.resolve(collectCompanyMonthlyData({
    ...
    const salesSummary = data.salesResult.summary || {};
    ...
    const expenseTotal = (data.expensesResult.items || []).reduce((sum, row) => sum + Number(row.amount || 0), 0);
    const cashIncomeTotal = (data.cashIncomeResult.items || []).reduce((sum, row) => sum + Number(row.amount || 0), 0);
    ...
    const attendanceSummaryRows = buildAttendanceSummary(data.attendanceRows);
    ...
    addSheet(workbook, 'Sales', salesColumns, buildRows(data.salesResult.items || [], salesColumns));
    addSheet(workbook, 'Attendance', attendanceColumns, buildRows(data.attendanceRows || [], attendanceColumns));
    addSheet(workbook, 'Expenses', expenseColumns, buildRows(data.expensesResult.items || [], expenseColumns));
    addSheet(workbook, 'Cash Income', cashIncomeColumns, buildRows(data.cashIncomeResult.items || [], cashIncomeColumns));
    addSheet(workbook, 'Inventory', inventoryColumns, buildRows(data.inventoryRows || [], inventoryColumns));
    addSheet(workbook, 'Inventory Variants', variantColumns, buildRows(data.variantRows || [], variantColumns));

    if (data.lbcRows.length) {
        addSheet(workbook, 'LBC Tracking', lbcColumns, buildRows(data.lbcRows || [], lbcColumns));
    }

    return workbook;
}

async function buildCompanyWorkbookBuffer({ company, monthKey, dateFrom, dateTo }) {
    const workbook = await buildCompanyWorkbook({ company, monthKey, dateFrom, dateTo });
    const buffer = await workbook.xlsx.writeBuffer();
    return Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer);
}

async function writeCompanyWorkbookFile({ company, monthKey, dateFrom, dateTo, outputPath }) {
    const workbook = await buildCompanyWorkbook({ company, monthKey, dateFrom, dateTo });
    await workbook.xlsx.writeFile(outputPath);
}

module.exports = {
    buildCompanyWorkbookBuffer,
    getMonthBounds,
    parseMonth,
    safeName,
