...
function getQuickChatReply(message = '') {
    ...
    return 'Hi! Pwede mo akong tanungin tungkol sa attendance, sales, inventory, users, reports, at ibang GMS ERP modules. Anong part ang gusto mong maintindihan?';
}
...
function buildAiHandoffThreadKey(session = null, threadId = '') {
    ...
    const userId = String(session?.userId || session?.username || 'guest').trim().toLowerCase();
    return `${companyId}:${userId}:${normalizedThreadId}`.slice(0, 180);
}

function buildAiHandoffContactNumber(session = null, threadId = '') {
    const rawCandidates = [
...
function buildAiHandoffRequestDetails({ session = null, userMessage = '' } = {}) {
    const lines = ['AI handoff from GMS AI Assistant.'];
    const userName = String(session?.user?.name || session?.userId || session?.username || 'Customer').trim();
...
function routeAiHandoffToCustomerServiceInbox({
    session = null,
    threadId = '',
    userMessage = ''
} = {}) {
    const normalizedMessage = sanitizeChatText(userMessage);
    if (!session?.companyId || !normalizedMessage) {
        return null;
    }

    const senderName = String(session?.user?.name || session?.userId || session?.username || 'Customer').trim() || 'Customer';
    const contactNumber = buildAiHandoffContactNumber(session, threadId);
    const requestMeta = {
        intent: 'ai_handoff',
        email: String(session?.user?.login_email || '').trim(),
        desiredId: String(session?.userId || session?.username || '').trim()
    };
    const threadKey = buildAiHandoffThreadKey(session, threadId);

    try {
        const thread = runInTenantContext(session, () => {
            if (threadKey) {
                const existingCode = String(AI_CHAT_HANDOFF_REQUESTS.get(threadKey) || '').trim();
                if (existingCode) {
                    try {
                        return store.addCustomerRequestMessageByCode(existingCode, {
                            senderName,
            ...
            if (threadKey && created?.request?.requestCode) {
        ...
        console.error('Failed to route AI handoff to customer service inbox:', error);
        return null;
    }
}
...
function appendAiHandoffConfirmation(reply = '', requestCode = '', userMessage = '') {
    ...
    const confirmationLine = filipino
        ? `Na-forward ko na rin ito sa Customer Service inbox. Reference code: ${normalizedCode}.`
        : `I also forwarded this to the Customer Service inbox. Reference code: ${normalizedCode}.`;
...
function finalizeAiReplyForDelivery({
    ...
    if (normalizedReply !== handoffReply) {
        return {
    ...
    const handoffThread = routeAiHandoffToCustomerServiceInbox({
        session,
        threadId,
        userMessage
    });
    const handoffRequestCode = String(handoffThread?.request?.requestCode || '').trim();
...
function getFeatureOverviewReply(message = '', role = '') {
    ...
    if (filipino) {
        return [
            ...
            '1. Attendance: para sa Time In/Out, Timecards, at Today Present.',
            '2. Sales: para sa Create Sales, Sales Report, Invoice Summary, Invoice Template, at Order Form.',
            '3. Inventory: para sa Inventory, Inventory Levels, at Composite Items.',
            '4. User and company setup: para sa Users, Employees, Branches, at Settings.',
            '5. Reports and tools: para sa Reports, Sales Inventory Insight, Customer Requests, Communication Panel, at LBC Tracking.',
            '6. Super Admin only: para sa Dashboard, Customer Requests, Plans and Subscriptions, Audit Logs, Access Logs, at Blocked Devices.',
            roleHint,
    ...
    return [
        ...
        '1. Attendance: for Time In/Out, Timecards, and Today Present.',
        '2. Sales: for Create Sales, Sales Report, Invoice Summary, Invoice Template, and Order Form.',
        '3. Inventory: for Inventory, Inventory Levels, and Composite Items.',
        '4. User and company setup: for Users, Employees, Branches, and Settings.',
        '5. Reports and tools: for Reports, Sales Inventory Insight, Customer Requests, Communication Panel, and LBC Tracking.',
        '6. Super Admin only: for Dashboard, Customer Requests, Plans and Subscriptions, Audit Logs, Access Logs, and Blocked Devices.',
        roleHint,
...
function getGoogleLoginHelpReply(message = '') {
    ...
    if (filipino) {
        return [
            'Possible reasons kung bakit ayaw ng Google login sa GMS ERP:',
            '1. Walang Company ID o mali ang Company ID.',
            '2. Hindi pa ready ang Google sign-in setup sa current domain o URL.',
            '3. Hindi pa authorized ang domain sa Firebase Auth.',
            '4. Hindi pa naka-link ang Google email mo sa mismong GMS user account.',
            'Try mo muna mag-login gamit ang Company ID, email, at password. Kapag okay iyon, ipa-connect ang Google account sa admin o sa Settings kung available.',
            'Kung may exact error message ka, i-send mo dito para ma-pinpoint natin.'
        ].join('\n');
    }

    return [
        'Here are the usual reasons why Google login does not work in GMS ERP:',
        '1. The Company ID is missing or incorrect.',
        '2. Google sign-in is not ready on the current domain or URL.',
        '3. The domain is not authorized in Firebase Auth.',
        '4. Your Google email is not linked to the matching GMS user account yet.',
        'Try logging in first with Company ID, email, and password. If that works, ask the admin to connect your Google account or use Settings if that option is available.',
        'If you have the exact error message, send it here and I can narrow it down.'
    ].join('\n');
}
...
function createRequestListener() {
    return (req, res) => {
        ...
        const bypassDeviceBlock = isSuperAdmin(requestSession) || Boolean(requestSession?.supportSession);

        if (!bypassDeviceBlock && store.isDeviceBlocked({ deviceId, ipAddress: clientIp })) {
