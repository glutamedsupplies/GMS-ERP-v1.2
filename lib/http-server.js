...
const GOOGLE_MOBILE_HANDOFF_TTL_MS = Math.max(60_000, Number(process.env.GOOGLE_MOBILE_HANDOFF_TTL_MS || 2 * 60 * 1000));
const GOOGLE_MOBILE_CALLBACK_URL = String(process.env.GOOGLE_MOBILE_CALLBACK_URL || 'gmsmobile://auth/google').trim();
...
const FEATURE_LABELS = Object.freeze({
    ...
    lbc_collection_confirmation: 'LBC Collection Confirmation',
...
const googleMobileHandoffs = new Map();
...
function applySecurityHeaders(res) {
    ...
    // Google/Firebase popup auth needs access to its opener to complete sign-in.
...
const COMPRESSIBLE_STATIC_EXTENSIONS = new Set([
    '.css', '.csv', '.html', '.js', '.json', '.map', '.svg', '.txt', '.webmanifest', '.xml'
]);

function acceptsGzipEncoding(res) {
    const value = String(res?.req?.headers?.['accept-encoding'] || '').toLowerCase();
    return /(?:^|,)\s*gzip\s*(?:;|,|$)/.test(value) && !/(?:^|,)\s*gzip\s*;\s*q=0(?:\.0*)?\s*(?:,|$)/.test(value);
}

function sendResponseBody(res, statusCode, headers, body, { allowCompression = false, headOnly = false } = {}) {
    const bodyBuffer = Buffer.isBuffer(body) ? body : Buffer.from(String(body ?? ''), 'utf8');
    const shouldCompress = allowCompression
        && bodyBuffer.length >= RESPONSE_COMPRESSION_MIN_BYTES
        && acceptsGzipEncoding(res);
    ...
    if (shouldCompress) {
        ...
        gzipStream.on('error', () => res.destroy());
        gzipStream.pipe(res);
        gzipStream.end(bodyBuffer);
        return;
    }

    responseHeaders['Content-Length'] = bodyBuffer.length;
    res.writeHead(statusCode, responseHeaders);
    res.end(headOnly ? undefined : bodyBuffer);
}

function sendJson(res, statusCode, payload) {
    sendResponseBody(res, statusCode, {
        'Content-Type': 'application/json; charset=utf-8',
        'Cache-Control': 'no-store'
    }, JSON.stringify(payload), { allowCompression: true });
}
...
function inferErrorCode(message = '') {
    ...
    if (
        ...
        || normalized.includes('only active employees')
        || normalized.includes('cannot rate')
        || normalized.includes('select a 1 to 5')
        || normalized.includes('rate and comment')
        || normalized.includes('explain the issue')
        || normalized.includes('currently disabled')
    ) {
        return 'VALIDATION_ERROR';
    }
...
function readJsonBody(req, { maxBytes = MAX_JSON_BODY_BYTES } = {}) {
    return new Promise((resolve, reject) => {
        let rawBody = '';
        const normalizedMaxBytes = Math.max(1, Number(maxBytes || MAX_JSON_BODY_BYTES));

        req.on('data', (chunk) => {
            rawBody += chunk;
            if (Buffer.byteLength(rawBody, 'utf8') > normalizedMaxBytes) {
...
function normalizeListQueryLimit(rawValue, {
    defaultLimit = 500,
    maxLimit = 2500,
...
function buildSessionCookie(token, { host = '' } = {}) {
    const parts = [
        ...
        'Path=/',
        'HttpOnly',
        // Google OAuth returns through a top-level cross-site navigation.
        'SameSite=Lax',
...
function getRequestMeta(req, res, { skipDeviceCookie = false } = {}) {
    if (req?.__gmsRequestMeta) {
        return req.__gmsRequestMeta;
    }

    const meta = {
        deviceId: skipDeviceCookie ? '' : (req && res ? resolveDeviceId(req, res) : ''),
...
function cleanupExpiredGoogleMobileHandoffs(now = Date.now()) {
    for (const [key, value] of googleMobileHandoffs.entries()) {
        if (!value || (now - value.createdAt) > GOOGLE_MOBILE_HANDOFF_TTL_MS) {
            googleMobileHandoffs.delete(key);
        }
    }
}

function createGoogleLoginState({ companyCode = '', mobile = false } = {}) {
...
function createGoogleMobileHandoff({ sessionToken = '', companyCode = '' } = {}) {
    cleanupExpiredGoogleMobileHandoffs();
    const code = crypto.randomBytes(24).toString('base64url');
    googleMobileHandoffs.set(code, {
...
function consumeGoogleMobileHandoff(code = '') {
    cleanupExpiredGoogleMobileHandoffs();
    const normalized = String(code || '').trim();
    const entry = googleMobileHandoffs.get(normalized);
    googleMobileHandoffs.delete(normalized);
    return entry || null;
}
...
function redirectToMobileGoogleResult(res, { code = '', error = '', companyCode = '' } = {}) {
    const callback = new URL(GOOGLE_MOBILE_CALLBACK_URL);
    if (code) {
    ...
    if (companyCode) {
        callback.searchParams.set('companyCode', companyCode);
    }
...
function canAccessWorkspaceConfig(session) {
    if (!session) {
        return false;
    }
    ...
    return hasUserFeatureAccess(session, 'order_form') || sessionHasOrderFormSetupAccess(session);
}
...
function getSessionUserId(session) {
    return String(session?.userId || session?.user?.id || '').trim();
}

function isSessionLbcCollectionAssignee(session) {
    if (!session?.companyId) {
        return false;
    }
    ...
    try {
        return Boolean(runInTenantContext(session, () => store.isLbcCollectionAssignee(userId)));
    } catch (_error) {
        return false;
    }
}

function canAccessLbcTracking(session) {
    return isHeadAdmin(session)
        || hasUserFeatureAccess(session, 'lbc_tracking')
        || isSessionLbcCollectionAssignee(session);
}

function canConfirmLbcCollection(session) {
    return isHeadAdmin(session) || isSessionLbcCollectionAssignee(session);
}

function requireLbcTrackingAccess(res, session) {
    if (!requireSession(res, session)) {
        return false;
    }
    if (canAccessLbcTracking(session)) {
        return true;
    }
    sendError(res, 403, 'LBC Tracking is not allowed for your account. Contact your administrator.', 'FORBIDDEN');
    return false;
}

function requireLbcCollectionAccess(res, session) {
    if (!requireSession(res, session)) {
        return false;
    }
    // List/view collections: head admins, assigned confirmers, or LBC Tracking feature users (read/view).
    // Confirm/undo still goes through requireLbcCollectionConfirmAccess (head admin + assignee only).
    if (canConfirmLbcCollection(session) || hasUserFeatureAccess(session, 'lbc_tracking')) {
        return true;
    }
    sendError(res, 403, 'LBC Collection Confirmation is not allowed for your account. Contact your administrator.', 'FORBIDDEN');
    return false;
}

function requireLbcCollectionConfirmAccess(res, session) {
    if (!requireSession(res, session)) {
        return false;
    }
    if (canConfirmLbcCollection(session)) {
        return true;
    }
    sendError(res, 403, 'LBC Collection Confirmation is not allowed for your account. Contact your administrator.', 'FORBIDDEN');
    return false;
...
function runSuperAdminTenantContext(session, company, work) {
    return store.runWithTenantContextByCompany(company.id, () => {
        const context = store.getCurrentContext();
        if (context) {
            context.requestUserId = session?.userId || '';
            context.actorUserId = session?.userId || '';
            context.actorRole = session?.role || store.ROLE_SUPER_ADMIN;
        }
        return work();
    });
}
...
const ATTENDANCE_CORRECTION_ERROR_CODES = new Set([
...
function sendAttendanceCorrectionStoreError(res, error) {
    const code = String(error?.code || '').trim().toUpperCase();
    if (ATTENDANCE_CORRECTION_ERROR_CODES.has(code)) {
        sendError(
            res,
...
async function handleApiRequest(req, res, urlObject, contextReady = false) {
    ...
    if (req.method === 'GET' && pathname === '/api/auth/google') {
        ...
        if (!companyCode) {
            if (mobile) {
                redirectToMobileGoogleResult(res, {
                    error: 'Company ID is required for Google login.'
                });
                return;
            }
        ...
        const state = createGoogleLoginState({ companyCode, mobile });
    ...
    if (req.method === 'GET' && pathname === '/api/auth/google/callback') {
        ...
        try {
            ...
            if (entry.mobile) {
                const handoffCode = createGoogleMobileHandoff({
                    sessionToken,
                    companyCode: user.company_code || entry.companyCode || ''
                });
                redirectToMobileGoogleResult(res, {
            ...
            appendSetCookie(res, buildSessionCookie(sessionToken, { host: req.headers.host || '' }));
    ...
    if (req.method === 'POST' && pathname === '/api/mobile/auth/google/complete') {
        const body = await readJsonBody(req);
        const code = String(body.code || body.handoffCode || body.handoff_code || '').trim();
        if (!code) {
            sendError(res, 400, 'Google login handoff code is required.', 'VALIDATION_ERROR');
            return;
        }

        const handoff = consumeGoogleMobileHandoff(code);
        if (!handoff?.sessionToken) {
            sendError(res, 400, 'Google login session expired. Please try again.', 'VALIDATION_ERROR');
            return;
        }

        const sessionPayload = verifySessionToken(handoff.sessionToken);
        if (!sessionPayload) {
            sendError(res, 401, 'Google login session is invalid. Please try again.', 'INVALID_CREDENTIALS');
            return;
        }

        let user = null;
        try {
            user = sessionPayload.companyId
                ? store.runWithTenantContextByCompany(
                    sessionPayload.companyId,
                    () => store.getUserById(sessionPayload.username)
                )
                : store.getUserById(sessionPayload.username);
        } catch (_error) {
            user = null;
        ...
        if (!user) {
            sendError(res, 401, 'Google account no longer matches an active user.', 'INVALID_CREDENTIALS');
            return;
        }

        appendSetCookie(res, buildSessionCookie(handoff.sessionToken, { host: req.headers.host || '' }));
    ...
    if (req.method === 'GET' && pathname === '/api/super/attendance/employees') {
        if (!requireSuperAdmin(res, session)) {
            return;
        }
        ...
        const attendanceUsers = runSuperAdminTenantContext(session, company, () => (
            store.listUsers('', filter, { includeInactive: false }).filter((user) => {
    ...
    const superAttendanceUserCutoffCardMatch = pathname.match(/^\/api\/super\/attendance\/user\/([^/]+)\/cutoff-card$/);
    if (superAttendanceUserCutoffCardMatch && req.method === 'GET') {
        if (!requireSuperAdmin(res, session)) {
            return;
        }
        ...
        const userId = decodeURIComponent(superAttendanceUserCutoffCardMatch[1]);
        const dateKey = searchParams.get('dateKey') || store.getDateKey();
        const rows = runSuperAdminTenantContext(session, company, () => (
            store.getUserSemiMonthlyTimeCard(userId, dateKey)
        ));
        sendSuccess(res, serializeAttendanceRows(rows));
        return;
    }

    const superAttendanceTimeCardEntryMatch = pathname.match(/^\/api\/super\/attendance\/user\/([^/]+)\/time-card-entry$/);
    if (superAttendanceTimeCardEntryMatch && req.method === 'PATCH') {
        if (!requireSuperAdmin(res, session)) {
            return;
        }
        ...
        const userId = decodeURIComponent(superAttendanceTimeCardEntryMatch[1]);
        validateRequired(body.dateKey || body.date, 'Date');
        const row = runSuperAdminTenantContext(session, company, () => (
            store.setManualTimeCardEntry(userId, {
                dateKey: body.dateKey ?? body.date,
                timeIn: body.timeIn ?? body.time_in,
                timeOut: body.timeOut ?? body.time_out,
                workedHours: body.workedHours ?? body.worked_hours,
                status: body.status,
                remarks: body.remarks ?? body.note
            })
        ));
        sendSuccess(res, serializeAttendanceRow(row));
        return;
    }
    ...
    if (req.method === 'GET' && pathname === '/api/kpi/context') {
        if (!requireTenantSession(res, session)) {
            return;
        }
        if (!isHeadAdmin(session) && !isEmployee(session) && !isStaff(session)) {
            sendError(res, 403, 'Forbidden.', 'FORBIDDEN');
            return;
        }
        const requestedUserId = String(searchParams.get('userId') || '').trim();
        const targetUserId = isHeadAdmin(session) ? (requestedUserId || session.userId) : session.userId;
        if (!isHeadAdmin(session) && requestedUserId && requestedUserId !== session.userId) {
            sendError(res, 403, 'Forbidden.', 'FORBIDDEN');
            return;
        }
        sendSuccess(res, store.getKpiEvaluationContext(targetUserId));
        return;
    }

    if (req.method === 'GET' && pathname === '/api/kpi/settings') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }
        sendSuccess(res, store.getKpiEvaluationSettings());
        return;
    }

    if (req.method === 'PUT' && pathname === '/api/kpi/settings') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }
        const body = await readJsonBody(req);
        sendSuccess(res, store.updateKpiEvaluationSettings(body));
        return;
    }

    if (req.method === 'GET' && pathname === '/api/kpi/evaluations') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }
        const summaryDateKey = searchParams.get('dateFrom') === searchParams.get('dateTo')
            ? searchParams.get('dateFrom')
            : '';
        sendSuccess(res, {
            items: store.listKpiEvaluations({
                ...
                limit: searchParams.get('limit') || 2500,
                offset: searchParams.get('offset') || 0
            }),
            summary: store.getKpiEvaluationSummary(new Date(), { dateKey: summaryDateKey }),
            settings: store.getKpiEvaluationSettings(),
            employees: store.listActiveKpiEmployees()
        });
        return;
    }

    if (req.method === 'GET' && pathname === '/api/kpi/scores/history') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }
        sendSuccess(res, store.getKpiDailyScoreHistory(
    ...
    if (req.method === 'POST' && pathname === '/api/kpi/evaluations/admin') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }
        const body = await readJsonBody(req);
        sendSuccess(res, store.submitKpiAdminEvaluation(session.userId, body), 201);
        return;
    }

    if (req.method === 'POST' && pathname === '/api/kpi/evaluations/issue') {
        if (!requireTenantSession(res, session)) {
            return;
        }
        if (!isHeadAdmin(session) && !isEmployee(session) && !isStaff(session)) {
        ...
        sendSuccess(res, store.submitKpiIssueEncounter(targetUserId, body), 201);
        return;
    }

    const kpiEvaluationMatch = pathname.match(/^\/api\/kpi\/evaluations\/([^/]+)$/);
    if (kpiEvaluationMatch && req.method === 'GET') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }
        sendSuccess(res, store.getKpiEvaluationById(decodeURIComponent(kpiEvaluationMatch[1])));
        return;
    }

    if (kpiEvaluationMatch && req.method === 'PATCH') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }
        const body = await readJsonBody(req);
        sendSuccess(res, store.reviewKpiEvaluation(decodeURIComponent(kpiEvaluationMatch[1]), {
            status: body.status,
            note: body.note,
    ...
    if (req.method === 'GET' && pathname === '/api/incident-reports') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }

        const ownerEmployeeId = String(searchParams.get('ownerEmployeeId') || '').trim();
        try {
            sendSuccess(res, {
                items: store.listIncidentReports({
                    ownerEmployeeId,
                    status: searchParams.get('status') || '',
                    search: searchParams.get('search') || '',
                    dateFrom: searchParams.get('dateFrom') || '',
                ...
                summary: store.getIncidentReportSummary({ ownerEmployeeId })
            });
        } catch (error) {
            sendStoreError(res, error);
        }
    ...
    if (req.method === 'POST' && pathname === '/api/incident-reports') {
        if (!requireTenantSession(res, session)) {
            return;
        }
        if (!isEmployee(session) && !isStaff(session)) {
        ...
        try {
            const body = await readJsonBody(req);
            const employeeId = getSessionUserId(session);
            sendSuccess(res, store.createIncidentReport({
                date: body.date ?? body.incidentDate ?? body.incident_date,
                department: body.department,
                reasonIncident: body.reasonIncident ?? body.reason_incident ?? body.reason,
                actionTaken: body.actionTaken ?? body.action_taken ?? body.action,
                preparedBy: body.preparedBy ?? body.prepared_by,
                checkedBy: '',
                dateChecked: '',
                resolved: false
            }, {
    ...
    const incidentReportMatch = pathname.match(/^\/api\/incident-reports\/([^/]+)$/);
    if (incidentReportMatch && req.method === 'GET') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }
        try {
            const report = store.getIncidentReportById(decodeURIComponent(incidentReportMatch[1]));
    ...
    if (incidentReportMatch && req.method === 'PATCH') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }
        try {
            const reportId = decodeURIComponent(incidentReportMatch[1]);
            const body = await readJsonBody(req);
            if (!Object.prototype.hasOwnProperty.call(body, 'resolved')) {
                sendError(res, 400, 'Resolved status is required.', 'VALIDATION_ERROR');
                return;
            }
            const nextResolved = ['1', 'true', 'yes', 'on'].includes(
                String(body.resolved ?? '').trim().toLowerCase()
            );
            ...
            sendSuccess(res, store.updateIncidentReport(reportId, {
                resolved: nextResolved,
                checkedBy: nextResolved ? reviewerName : '',
                dateChecked: nextResolved ? store.getDateKey() : ''
            }, {
    ...
    if (req.method === 'GET' && pathname === '/api/company/workspace-config') {
        if (!requireSession(res, session) || !requireTenantSession(res, session)) {
            return;
        }
        if (!canAccessWorkspaceConfig(session)) {
            sendError(res, 403, 'Forbidden.', 'FORBIDDEN');
            return;
        }
    ...
    if (req.method === 'GET' && pathname === '/api/attendance/corrections') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }

        try {
            sendSuccess(res, store.listAttendanceCorrections({
                status: searchParams.get('status') || '',
                limit: Number(searchParams.get('limit') || 100),
                offset: Number(searchParams.get('offset') || 0)
            }));
        } catch (error) {
            sendAttendanceCorrectionStoreError(res, error);
        }
        return;
    }

    const attendanceCorrectionReviewMatch = pathname.match(/^\/api\/attendance\/corrections\/([^/]+)\/(approve|reject)$/);
    if (attendanceCorrectionReviewMatch && req.method === 'POST') {
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }

        try {
            const correctionId = decodeURIComponent(attendanceCorrectionReviewMatch[1]);
            const action = attendanceCorrectionReviewMatch[2];
            const body = action === 'reject' ? await readJsonBody(req) : {};
            const reviewed = action === 'approve'
                ? store.approveAttendanceCorrection(correctionId, { reviewedByUserId: session.userId })
                : store.rejectAttendanceCorrection(correctionId, {
                    reviewedByUserId: session.userId,
                    reason: body.reason ?? body.rejectionReason ?? body.rejection_reason ?? ''
                });
    ...
    if (req.method === 'GET' && pathname === '/api/users') {
        ...
        const limit = Number(searchParams.get('limit') || 500);
        const offset = Number(searchParams.get('offset') || 0);
        const includeInactive = isTruthyQueryValue(
    ...
    if (req.method === 'GET' && pathname === '/api/tasks') {
        ...
        sendSuccess(res, store.listTasks());
        return;
    }
    ...
    if (req.method === 'POST' && pathname === '/api/tasks') {
        ...
        const body = await readJsonBody(req);
        sendSuccess(res, store.addTask({
            name: String(body.name || body.task_name || body.taskName || '').trim(),
            inputType: String(body.inputType || body.input_type || body.type || '').trim()
        }), 201);
        return;
    }
    ...
    if (taskMatch && req.method === 'DELETE') {
        ...
        sendSuccess(res, store.deleteTask(decodeURIComponent(taskMatch[1])));
        return;
    }
    ...
    if (req.method === 'GET' && pathname === '/api/employees') {
        ...
        const filter = searchParams.get('filter') || '';
        const attendanceUsers = store.listUsers('', filter, { includeInactive: false }).filter((user) => {
            const normalizedRole = String(user?.role || '').trim().toLowerCase();
            if (!normalizedRole) {
                return true;
            }
    ...
    if (req.method === 'GET' && pathname === '/api/clients') {
        ...
        const filter = searchParams.get('filter') || '';
        const clientType = searchParams.get('clientType') || searchParams.get('client_type') || '';
        const limit = Number(searchParams.get('limit') || 500);
        const offset = Number(searchParams.get('offset') || 0);
    ...
    if (req.method === 'POST' && pathname === '/api/clients') {
        ...
        const client = store.addClient({
            ...
            clientType: String(body.clientType || body.client_type || '').trim(),
            source: 'manual'
        });
    ...
    if (req.method === 'GET' && pathname === '/api/sales/references') {
        if (!requireHeadAdminOrAnyFeatureAccess(res, session, ['order_form', 'expenses', 'sales_report'])) {
            return;
        }
        if (!requireModuleAccess(res, session, 'sales')) {
            return;
    ...
    if (req.method === 'GET' && pathname === '/api/inventory/out-tracking') {
        if (!requireHeadAdmin(res, session)) {
            return;
        }
        if (!requireModuleAccess(res, session, 'sales')) {
        ...
        const limit = normalizeListQueryLimit(searchParams.get('limit'), { allowUnlimited: true });
        const offset = Number(searchParams.get('offset') || 0);
        sendSuccess(res, salesStore.listInventoryOutTracking({
            dateFrom: searchParams.get('dateFrom') || '',
            dateTo: searchParams.get('dateTo') || '',
            branch: searchParams.get('branch') || '',
            search: searchParams.get('search') || '',
    ...
    if (req.method === 'GET' && pathname === '/api/sales') {
        if (!requireHeadAdminOrFeatureAccess(res, session, 'sales_report')) {
            return;
        }
        if (!requireModuleAccess(res, session, 'sales')) {
            return;
    ...
    if (req.method === 'GET' && pathname === '/api/super/kpi/settings') {
        if (!requireSuperAdmin(res, session)) {
            return;
        }
        const company = resolveSuperAdminCompany(searchParams.get('companyId') || '');
        sendSuccess(res, runSuperAdminTenantContext(session, company, () => (
            store.getKpiEvaluationSettings()
        )));
        return;
    }

    if (req.method === 'GET' && pathname === '/api/super/kpi/evaluations') {
        if (!requireSuperAdmin(res, session)) {
            return;
        }
        const company = resolveSuperAdminCompany(searchParams.get('companyId') || '');
        const summaryDateKey = searchParams.get('dateFrom') === searchParams.get('dateTo')
            ? searchParams.get('dateFrom')
            : '';
        const payload = runSuperAdminTenantContext(session, company, () => ({
            company: {
                id: company.id,
                name: company.name || '',
                companyCode: company.company_code || ''
    ...
    if (req.method === 'GET' && pathname === '/api/super/kpi/scores/history') {
        if (!requireSuperAdmin(res, session)) {
            return;
        }
        const company = resolveSuperAdminCompany(searchParams.get('companyId') || '');
        sendSuccess(res, runSuperAdminTenantContext(session, company, () => (
            store.getKpiDailyScoreHistory(
                searchParams.get('employeeId') || '',
                { days: searchParams.get('days') || 90 }
            )
        )));
    ...
    if (req.method === 'POST' && pathname === '/api/super/kpi/evaluations/admin') {
        if (!requireSuperAdmin(res, session)) {
            return;
        }
        const company = resolveSuperAdminCompany(searchParams.get('companyId') || '');
        const body = await readJsonBody(req);
        sendSuccess(res, runSuperAdminTenantContext(session, company, () => (
            store.submitKpiAdminEvaluation(session.userId, body, new Date(), {
                allowExternalAdmin: true,
                actorName: session.displayName || session.name || session.username || session.userId
            })
        )), 201);
        return;
    }

    const superKpiEvaluationMatch = pathname.match(/^\/api\/super\/kpi\/evaluations\/([^/]+)$/);
    if (superKpiEvaluationMatch && req.method === 'GET') {
        if (!requireSuperAdmin(res, session)) {
            return;
        }
        const company = resolveSuperAdminCompany(searchParams.get('companyId') || '');
        sendSuccess(res, runSuperAdminTenantContext(session, company, () => (
            store.getKpiEvaluationById(decodeURIComponent(superKpiEvaluationMatch[1]))
        )));
        return;
    }

    if (superKpiEvaluationMatch && req.method === 'PATCH') {
        if (!requireSuperAdmin(res, session)) {
            return;
        }
        const company = resolveSuperAdminCompany(searchParams.get('companyId') || '');
        const body = await readJsonBody(req);
        sendSuccess(res, runSuperAdminTenantContext(session, company, () => (
            store.reviewKpiEvaluation(decodeURIComponent(superKpiEvaluationMatch[1]), {
                status: body.status,
                note: body.note,
                actorUserId: session.userId
            })
    ...
    if (req.method === 'GET' && pathname === '/api/lbc-tracking') {
        if (!requireLbcTrackingAccess(res, session)) {
            return;
        }
        if (!requireTenantSession(res, session)) {
            return;
    ...
    if (req.method === 'GET' && pathname === '/api/lbc-tracking/collection-assignees') {
        if (!requireSession(res, session)) {
            return;
        }
        if (!requireTenantSession(res, session)) {
        ...
        const employees = runInTenantContext(session, () => store.getLbcCollectionAssignees());
        const currentUserId = getSessionUserId(session);
        const isAssigned = employees.some((entry) => String(entry.id || '').trim() === currentUserId);
        // Only head admins (and super admins) get the full assignee roster.
        // Employees only learn whether they themselves are assigned.
        sendSuccess(res, {
            employees: (isHeadAdmin(session) || isSuperAdmin(session)) ? employees : [],
            isAssigned
        });
        return;
    }

    if (req.method === 'PUT' && pathname === '/api/lbc-tracking/collection-assignees') {
        ...
        const body = await readJsonBody(req);
        const employees = runInTenantContext(session, () => store.setLbcCollectionAssignees(body));
        sendSuccess(res, { employees });
        return;
    }

    if (req.method === 'GET' && pathname === '/api/lbc-tracking/collections') {
        if (!requireLbcCollectionAccess(res, session)) {
            return;
        }
        if (!requireTenantSession(res, session)) {
            return;
    ...
    if (lbcCollectionConfirmMatch && req.method === 'POST') {
        if (!requireLbcCollectionConfirmAccess(res, session)) {
            return;
        }
        if (!requireTenantSession(res, session)) {
            return;
    ...
    if (lbcTrackingMatch && req.method === 'PUT') {
        ...
        if (!canAccessLbcTracking(session) && !canConfirmLbcCollection(session)) {
            sendError(res, 403, 'LBC Tracking is not allowed for your account. Contact your administrator.', 'FORBIDDEN');
            return;
        }
        if (!requireTenantSession(res, session)) {
    ...
    if (orderMatch && req.method === 'GET') {
        ...
        const requestedOrderLookup = decodeURIComponent(orderMatch[1]);
        console.debug('[orders:debug] API order load request', {
            requestedOrderLookup,
            requestPath: pathname
        });
        const order = salesStore.getSaleOrder(requestedOrderLookup);
        console.debug('[orders:debug] API order load response', {
            requestedOrderLookup,
        ...
        sendSuccess(res, sanitizeSaleOrderForSession(session, order));
        return;
    }
    ...
    if (orderMatch && req.method === 'PUT') {
        ...
        const requestedOrderLookup = decodeURIComponent(orderMatch[1]);
        const body = await readJsonBody(req);
        console.debug('[orders:debug] API order update request', {
            requestedOrderLookup,
            payloadOrderNumber: body?.orderNumber || '',
            payloadReceiptNumber: body?.receiptNumber || '',
    ...
    if (employeeMatch && req.method === 'PUT') {
        ...
        store.updateEmployee(employeeUpdatePayload);
    ...
    const userTaskMatch = pathname.match(/^\/api\/users\/([^/]+)\/task$/);
    if (userTaskMatch && req.method === 'PATCH') {
        const userId = decodeURIComponent(userTaskMatch[1]);
        if (!requireHeadAdmin(res, session) || !requireTenantSession(res, session)) {
            return;
        }

        const body = await readJsonBody(req);
        sendSuccess(res, serializeUser(store.setUserAssignedTask(
            userId,
            {
                taskIds: body.taskIds ?? body.task_ids ?? body.taskId ?? body.task_id ?? [],
                customTasks: body.customTasks ?? body.custom_tasks
            }
        )));
        return;
    }
    ...
    if (attendanceUserCutoffPayrollMatch && req.method === 'PATCH') {
        ...
        const hasPayoutStatus = Object.prototype.hasOwnProperty.call(body, 'status')
            || Object.prototype.hasOwnProperty.call(body, 'payoutStatus')
            || Object.prototype.hasOwnProperty.call(body, 'payout_status');
        const hasPayslipPhotoUpdate = Object.prototype.hasOwnProperty.call(body, 'payslipPhotoDataUrl')
            || Object.prototype.hasOwnProperty.call(body, 'payslip_photo_data_url')
            || Object.prototype.hasOwnProperty.call(body, 'payslipPhotoName')
            || Object.prototype.hasOwnProperty.call(body, 'payslip_photo_name')
            || Object.prototype.hasOwnProperty.call(body, 'removePayslipPhoto')
            || Object.prototype.hasOwnProperty.call(body, 'remove_payslip_photo');
        if (!hasPayoutStatus && !hasPayslipPhotoUpdate) {
            validateRequired('', 'Payout status or payslip photo');
        }
        ...
        if (hasPayoutStatus) {
            ...
            updatedState = store.setUserCutoffPayrollStatus(userId, dateKey, payoutStatus);
        }
        if (hasPayslipPhotoUpdate) {
            updatedState = store.setUserCutoffPayslipPhoto(userId, dateKey, body);
        }

        sendSuccess(res, updatedState || store.getUserCutoffPayrollStatus(userId, dateKey));
        return;
    }
    ...
    if (attendanceUserTodayMatch && req.method === 'GET') {
        ...
        const dateKey = searchParams.get('dateKey') || store.getDateKey();
        sendSuccess(res, serializeAttendanceRow(store.getAttendanceRecordForDate(userId, dateKey)));
        return;
    }
    ...
    if (req.method === 'POST' && pathname === '/api/attendance/time-out') {
        ...
        const result = store.recordTimeOut(targetUserId, new Date(), {
            taskId: body.taskId ?? body.task_id,
            taskCount: body.taskCount ?? body.task_count,
            taskResults: body.taskResults ?? body.task_results,
            timeoutIssues: body.timeoutIssues ?? body.timeout_issues,
            timeoutRemarks: body.timeoutRemarks ?? body.timeout_remarks,
            requiredEvaluations: body.requiredEvaluations ?? body.required_evaluations,
            issueEncounters: body.issueEncounters ?? body.issue_encounters
        });
        sendSuccess(res, {
            user: serializeUser(result.user),
            time: result.time,
            ...
            kpiEvaluation: result.kpiEvaluation || null,
            record: serializeAttendanceRow(result.record)
        });
        return;
    }
