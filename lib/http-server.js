...
const AI_CHAT_TEMPERATURE = Number.isFinite(Number(process.env.ATTENDANCE_AI_CHAT_TEMPERATURE))
    ...
    : 0.2;
const AI_CHAT_MAX_OUTPUT_TOKENS = Math.max(50, Number(process.env.ATTENDANCE_AI_CHAT_MAX_OUTPUT_TOKENS || 700));
    ...
    if (req.method === 'POST' && pathname === '/api/public/ai/chat') {
        ...
        if (!provider) {
            sendError(
                res,
                503,
                'AI chat is not configured. Set ATTENDANCE_AI_CHAT_PROVIDER=ollama with a local model, or provide ATTENDANCE_GEMINI_API_KEY.',
                'AI_NOT_CONFIGURED'
            );
            return;
        }
        ...
        if (provider === 'ollama' || provider === 'local') {
            if (!isLocalChatConfigured()) {
                sendError(
                    res,
                    503,
                    'AI chat is not configured. Set ATTENDANCE_AI_CHAT_PROVIDER=ollama and provide ATTENDANCE_LOCAL_AI_URL and ATTENDANCE_LOCAL_AI_MODEL.',
                    'AI_NOT_CONFIGURED'
                );
                return;
            }
        ...
        if (provider === 'gemini' && !GEMINI_CHAT_API_KEY) {
            sendError(res, 503, 'AI chat is not configured on this server.', 'AI_NOT_CONFIGURED');
            return;
        }
        ...
        try {
            ...
            sendError(res, 502, error?.message || 'AI chat failed.', 'AI_ERROR');
            return;
        }
    }
