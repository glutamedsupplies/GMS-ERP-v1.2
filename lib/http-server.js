...
function getQuickChatReply(message = '') {
    ...
    return 'Hi! Pwede kitang tulungan sa login, sales, orders, tracking, payments, reports, users, inventory, at common GMS ERP issues. Anong task o problem ang gusto mong ayusin?';
}
...
function getSmallTalkReply(message = '') {
    ...
    if (/(perfect|perpekto).*(ka|mo)?|\bikaw ba perfect\b|\bare you perfect\b/.test(lower)) {
        return filipino
            ? [
                'Hindi naman ako perfect, pero pwede kitang tulungan mag-isip nang malinaw at step by step sa GMS ERP.',
                'Sabihin mo lang kung login, sales, tracking, inventory, users, reports, o settings ang concern mo.'
            ].join('\n')
            : [
                'I am not perfect, but I can help you reason through GMS ERP clearly and step by step.',
    ...
    if (/^(sino ka|who are you|ano ka|what are you)\b/.test(lower)) {
        return filipino
            ? [
                'Ako ang GMS AI Assistant.',
                'Parang support teammate ako para sa login, app steps, troubleshooting, at module guidance ng GMS ERP.'
            ].join('\n')
            : [
                'I am the GMS AI Assistant.',
                'I work like a support teammate for login, app steps, troubleshooting, and module guidance in GMS ERP.'
            ].join('\n');
    }

    if (/(salamat|thank you|thanks)\b/.test(lower)) {
        return filipino
            ? 'Walang anuman. Sabihin mo lang kung anong next na part sa GMS ERP ang gusto mong ayusin.'
            : 'You are welcome. Tell me which next part of GMS ERP you want to fix.';
    }
...
function resolveAccountDeletionCandidate(req, body = {}) {
    const companyCode = normalizeCompanyCode(body.companyCode || body.company_code || '');
    const email = normalizeEmailAddress(body.email || '');

    validateRequired(companyCode, 'Company ID');
    validateRequired(email, 'Email');

    const candidate = store.findUserAccountDeletionCandidate({
        companyCode,
        email,
        host: req.headers.host || ''
    });
    if (!candidate) {
        throw new Error('No verified account matched for that company ID and email.');
    }
...
async function handleApiRequest(req, res, urlObject, contextReady = false) {
    ...
    if (req.method === 'POST' && pathname === '/api/public/ai/chat') {
        ...
        const staticReply = directCustomerServiceReply
            || getSmallTalkReply(trimmedMessage)
        ...
        const aiCompany = session?.companyId ? store.getCompanyById(session.companyId) : null;
    ...
    if (req.method === 'POST' && pathname === '/api/account/delete/request') {
        ...
        if (session?.supportSession) {
            sendError(res, 403, 'Support sessions cannot request account deletion.', 'FORBIDDEN');
            return;
        }
        ...
        try {
            if (session?.companyId && session?.userId) {
                company = store.getCompanyById(session.companyId);
                if (!company) {
                    throw new Error('Company not found.');
                }

                result = store.runWithTenantContextByCompany(company.id, () => store.requestUserAccountDeletion({
                    userId: session.userId,
                    email: normalizeEmailAddress(body.email || ''),
                    ttlMinutes: EMAIL_CODE_TTL_MINUTES
                }));
            } else {
                const candidate = resolveAccountDeletionCandidate(req, body);
                company = candidate.company;
                result = store.runWithTenantContextByCompany(company.id, () => store.requestUserAccountDeletion({
        ...
        try {
            await sendVerificationEmail({
                ...
                companyName: company?.name || '',
                appName: company?.app_name || store.PARENT_BRAND.appName,
                actionText: 'confirm permanent account deletion for your GMS ERP account',
                contextLabel: 'account deletion code'
            });
        } catch (error) {
    ...
    if (req.method === 'POST' && pathname === '/api/account/delete/confirm') {
        const body = await readJsonBody(req);
        const code = String(body.code || '').trim();
        validateRequired(code, 'Verification code');

        if (session?.supportSession) {
            sendError(res, 403, 'Support sessions cannot confirm account deletion.', 'FORBIDDEN');
            return;
        }
        ...
        try {
            if (session?.companyId && session?.userId) {
                ...
                result = store.runWithTenantContextByCompany(company.id, () => store.confirmUserAccountDeletion({
                    userId: session.userId,
                    email: normalizeEmailAddress(body.email || ''),
                    code
                }));
        ...
        if (session?.companyId && session?.userId && String(session.companyId) === String(result.company_id || '')) {
            clearSessionCookie(res, { host: req.headers.host || '' });
        }

        sendSuccess(res, {
            ...result,
            companyCode: company?.company_code || result.company_code || '',
            loginUrl: buildProvisionedCompanyLoginUrl(req, company?.company_code || result.company_code || '')
        });
        return;
    }
