...
function getQuickChatReply(message = '') {
    ...
    return 'Hi! Pwede kitang tulungan sa sales, orders, delivery tracking, payments, reports, client records, at common GMS ERP issues. Anong task o problem ang gusto mong ayusin?';
}
...
function getFeatureOverviewReply(message = '', role = '') {
    ...
    if (filipino) {
        return [
            ...
            '6. Super Admin only: para sa companies, plans, customer service setup, audit logs, access logs, at blocked devices.',
            roleHint,
    ...
    return [
        ...
        '6. Super Admin only: for companies, plans, customer service setup, audit logs, access logs, and blocked devices.',
        roleHint,
    ...
    return (req, res) => {
        ...
        if (store.isDeviceBlocked({ deviceId, ipAddress: clientIp })) {
            if (requestSession?.token) {
                invalidateSession(requestSession.token);
            }
            if (req) {
                req.__gmsSession = null;
            }
