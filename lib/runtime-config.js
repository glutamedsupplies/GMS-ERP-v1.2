...
function getVercelPreviewUrl() {
    const hostname = String(process.env.VERCEL_URL ?? '').trim();
    if (!hostname) {
        return '';
    }
...
function getVercelProductionUrl() {
    const hostname = String(process.env.VERCEL_PROJECT_PRODUCTION_URL ?? '').trim();
    if (!hostname) {
        return '';
    }
...
function readProjectRuntimeConfig() {
    ...
    const fileServerConfig = fileConfig.server && typeof fileConfig.server === 'object'
        ? fileConfig.server
        : {};
    const renderPublicUrl = getRenderPublicUrl();
    const vercelPreviewUrl = getVercelPreviewUrl();
    const vercelProductionUrl = getVercelProductionUrl();
    const isRender = normalizeBoolean(process.env.RENDER, false) || Boolean(renderPublicUrl);
    const isVercel = normalizeBoolean(process.env.VERCEL, false)
        || Boolean(vercelPreviewUrl)
        || Boolean(vercelProductionUrl);
    const trustProxyDefault = (isRender || isVercel)
        ? true
        : normalizeBoolean(fileServerConfig.trustProxy, DEFAULT_SERVER_CONFIG.trustProxy);
    const secureCookiesDefault = (isRender || isVercel)
        ? true
