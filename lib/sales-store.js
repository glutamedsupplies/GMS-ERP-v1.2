...
let inventoryDerivedState;

try {
    inventoryDerivedState = require('./inventory-derived-state');
} catch (_error) {
    const globalStateKey = '__gmsInventoryDerivedStateStore';
    const sharedInventoryDerivedState = globalThis[globalStateKey] || (globalThis[globalStateKey] = {
        stateByDb: new WeakMap()
    });

    inventoryDerivedState = {
        getInventoryDerivedState(db) {
            ...
            const existing = sharedInventoryDerivedState.stateByDb.get(db);
            if (existing) {
                return existing;
            }
            ...
            sharedInventoryDerivedState.stateByDb.set(db, initialState);
            return initialState;
        },
        markInventoryDerivedDataDirty(db) {
            const state = inventoryDerivedState.getInventoryDerivedState(db);
            state.dirty = true;
...
const T_SET_COMPANY_CODES = new Set(['default', 'gms', 'gms-erp', 'gmserp']);
const ADMIN_OPTIONS = [];
...
const CUSTOMER_CREDIT_LOOKUP_TYPES = ['contact', 'name_address'];
const CUSTOMER_CREDIT_ENTRY_TYPES = ['credit', 'apply', 'adjust'];
const CUSTOMER_CREDIT_BACKFILL_SAMPLE_LIMIT = 12;
const CUSTOMER_CREDIT_BACKFILL_NOTE = 'Legacy backfill from historical overpayment';
const ORDER_STATUS_OPTIONS = ['Pending', 'Packed', 'Shipped', 'Completed', 'Cancelled'];
const AUTO_ORDER_PLACEHOLDER = 'Auto on save';
...
const EXCLUDED_CLIENT_HISTORY_PRODUCT_LOOKUPS = new Set(['handlingfee']);
...
function getBranchSignature(branches = BRANCHES) {
    return (Array.isArray(branches) ? branches : [])
        .map((branch) => normalizeText(branch).toLowerCase())
        .filter(Boolean)
        .join('||');
}

function ensureInventoryDerivedData(db) {
    const state = inventoryDerivedState.getInventoryDerivedState(db);
    const branchSignature = getBranchSignature(BRANCHES);
    const shouldSyncItems = Boolean(state.dirty);
    const shouldSyncLevels = shouldSyncItems || state.branchSignature !== branchSignature;
    ...
    state.dirty = false;
    state.branchSignature = branchSignature;
}
...
function getDb() {
    ...
    // only when the source catalog changes or branch coverage changes.
    ensureInventoryDerivedData(db);
...
function ensureSchema(db) {
    ...
    `);
    db.exec(`
        CREATE INDEX IF NOT EXISTS idx_sales_entries_client_name
        ON sales_entries (client_name)
    `);
    db.exec(`
        CREATE INDEX IF NOT EXISTS idx_sales_entries_order_lookup
        ON sales_entries (order_number, receipt_number)
    `);
    ...
    ensureTableColumn(db, 'inventory_variants', 'cost_price', 'REAL NOT NULL DEFAULT 0');
    ensureTableColumn(db, 'sales_entries', 'unit_cost_price', 'REAL NOT NULL DEFAULT 0');
    ensureTableColumn(db, 'sales_entries', 'line_cost_total', 'REAL NOT NULL DEFAULT 0');
    ensureTableColumn(db, 'sales_entries', 'line_profit', 'REAL NOT NULL DEFAULT 0');
    ...
    ensureTableColumn(db, 'lbc_tracking_entries', 'tracking_entries_json', "TEXT NOT NULL DEFAULT '[]'");
    db.prepare(`
    ...
    db.prepare(`
        UPDATE lbc_tracking_entries
        SET tracking_entries_json = '[]'
        WHERE TRIM(COALESCE(tracking_entries_json, '')) = ''
    `).run();
    db.prepare(`
...
function syncSeedData(db) {
    ...
    const state = inventoryDerivedState.getInventoryDerivedState(db);
    state.dirty = false;
    state.branchSignature = getBranchSignature(BRANCHES);
...
function isAutoOrderPlaceholder(value) {
    return normalizeText(value).toLowerCase() === AUTO_ORDER_PLACEHOLDER.toLowerCase();
}
...
function runIgnoringUniqueConstraint(statement, ...params) {
    const runStatement = () => statement.run(...params);

    try {
        if (statement?.database?.__isPostgresCompat) {
            return statement.database.transaction(() => runStatement())();
        }
        return runStatement();
    } catch (error) {
...
function normalizeOptionLookupKey(value) {
...
function supportsTSetForCurrentCompany() {
    const contextCompanyCode = normalizeCompanyCode(sqliteStore.getCurrentContext?.()?.company?.company_code || '');
    return !contextCompanyCode || T_SET_COMPANY_CODES.has(contextCompanyCode);
}

function getReferenceSetOptions() {
    if (supportsTSetForCurrentCompany()) {
        return SET_OPTIONS;
    }

    return SET_OPTIONS.filter((option) => option !== 'T');
}

function normalizeFlexibleOption(value, options) {
    const exactMatch = normalizeStrictOption(value, options);
    if (exactMatch) {
        return exactMatch;
    }

    const valueKey = normalizeOptionLookupKey(value);
    if (!valueKey) {
        return '';
    }
    ...
    const normalizedOptions = (Array.isArray(options) ? options : [])
        .map((option) => ({
            option,
            key: normalizeOptionLookupKey(option)
        }))
        .filter((entry) => entry.key);
    const directMatches = normalizedOptions.filter((entry) => (
        entry.key === valueKey
        || entry.key.includes(valueKey)
        || valueKey.includes(entry.key)
    ));
    if (directMatches.length === 1) {
        return directMatches[0].option;
    }

    const prefixMatches = directMatches.filter((entry) => (
        entry.key.startsWith(valueKey)
        || valueKey.startsWith(entry.key)
    ));
    if (prefixMatches.length === 1) {
        return prefixMatches[0].option;
    }
...
function normalizePaymentMethods(value) {
    ...
    return uniqueSorted(rawValues.map((entry) => normalizeFlexibleOption(entry, paymentOptions)).filter(Boolean));
}
...
function buildLbcTrackingEntryId() {
    return `trk_${crypto.randomBytes(6).toString('hex')}`;
}

function hasMeaningfulLbcTrackingEntry(entry = {}) {
    const trackingNumber = normalizeText(entry.trackingNumber || '');
    const amountToCollect = Number(entry.amountToCollect || 0);
    const deliveryStatus = normalizeLbcTrackingStatus(entry.deliveryStatus, trackingNumber ? 'In Transit' : 'Pending');
    return Boolean(trackingNumber || amountToCollect > 0 || deliveryStatus !== 'Pending');
}

function normalizeLbcTrackingEntry(entry = {}, {
    fallbackAmount = 0,
    generateId = true,
    fallbackCollectionStatus = '',
    fallbackCollectionConfirmedBy = '',
    fallbackCollectionConfirmedAt = ''
} = {}) {
    const trackingEntryId = normalizeText(
        entry.id
        || entry.entryId
        || entry.trackingEntryId
        || entry.tracking_entry_id
        || ''
    ) || (generateId ? buildLbcTrackingEntryId() : '');
    const trackingNumber = normalizeLbcTrackingNumber(
        entry.trackingNumber ?? entry.tracking_number ?? '',
        { allowEmpty: true }
    );
    const fallbackStatus = trackingNumber ? 'In Transit' : 'Pending';
    const deliveryStatus = normalizeLbcTrackingStatus(
        entry.deliveryStatus ?? entry.delivery_status,
        fallbackStatus
    );
    const deliveredEntry = getLbcDeliveryStatusBucket(deliveryStatus) === 'delivered';
    const collectionStatus = deliveredEntry
        ? normalizeLbcCollectionStatus(
            entry.collectionStatus ?? entry.collection_status,
            normalizeLbcCollectionStatus(fallbackCollectionStatus, 'Pending')
        )
        : 'Pending';
    const collectionConfirmedBy = collectionStatus === 'Confirmed'
        ? normalizeText(
            entry.collectionConfirmedBy
            ?? entry.collection_confirmed_by
            ?? fallbackCollectionConfirmedBy
        )
        : '';
    const collectionConfirmedAt = collectionStatus === 'Confirmed'
        ? normalizeText(
            entry.collectionConfirmedAt
            ?? entry.collection_confirmed_at
            ?? fallbackCollectionConfirmedAt
        )
        : '';
    return {
        id: trackingEntryId,
        trackingNumber,
        amountToCollect: normalizeLbcTrackingAmountToCollect(
            entry.amountToCollect ?? entry.amount_to_collect,
            fallbackAmount
        ),
        deliveryStatus,
        collectionStatus,
        ...
        dateMonitored: normalizeText(entry.dateMonitored ?? entry.date_monitored ?? ''),
        updatedBy: normalizeText(entry.updatedBy ?? entry.updated_by ?? ''),
        createdAt: normalizeText(entry.createdAt ?? entry.created_at ?? ''),
        updatedAt: normalizeText(entry.updatedAt ?? entry.updated_at ?? '')
    };
}

function isLbcTrackingEntryDelivered(entry = {}) {
    return getLbcDeliveryStatusBucket(
        normalizeLbcTrackingStatus(
            entry.deliveryStatus,
            normalizeText(entry.trackingNumber || '') ? 'In Transit' : 'Pending'
        )
    ) === 'delivered';
}

function buildLegacyLbcTrackingEntries(trackingRow = {}, orderHeader = {}) {
    if (!trackingRow) {
        return [];
    }

    const entry = normalizeLbcTrackingEntry({
        id: 'legacy-primary',
        trackingNumber: trackingRow.tracking_number || trackingRow.trackingNumber || '',
        amountToCollect: trackingRow.amount_to_collect ?? trackingRow.amountToCollect ?? getLbcOrderCollectionAmount(orderHeader),
        deliveryStatus: trackingRow.delivery_status || trackingRow.deliveryStatus || (
            normalizeText(trackingRow.tracking_number || trackingRow.trackingNumber || '')
                ? 'In Transit'
                : 'Pending'
        ),
        collectionStatus: trackingRow.collection_status || trackingRow.collectionStatus || '',
        collectionConfirmedBy: trackingRow.collection_confirmed_by || trackingRow.collectionConfirmedBy || '',
        collectionConfirmedAt: trackingRow.collection_confirmed_at || trackingRow.collectionConfirmedAt || '',
        dateMonitored: trackingRow.date_monitored || trackingRow.dateMonitored || '',
        updatedBy: trackingRow.updated_by || trackingRow.updatedBy || '',
        createdAt: trackingRow.created_at || trackingRow.createdAt || '',
        updatedAt: trackingRow.updated_at || trackingRow.updatedAt || ''
    }, {
        fallbackAmount: getLbcOrderCollectionAmount(orderHeader),
        generateId: false
    });

    return hasMeaningfulLbcTrackingEntry(entry) ? [entry] : [];
}

function parseStoredLbcTrackingEntries(rawValue, orderHeader = {}, trackingRow = null) {
    const rawText = normalizeText(rawValue);
    if (!rawText) {
        return buildLegacyLbcTrackingEntries(trackingRow, orderHeader);
    }
    ...
    try {
        ...
        return parsed
            .map((entry) => normalizeLbcTrackingEntry(entry, {
                fallbackAmount: 0,
                generateId: true,
                fallbackCollectionStatus: trackingRow?.collection_status || trackingRow?.collectionStatus || '',
                fallbackCollectionConfirmedBy: trackingRow?.collection_confirmed_by || trackingRow?.collectionConfirmedBy || '',
                fallbackCollectionConfirmedAt: trackingRow?.collection_confirmed_at || trackingRow?.collectionConfirmedAt || ''
            }))
            .filter((entry) => entry.id && hasMeaningfulLbcTrackingEntry(entry));
    } catch (_error) {
        return buildLegacyLbcTrackingEntries(trackingRow, orderHeader);
    }
}

function serializeStoredLbcTrackingEntries(entries = []) {
    return JSON.stringify(
        (Array.isArray(entries) ? entries : [])
            ...
            .map((entry) => ({
                ...
                amountToCollect: Number(Math.max(0, Number(entry.amountToCollect || 0)).toFixed(2)),
                deliveryStatus: normalizeLbcTrackingStatus(entry.deliveryStatus, entry.trackingNumber ? 'In Transit' : 'Pending'),
                collectionStatus: normalizeLbcCollectionStatus(entry.collectionStatus, 'Pending'),
                collectionConfirmedBy: normalizeText(entry.collectionConfirmedBy || ''),
                collectionConfirmedAt: normalizeText(entry.collectionConfirmedAt || ''),
                dateMonitored: normalizeText(entry.dateMonitored || ''),
...
function deriveAggregateLbcDeliveryStatus(entries = [], fallbackStatus = 'Pending') {
    const normalizedEntries = (Array.isArray(entries) ? entries : [])
        .filter((entry) => hasMeaningfulLbcTrackingEntry(entry))
        .map((entry) => normalizeLbcTrackingStatus(
            entry.deliveryStatus,
            normalizeText(entry.trackingNumber || '') ? 'In Transit' : 'Pending'
        ));

    if (!normalizedEntries.length) {
        return normalizeLbcTrackingStatus(fallbackStatus, 'Pending');
    }

    const uniqueStatuses = [...new Set(normalizedEntries)];
    if (uniqueStatuses.length === 1) {
        return uniqueStatuses[0];
    }

    if (normalizedEntries.some((status) => LBC_CONCERN_STATUS_SET.has(status))) {
        return 'W/ Concern';
    }

    const hasActiveTransit = normalizedEntries.some((status) => LBC_IN_TRANSIT_STATUS_SET.has(status));
    if (hasActiveTransit) {
        return 'In Transit';
    }

    if (normalizedEntries.every((status) => status === 'Delivered' || status === 'RTS')) {
        return 'RTS';
    }

    if (normalizedEntries.every((status) => status === 'Delivered' || status === 'RTO')) {
        return 'RTO';
    }

    if (normalizedEntries.includes('RTO')) {
        return 'RTO';
    }

    if (normalizedEntries.includes('RTS')) {
        return 'RTS';
    }

    return uniqueStatuses[0] || normalizeLbcTrackingStatus(fallbackStatus, 'Pending');
}

function summarizeLbcTrackingEntries(entries = [], orderHeader = {}, trackingRow = null) {
    const trackingEntries = (Array.isArray(entries) ? entries : [])
        .map((entry) => normalizeLbcTrackingEntry(entry, {
            fallbackAmount: 0,
            generateId: true,
            fallbackCollectionStatus: trackingRow?.collection_status || trackingRow?.collectionStatus || '',
        ...
        .filter((entry) => entry.id && hasMeaningfulLbcTrackingEntry(entry));
    const numberedEntries = trackingEntries.filter((entry) => normalizeText(entry.trackingNumber || ''));
    const firstTrackingEntry = numberedEntries[0] || trackingEntries[0] || null;
    const trackingEntryCount = trackingEntries.length;
    const uniqueStatuses = [...new Set(trackingEntries.map((entry) => normalizeLbcTrackingStatus(
        entry.deliveryStatus,
        normalizeText(entry.trackingNumber || '') ? 'In Transit' : 'Pending'
    )))];
    const fallbackAmountSource = trackingEntries.length
        ? (trackingRow?.amount_to_collect ?? trackingRow?.amountToCollect ?? getLbcOrderCollectionAmount(orderHeader))
        : getLbcOrderCollectionAmount(orderHeader);
    const fallbackAmount = Number(Math.max(0, parseNumber(fallbackAmountSource)).toFixed(2));
    const amountToCollect = trackingEntries.length
        ? Number(trackingEntries.reduce((sum, entry) => sum + Number(entry.amountToCollect || 0), 0).toFixed(2))
        : fallbackAmount;
    const deliveryStatus = deriveAggregateLbcDeliveryStatus(
        trackingEntries,
        trackingRow?.delivery_status || trackingRow?.deliveryStatus || deriveLbcTrackingFallbackStatus(orderHeader)
    );
    const dateMonitored = trackingEntries
        .map((entry) => normalizeText(entry.dateMonitored || entry.updatedAt || entry.createdAt || ''))
        .filter(Boolean)
        .sort()
        .slice(-1)[0] || normalizeText(trackingRow?.date_monitored || trackingRow?.dateMonitored || '');
    const updatedBy = trackingEntries
        .map((entry) => normalizeText(entry.updatedBy || ''))
        .filter(Boolean)
        .slice(-1)[0] || normalizeText(trackingRow?.updated_by || trackingRow?.updatedBy || '');
    const trackingNumber = normalizeText(firstTrackingEntry?.trackingNumber || '');
    const trackingSummary = trackingEntryCount > 1
        ? `${trackingNumber || 'Tracking'} +${trackingEntryCount - 1} more`
        : trackingNumber;
    return {
        entries: trackingEntries,
        trackingEntryCount,
        hasTrackingNumber: numberedEntries.length > 0,
        trackingNumber,
        trackingSummary,
        amountToCollect,
        deliveryStatus,
        dateMonitored,
        updatedBy,
        mixedStatus: uniqueStatuses.length > 1,
        statusSummary: uniqueStatuses.length > 1
            ? uniqueStatuses.slice(0, 3).join(' / ')
            : (uniqueStatuses[0] || deliveryStatus)
    };
}

function deriveAggregateLbcCollectionState(entries = [], trackingRow = null) {
    const normalizedEntries = (Array.isArray(entries) ? entries : [])
        .map((entry) => normalizeLbcTrackingEntry(entry, {
            fallbackAmount: 0,
            generateId: true,
        ...
        .filter((entry) => entry.id && hasMeaningfulLbcTrackingEntry(entry));
    const deliveredEntries = normalizedEntries.filter((entry) => isLbcTrackingEntryDelivered(entry));
    const confirmedEntries = deliveredEntries.filter((entry) => normalizeLbcCollectionStatus(entry.collectionStatus, 'Pending') === 'Confirmed');
    const allDeliveredConfirmed = deliveredEntries.length > 0 && confirmedEntries.length === deliveredEntries.length;
    const latestConfirmedEntry = confirmedEntries
        .slice()
        .sort((left, right) => String(
            right.collectionConfirmedAt
            || right.updatedAt
            || right.createdAt
            || ''
        ).localeCompare(String(
            left.collectionConfirmedAt
            || left.updatedAt
            || left.createdAt
            || ''
        )))[0] || null;
    ...
    return {
        ...
        pendingEntryCount: Math.max(0, deliveredEntries.length - confirmedEntries.length),
        collectionStatus: allDeliveredConfirmed ? 'Confirmed' : 'Pending',
        collectionConfirmedBy: allDeliveredConfirmed
            ? normalizeText(
                latestConfirmedEntry?.collectionConfirmedBy
                || trackingRow?.collection_confirmed_by
                || trackingRow?.collectionConfirmedBy
                || ''
            )
            : '',
        collectionConfirmedAt: allDeliveredConfirmed
            ? normalizeText(
                latestConfirmedEntry?.collectionConfirmedAt
                || trackingRow?.collection_confirmed_at
                || trackingRow?.collectionConfirmedAt
                || ''
            )
            : '',
        collectionActionRequired: deliveredEntries.some((entry) =>
            normalizeLbcCollectionStatus(entry.collectionStatus, 'Pending') !== 'Confirmed'
        )
    };
}
...
function canActorEditLbcTrackingBranch({
    actorRole = '',
    actorBranchName = '',
    orderBranch = ''
} = {}) {
    const normalizedRole = normalizeText(actorRole).toLowerCase();
    if (sqliteStore.isCompanyAdminRole?.(normalizedRole) || normalizedRole === 'super_admin') {
        return true;
    }

    const actorBranch = resolveLbcTrackingBranch(actorBranchName) || normalizeText(actorBranchName);
    const targetBranch = resolveLbcTrackingBranch(orderBranch) || normalizeText(orderBranch);
    if (!actorBranch || !targetBranch) {
        return false;
    }

    return normalizeActorValue(actorBranch) === normalizeActorValue(targetBranch);
}
...
function persistClientProfileFromSaleEntry(saleEntryData = {}) {
    ...
    sqliteStore.saveClientProfile({
        ...
        source: saleEntryData.source || 'manual'
    });
}

function buildCustomerCreditContextMapKey(context = {}) {
    const lookupType = normalizeText(context.lookupType).toLowerCase();
    const lookupKey = normalizeText(context.lookupKey);
    if (!lookupType || !lookupKey) {
        return '';
    }

    return `${lookupType}::${lookupKey}`;
}

function resolveCustomerCreditContext({ clientName = '', clientContact = '', clientAddress = '' } = {}) {
    const normalizedClientName = normalizePendingClientLookup(clientName);
    const normalizedContactNumber = normalizePendingClientContact(clientContact);
    const normalizedClientAddress = normalizePendingClientLookup(clientAddress);
    const normalizedNameText = normalizeText(clientName);
    const normalizedContactText = normalizeText(clientContact);
    const normalizedAddressText = normalizeText(clientAddress);

    if (normalizedContactNumber) {
        return {
            clientName: normalizedNameText,
            normalizedClientName,
            clientContact: normalizedContactText,
            normalizedContactNumber,
            clientAddress: normalizedAddressText,
            normalizedClientAddress,
            lookupType: CUSTOMER_CREDIT_LOOKUP_TYPES[0],
            lookupKey: normalizedContactNumber
        };
    }

    if (normalizedClientName && normalizedClientAddress) {
        return {
            clientName: normalizedNameText,
            normalizedClientName,
            clientContact: normalizedContactText,
            ...
            clientAddress: normalizedAddressText,
            normalizedClientAddress,
            lookupType: CUSTOMER_CREDIT_LOOKUP_TYPES[1],
            lookupKey: `${normalizedClientName}||${normalizedClientAddress}`
        };
    }
...
function createCustomerCreditContextFromLedgerRow(row = {}) {
    const lookupType = normalizeText(row.client_lookup_type || row.lookupType).toLowerCase();
    const lookupKey = normalizeText(row.client_lookup_key || row.lookupKey);
    if (!lookupType || !lookupKey) {
        return null;
    }
    ...
    return {
        ...
        normalizedClientName: normalizePendingClientLookup(row.normalized_client_name || row.normalizedClientName || row.client_name || row.clientName),
        clientContact: normalizeText(row.client_contact || row.clientContact),
        normalizedContactNumber: normalizePendingClientContact(row.normalized_contact_number || row.normalizedContactNumber || row.client_contact || row.clientContact),
        clientAddress: normalizeText(row.client_address || row.clientAddress),
        normalizedClientAddress: normalizePendingClientLookup(row.normalized_client_address || row.normalizedClientAddress || row.client_address || row.clientAddress),
        lookupType,
        lookupKey
    };
}

function getCustomerCreditOrderLookups(...values) {
    return uniqueSorted(values.flat().map((value) => normalizeText(value)).filter(Boolean));
}

function getAvailableCustomerCreditBalance(db, customerCreditContext, {
    excludeOrderLookups = [],
    allowNegative = false
} = {}) {
    if (!db || !customerCreditContext?.lookupType || !customerCreditContext?.lookupKey) {
        return 0;
    }
    ...
    const params = [customerCreditContext.lookupType, customerCreditContext.lookupKey];
    const normalizedExcludes = getCustomerCreditOrderLookups(excludeOrderLookups).map((value) => value.toLowerCase());
    if (normalizedExcludes.length) {
        const placeholders = normalizedExcludes.map(() => '?').join(', ');
        clauses.push(`LOWER(COALESCE(related_order_number, '')) NOT IN (${placeholders})`);
        params.push(...normalizedExcludes);
        clauses.push(`LOWER(COALESCE(related_receipt_number, '')) NOT IN (${placeholders})`);
        params.push(...normalizedExcludes);
    }
    ...
    return allowNegative ? balance : Math.max(0, balance);
}

function listCustomerCreditLedgerEntriesByOrder(db, orderLookups = []) {
    const normalizedLookups = getCustomerCreditOrderLookups(orderLookups).map((value) => value.toLowerCase());
    if (!db || !normalizedLookups.length) {
        return [];
    }

    const placeholders = normalizedLookups.map(() => '?').join(', ');
    return db.prepare(`
        SELECT *
        FROM customer_credit_ledger
        WHERE LOWER(COALESCE(related_order_number, '')) IN (${placeholders})
           OR LOWER(COALESCE(related_receipt_number, '')) IN (${placeholders})
        ORDER BY id ASC
    `).all(...normalizedLookups, ...normalizedLookups);
}

function deleteCustomerCreditLedgerEntriesForOrder(db, orderLookups = []) {
    const normalizedLookups = getCustomerCreditOrderLookups(orderLookups).map((value) => value.toLowerCase());
    if (!db || !normalizedLookups.length) {
        return;
    }
    ...
    db.prepare(`
        ...
        WHERE LOWER(COALESCE(related_order_number, '')) IN (${placeholders})
           OR LOWER(COALESCE(related_receipt_number, '')) IN (${placeholders})
    `).run(...normalizedLookups, ...normalizedLookups);
}

function insertCustomerCreditLedgerEntry(db, customerCreditContext, {
    entryType = CUSTOMER_CREDIT_ENTRY_TYPES[0],
    amount = 0,
    relatedOrderNumber = '',
    ...
    const normalizedEntryType = CUSTOMER_CREDIT_ENTRY_TYPES.find((value) => value === normalizeText(entryType).toLowerCase())
        || CUSTOMER_CREDIT_ENTRY_TYPES[0];
    if (!normalizedAmount) {
        return;
    }
    ...
    db.prepare(`
        INSERT INTO customer_credit_ledger (
            client_name, normalized_client_name, client_contact, normalized_contact_number,
            client_address, normalized_client_address, client_lookup_type, client_lookup_key,
            entry_type, amount, related_order_number, related_receipt_number, note
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
        customerCreditContext.clientName || '',
        customerCreditContext.normalizedClientName || '',
        customerCreditContext.clientContact || '',
        customerCreditContext.normalizedContactNumber || '',
        customerCreditContext.clientAddress || '',
        customerCreditContext.normalizedClientAddress || '',
        customerCreditContext.lookupType || '',
        customerCreditContext.lookupKey || '',
        normalizedEntryType,
        normalizedAmount,
        normalizeText(relatedOrderNumber),
        normalizeText(relatedReceiptNumber),
        normalizeText(note)
    );
}

function getCustomerCreditNetAmountForSaleEntry(saleEntryData = null) {
    if (!saleEntryData) {
        return 0;
    }

    const appliedCreditAmount = roundMoney(
        saleEntryData.paymentSummary?.appliedCreditAmount
        ?? saleEntryData.appliedCreditAmount
        ?? 0
    );
    const overpaymentAmount = roundMoney(saleEntryData.paymentSummary?.overpaymentAmount ?? 0);
    return roundMoney(overpaymentAmount - appliedCreditAmount);
}

function buildCustomerCreditLedgerEntriesForSaleEntry(saleEntryData = {}) {
    const customerCreditContext = saleEntryData.customerCreditContext;
    if (!customerCreditContext) {
        return [];
    }

    const relatedOrderNumber = saleEntryData.orderNumber || '';
    const relatedReceiptNumber = saleEntryData.receiptNumber || saleEntryData.orderNumber || '';
    const appliedCreditAmount = roundMoney(saleEntryData.paymentSummary?.appliedCreditAmount ?? saleEntryData.appliedCreditAmount ?? 0);
    const overpaymentAmount = roundMoney(saleEntryData.paymentSummary?.overpaymentAmount ?? 0);
    const entries = [];

    if (appliedCreditAmount > 0) {
        entries.push({
            entryType: CUSTOMER_CREDIT_ENTRY_TYPES[1],
            amount: -appliedCreditAmount,
            relatedOrderNumber,
            relatedReceiptNumber,
            note: 'Applied customer credit'
        });
    }
...
function syncCustomerCreditLedgerForSaleEntry(db, saleEntryData = {}) {
    const orderLookups = getCustomerCreditOrderLookups(
        saleEntryData.orderNumber,
        saleEntryData.receiptNumber
    );
    if (orderLookups.length) {
        deleteCustomerCreditLedgerEntriesForOrder(db, orderLookups);
    }

    buildCustomerCreditLedgerEntriesForSaleEntry(saleEntryData).forEach((entry) => {
        insertCustomerCreditLedgerEntry(db, saleEntryData.customerCreditContext, entry);
    });
}

function assertCustomerCreditReplacementAllowed(db, existingOrder = null, nextSaleEntryData = null) {
    const orderLookups = getCustomerCreditOrderLookups(
        existingOrder?.order_number,
        existingOrder?.receipt_number,
        nextSaleEntryData?.orderNumber,
        nextSaleEntryData?.receiptNumber
    );
    if (!orderLookups.length) {
        return;
    }
    ...
    listCustomerCreditLedgerEntriesByOrder(db, orderLookups).forEach((entry) => {
        const customerCreditContext = createCustomerCreditContextFromLedgerRow(entry);
        const contextKey = buildCustomerCreditContextMapKey(customerCreditContext);
        if (!contextKey || replacementMap.has(contextKey)) {
            return;
        }

        replacementMap.set(contextKey, {
            customerCreditContext,
            desiredNetAmount: 0
        });
    });

    const nextNetAmount = getCustomerCreditNetAmountForSaleEntry(nextSaleEntryData);
    if (nextSaleEntryData?.customerCreditContext && nextNetAmount) {
        const contextKey = buildCustomerCreditContextMapKey(nextSaleEntryData.customerCreditContext);
        const currentEntry = replacementMap.get(contextKey) || {
            customerCreditContext: nextSaleEntryData.customerCreditContext,
            desiredNetAmount: 0
        };
        currentEntry.desiredNetAmount = roundMoney(Number(currentEntry.desiredNetAmount || 0) + nextNetAmount);
        replacementMap.set(contextKey, currentEntry);
    }

    replacementMap.forEach(({ customerCreditContext, desiredNetAmount }) => {
        const balanceExcludingCurrentOrder = getAvailableCustomerCreditBalance(db, customerCreditContext, {
            excludeOrderLookups: orderLookups,
            allowNegative: true
        });
        const nextBalance = roundMoney(balanceExcludingCurrentOrder + Number(desiredNetAmount || 0));
        if (nextBalance < -0.009) {
            throw new Error('This order already issued customer credit that is being used by other receipts. Adjust the later credit usage first.');
        }
    });
}

function normalizeCustomerCreditBackfillFilters(filters = {}) {
    const normalizedDateFrom = normalizeText(filters.dateFrom) ? normalizeDate(filters.dateFrom) : '';
    const normalizedDateTo = normalizeText(filters.dateTo) ? normalizeDate(filters.dateTo) : '';
    ...
    const parsedSampleLimit = Math.floor(parseNumber(filters.sampleLimit || CUSTOMER_CREDIT_BACKFILL_SAMPLE_LIMIT));
    const sampleLimit = Math.min(30, Math.max(1, parsedSampleLimit || CUSTOMER_CREDIT_BACKFILL_SAMPLE_LIMIT));
    return {
...
function listLegacyCustomerCreditBackfillSourceRows(db, filters = {}) {
    if (!db) {
        return [];
    }

    const clauses = ['COALESCE(overpayment_amount, 0) > 0'];
    const params = [];
...
function getLegacyCustomerCreditBackfillRowKey(row = {}) {
    const orderLookups = getCustomerCreditOrderLookups(row.order_number, row.receipt_number);
    if (orderLookups.length) {
        return orderLookups.join('||');
    }
    return `row:${Number(row.id || 0) || normalizeText(row.sale_date) || 'unknown'}`;
}

function getLegacyCustomerCreditBackfillRowPriority(row = {}) {
    let score = 0;
    if (Number(row.line_index || 0) === 1) {
        score += 6;
    }
    if (Number(row.order_total || 0) > 0) {
        score += 4;
    }
    if (normalizeText(row.order_number) || normalizeText(row.receipt_number)) {
        score += 2;
    }
    if (normalizeText(row.client_contact) || normalizeText(row.client_address)) {
        score += 1;
    }
    return score;
}

function dedupeLegacyCustomerCreditBackfillRows(rows = []) {
    const rowMap = new Map();
    rows.forEach((row) => {
        const key = getLegacyCustomerCreditBackfillRowKey(row);
        const currentEntry = rowMap.get(key);
        if (!currentEntry) {
            rowMap.set(key, row);
            return;
        }

        const nextPriority = getLegacyCustomerCreditBackfillRowPriority(row);
        const currentPriority = getLegacyCustomerCreditBackfillRowPriority(currentEntry);
...
function buildCustomerCreditBackfillItem(row = {}, {
    reason = '',
    customerCreditContext = null
} = {}) {
    const overpaymentAmount = roundMoney(Number(row.overpayment_amount || 0));
    return {
        saleDate: normalizeDate(row.sale_date) || normalizeText(row.sale_date),
        orderNumber: normalizeText(row.order_number),
        receiptNumber: normalizeText(row.receipt_number),
        clientName: normalizeText(row.client_name),
        clientContact: normalizeText(row.client_contact),
        clientAddress: normalizeText(row.client_address),
        overpaymentAmount,
        overpaymentDisplay: formatMoney(overpaymentAmount),
        reason: normalizeText(reason),
        lookupType: normalizeText(customerCreditContext?.lookupType || '')
    };
}

function buildCustomerCreditBackfillWarnings(summary = {}) {
    const warnings = [
        'Backfill cannot detect credits that were already consumed manually outside the new customer credit ledger. Use a date range if you only want a safer recent period.'
    ];

    if (Number(summary.alreadyBackfilledCount || 0) > 0) {
        warnings.push(`${summary.alreadyBackfilledCount} order(s) already have customer credit entries, so they were skipped automatically.`);
    }
    if (Number(summary.missingIdentityCount || 0) > 0) {
        warnings.push(`${summary.missingIdentityCount} order(s) were skipped because they need a contact number or an exact client address for a reliable client match.`);
    }
    if (Number(summary.missingOrderLookupCount || 0) > 0) {
        warnings.push(`${summary.missingOrderLookupCount} order row(s) were skipped because the order or receipt number is missing.`);
    }

    return warnings;
}

function buildLegacyCustomerCreditBackfillReport(db, filters = {}) {
    const normalizedFilters = normalizeCustomerCreditBackfillFilters(filters);
    const sourceRows = listLegacyCustomerCreditBackfillSourceRows(db, normalizedFilters);
    const dedupedRows = dedupeLegacyCustomerCreditBackfillRows(sourceRows);
    const sourceOrderLookups = uniqueSorted(
        dedupedRows.flatMap((row) => getCustomerCreditOrderLookups(row.order_number, row.receipt_number))
    );
    const existingLedgerLookupSet = new Set();
    listCustomerCreditLedgerEntriesByOrder(db, sourceOrderLookups).forEach((entry) => {
        getCustomerCreditOrderLookups(entry.related_order_number, entry.related_receipt_number).forEach((lookup) => {
            existingLedgerLookupSet.add(String(lookup || '').trim().toLowerCase());
        });
    });
    ...
    let missingOrderLookupCount = 0;
    let totalEligibleCreditAmount = 0;
    ...
    dedupedRows.forEach((row) => {
        ...
        const orderLookups = getCustomerCreditOrderLookups(row.order_number, row.receipt_number);
        const customerCreditContext = resolveCustomerCreditContext({
        ...
        if (!orderLookups.length) {
            missingOrderLookupCount += 1;
            if (skippedItems.length < normalizedFilters.sampleLimit) {
                skippedItems.push(buildCustomerCreditBackfillItem(row, {
                    reason: 'Missing order/receipt number',
                    customerCreditContext
                }));
            }
            return;
        ...
        if (orderLookups.some((lookup) => existingLedgerLookupSet.has(String(lookup || '').trim().toLowerCase()))) {
            alreadyBackfilledCount += 1;
            if (skippedItems.length < normalizedFilters.sampleLimit) {
                skippedItems.push(buildCustomerCreditBackfillItem(row, {
                    reason: 'Already in customer credit ledger',
                    customerCreditContext
                }));
            }
            return;
        ...
        if (!customerCreditContext) {
            ...
            if (skippedItems.length < normalizedFilters.sampleLimit) {
                skippedItems.push(buildCustomerCreditBackfillItem(row, {
                    reason: 'Missing contact or exact address',
                    customerCreditContext
                }));
            }
            return;
        ...
        const entry = {
            customerCreditContext,
            amount: overpaymentAmount,
            relatedOrderNumber: normalizeText(row.order_number) || normalizeText(row.receipt_number),
            relatedReceiptNumber: normalizeText(row.receipt_number) || normalizeText(row.order_number),
            note: CUSTOMER_CREDIT_BACKFILL_NOTE,
            item: buildCustomerCreditBackfillItem(row, {
                customerCreditContext
            })
        };
        eligibleEntries.push(entry);
        totalEligibleCreditAmount = roundMoney(totalEligibleCreditAmount + overpaymentAmount);
        if (previewItems.length < normalizedFilters.sampleLimit) {
            previewItems.push(entry.item);
        }
    });

    const summary = {
        sourceRowCount: sourceRows.length,
        scannedCount: dedupedRows.length,
        eligibleCount: eligibleEntries.length,
        alreadyBackfilledCount,
        missingIdentityCount,
        missingOrderLookupCount,
        totalEligibleCreditAmount,
        totalEligibleCreditDisplay: formatMoney(totalEligibleCreditAmount)
    };

    return {
        filters: normalizedFilters,
        summary,
        warnings: buildCustomerCreditBackfillWarnings(summary),
        items: previewItems,
        skippedItems,
        eligibleEntries
    };
...
function previewCustomerCreditBackfill(filters = {}) {
    const db = getDb();
    const report = buildLegacyCustomerCreditBackfillReport(db, filters);
    return {
...
function applyCustomerCreditBackfill(filters = {}) {
    const db = getDb();
    const report = buildLegacyCustomerCreditBackfillReport(db, filters);
    let appliedCount = 0;
    let appliedAmount = 0;
    ...
    db.transaction(() => {
        report.eligibleEntries.forEach((entry) => {
            insertCustomerCreditLedgerEntry(db, entry.customerCreditContext, {
                entryType: CUSTOMER_CREDIT_ENTRY_TYPES[0],
                amount: entry.amount,
                relatedOrderNumber: entry.relatedOrderNumber,
                relatedReceiptNumber: entry.relatedReceiptNumber,
                note: entry.note
            });
            appliedCount += 1;
            appliedAmount = roundMoney(appliedAmount + Number(entry.amount || 0));
        });
    })();
    ...
    return {
        ...
        summary: {
            ...
            appliedCount,
            appliedAmount,
            appliedAmountDisplay: formatMoney(appliedAmount)
        },
        warnings: report.warnings,
        items: report.items,
        skippedItems: report.skippedItems
...
function addManualCustomerCreditEntry(payload = {}) {
    const db = getDb();
    const clientName = normalizeText(payload.clientName);
    const clientContact = normalizeText(payload.clientContact);
    const clientAddress = normalizeText(payload.clientAddress);
    const amount = roundMoney(payload.amount);
    const action = normalizeText(payload.action || payload.mode).toLowerCase() === 'remove'
        ? 'remove'
        : 'add';
    const note = normalizeText(payload.note)
        || (action === 'remove' ? 'Manual customer credit reversal' : 'Manual opening customer credit');
    const customerCreditContext = resolveCustomerCreditContext({
        clientName,
        clientContact,
        clientAddress
    ...
    if (!clientName) {
        throw new Error('Client name is required for manual customer credit.');
    }
    if (amount <= 0) {
        throw new Error('Manual customer credit amount must be greater than zero.');
    }
    if (!customerCreditContext) {
        throw new Error('Manual customer credit requires a contact number or the exact client address.');
    }

    const currentAvailableCreditAmount = getAvailableCustomerCreditBalance(db, customerCreditContext);
    if (action === 'remove' && amount > currentAvailableCreditAmount + 0.009) {
        throw new Error(`You can only remove up to the remaining available credit of ${formatMoney(currentAvailableCreditAmount)} for this client.`);
    }

    const signedAmount = action === 'remove'
        ? -amount
        : amount;

    insertCustomerCreditLedgerEntry(db, customerCreditContext, {
        entryType: action === 'remove' ? CUSTOMER_CREDIT_ENTRY_TYPES[2] : CUSTOMER_CREDIT_ENTRY_TYPES[0],
        amount: signedAmount,
        note
    });
    const availableCreditAmount = getAvailableCustomerCreditBalance(db, customerCreditContext);
    ...
    return {
        ...
        note,
        lookupType: customerCreditContext.lookupType,
        availableCreditAmount,
        availableCreditDisplay: formatMoney(availableCreditAmount)
    };
}

function getCustomerCreditLedgerEntryById(db, entryId) {
    const normalizedEntryId = Math.max(0, Math.floor(parseNumber(entryId) || 0));
    if (!db || normalizedEntryId <= 0) {
        return null;
    }
    ...
    return db.prepare(`
        ...
        WHERE id = ?
        LIMIT 1
    `).get(normalizedEntryId) || null;
}

function isEditableManualCustomerCreditLedgerRow(row = {}) {
    const entryType = normalizeText(row.entry_type || row.entryType).toLowerCase();
    const hasLinkedOrder = Boolean(
        normalizeText(row.related_order_number || row.relatedOrderNumber)
        || normalizeText(row.related_receipt_number || row.relatedReceiptNumber)
    );

    return !hasLinkedOrder && (
        entryType === CUSTOMER_CREDIT_ENTRY_TYPES[0]
        || entryType === CUSTOMER_CREDIT_ENTRY_TYPES[2]
    );
}

function resolveManualCustomerCreditActionFromLedgerRow(row = {}) {
    const entryType = normalizeText(row.entry_type || row.entryType).toLowerCase();
    return entryType === CUSTOMER_CREDIT_ENTRY_TYPES[2] || Number(row.amount || 0) < 0
        ? 'remove'
        : 'add';
}

function updateManualCustomerCreditEntry(entryId, payload = {}) {
    const db = getDb();
    const normalizedEntryId = Math.max(0, Math.floor(parseNumber(entryId) || 0));
    if (normalizedEntryId <= 0) {
        throw new Error('A valid manual customer credit entry is required.');
    }

    const existingRow = getCustomerCreditLedgerEntryById(db, normalizedEntryId);
    if (!existingRow) {
        throw new Error('Manual customer credit entry not found.');
    }
    if (!isEditableManualCustomerCreditLedgerRow(existingRow)) {
        throw new Error('Only standalone manual customer credit entries can be edited.');
    }

    const existingCustomerCreditContext = createCustomerCreditContextFromLedgerRow(existingRow);
    if (!existingCustomerCreditContext) {
        throw new Error('The original customer credit lookup is incomplete, so this entry cannot be edited.');
    }
    ...
    const nextAmountDelta = action === 'remove'
        ? -amount
        : amount;
    const previousAmountDelta = roundMoney(Number(existingRow.amount || 0));
    const existingContextKey = buildCustomerCreditContextMapKey(existingCustomerCreditContext);
    const nextContextKey = buildCustomerCreditContextMapKey(customerCreditContext);
    const isSameClientLookup = existingContextKey && existingContextKey === nextContextKey;

    if (isSameClientLookup) {
        const currentAvailableCreditAmount = getAvailableCustomerCreditBalance(db, customerCreditContext, { allowNegative: true });
        const projectedAvailableCreditAmount = roundMoney(currentAvailableCreditAmount - previousAmountDelta + nextAmountDelta);
        if (projectedAvailableCreditAmount < -0.009) {
            if (nextAmountDelta < 0) {
                const removableCreditAmount = Math.max(0, roundMoney(currentAvailableCreditAmount - previousAmountDelta));
                throw new Error(`You can only remove up to the remaining available credit of ${formatMoney(removableCreditAmount)} for this client after excluding the original manual entry.`);
            }
            throw new Error('This manual credit entry has already been used by later receipts, so it cannot be reduced below the consumed amount.');
        }
    } else {
        const originalAvailableCreditAmount = getAvailableCustomerCreditBalance(db, existingCustomerCreditContext, { allowNegative: true });
        const nextAvailableCreditAmount = getAvailableCustomerCreditBalance(db, customerCreditContext, { allowNegative: true });
        const projectedOriginalBalance = roundMoney(originalAvailableCreditAmount - previousAmountDelta);
        const projectedNextBalance = roundMoney(nextAvailableCreditAmount + nextAmountDelta);

        if (projectedOriginalBalance < -0.009) {
            throw new Error('This manual credit entry is already being used by the original client, so you cannot move or reduce it that far.');
        }
        if (projectedNextBalance < -0.009) {
            if (nextAmountDelta < 0) {
                throw new Error(`You can only remove up to the remaining available credit of ${formatMoney(Math.max(0, nextAvailableCreditAmount))} for this client.`);
            }
            throw new Error('This edit would make the destination client credit balance invalid.');
        }
    }
    ...
    db.prepare(`
        ...
        SET
            ...
            client_address = ?,
            normalized_client_address = ?,
            client_lookup_type = ?,
            client_lookup_key = ?,
            entry_type = ?,
            amount = ?,
            related_order_number = '',
            related_receipt_number = '',
            note = ?
        WHERE id = ?
    `).run(
        customerCreditContext.clientName || '',
        ...
        customerCreditContext.lookupType || '',
        customerCreditContext.lookupKey || '',
        action === 'remove' ? CUSTOMER_CREDIT_ENTRY_TYPES[2] : CUSTOMER_CREDIT_ENTRY_TYPES[0],
        nextAmountDelta,
        note,
        normalizedEntryId
    );
...
function getCustomerCreditActivityDateKey(value = '') {
    const text = normalizeText(value);
    if (!text) {
        return '';
    }
...
function listCustomerCreditLedgerRows(db) {
    if (!db) {
        return [];
    }
...
function listSaleHeaderRowsForOrderLookups(db, orderLookups = []) {
    const normalizedLookups = getCustomerCreditOrderLookups(orderLookups).map((value) => value.toLowerCase());
    if (!db || !normalizedLookups.length) {
        return [];
    }
    ...
    return db.prepare(`
        ...
        FROM sales_entries
        WHERE (
            LOWER(COALESCE(order_number, '')) IN (${placeholders})
            OR LOWER(COALESCE(receipt_number, '')) IN (${placeholders})
        )
        AND (COALESCE(line_index, 1) = 1 OR COALESCE(order_total, 0) > 0 OR COALESCE(base_total, 0) > 0)
        ORDER BY sale_date DESC, id DESC
    `).all(...normalizedLookups, ...normalizedLookups);
}

function buildSaleHeaderLookupMap(rows = []) {
    const lookupMap = new Map();
    (Array.isArray(rows) ? rows : []).forEach((row) => {
        getCustomerCreditOrderLookups(row.order_number, row.receipt_number).forEach((lookup) => {
...
function resolveCustomerCreditEntrySourceLabel(row = {}) {
    const entryType = normalizeText(row.entry_type || row.entryType).toLowerCase();
    const note = normalizeText(row.note);
    const hasLinkedOrder = Boolean(normalizeText(row.related_order_number || row.relatedOrderNumber) || normalizeText(row.related_receipt_number || row.relatedReceiptNumber));
    ...
    if (entryType === 'credit' && hasLinkedOrder) {
        return 'Overpayment Credit';
    }
    if (entryType === 'credit' && /manual/i.test(note)) {
        return 'Manual Add';
    }
...
function listCustomerCredits(filters = {}) {
    const db = getDb();
    const normalizedDateFrom = normalizeText(filters.dateFrom) ? normalizeDate(filters.dateFrom) : '';
    const normalizedDateTo = normalizeText(filters.dateTo) ? normalizeDate(filters.dateTo) : '';
    if (normalizedDateFrom && normalizedDateTo && normalizedDateFrom > normalizedDateTo) {
    ...
    const statusFilter = ['all', 'zero_balance', 'with_balance'].includes(normalizedStatus)
        ? normalizedStatus
        : 'with_balance';
    const limitClients = Math.min(500, Math.max(1, Math.floor(parseNumber(filters.limitClients || 250)) || 250));
    const limitEntries = Math.min(500, Math.max(1, Math.floor(parseNumber(filters.limitEntries || 250)) || 250));

    const ledgerRows = listCustomerCreditLedgerRows(db);
    const orderLookupMap = buildSaleHeaderLookupMap(
        listSaleHeaderRowsForOrderLookups(
            db,
            ledgerRows.flatMap((row) => getCustomerCreditOrderLookups(row.related_order_number, row.related_receipt_number))
        )
    );

    const groupedClients = new Map();
    const activityEntries = [];
    const matchingActivityClientKeys = new Set();

    ledgerRows.forEach((row) => {
        const customerCreditContext = createCustomerCreditContextFromLedgerRow(row);
        const contextKey = buildCustomerCreditContextMapKey(customerCreditContext);
        if (!contextKey) {
            return;
        }

        const relatedLookups = getCustomerCreditOrderLookups(row.related_order_number, row.related_receipt_number);
        const headerRow = relatedLookups
            .map((lookup) => orderLookupMap.get(String(lookup || '').trim().toLowerCase()))
            .find(Boolean) || null;
        const createdDateKey = getCustomerCreditActivityDateKey(row.created_at);
        const amount = roundMoney(Number(row.amount || 0));
        const sourceLabel = resolveCustomerCreditEntrySourceLabel(row);
        const searchableText = [
            ...
            row.related_receipt_number,
            headerRow?.branch,
            headerRow?.cash_branch,
            headerRow?.admin_name,
            headerRow?.sales_representative,
            sourceLabel
        ].map((value) => normalizeText(value).toLowerCase()).join(' | ');
        const matchesSearch = !normalizedSearch || searchableText.includes(normalizedSearch);
        const matchesDate = (!normalizedDateFrom || (createdDateKey && createdDateKey >= normalizedDateFrom))
            && (!normalizedDateTo || (createdDateKey && createdDateKey <= normalizedDateTo));
        ...
        const entry = {
            ...
            clientContact: normalizeText(row.client_contact),
            clientAddress: normalizeText(row.client_address),
            lookupType: normalizeText(row.client_lookup_type),
            lookupKey: normalizeText(row.client_lookup_key),
            entryType: normalizeText(row.entry_type).toLowerCase() || 'credit',
            sourceLabel,
            action: resolveManualCustomerCreditActionFromLedgerRow(row),
            editable: isEditableManualCustomerCreditLedgerRow(row),
            amount,
            amountDisplay: `${amount < 0 ? '-' : '+'}${formatMoney(Math.abs(amount))}`,
            amountAbsolute: Math.abs(amount),
            relatedOrderNumber: normalizeText(row.related_order_number),
            relatedReceiptNumber: normalizeText(row.related_receipt_number),
            note: normalizeText(row.note),
            createdAt: normalizeText(row.created_at),
            createdDate: createdDateKey,
            branch: normalizeText(headerRow?.cash_branch || headerRow?.branch),
            saleDate: normalizeText(headerRow?.sale_date),
            adminName: normalizeText(headerRow?.admin_name),
            salesRepresentative: normalizeText(headerRow?.sales_representative)
        };

        const existingGroup = groupedClients.get(contextKey) || {
            clientName: customerCreditContext?.clientName || normalizeText(row.client_name),
            clientContact: customerCreditContext?.clientContact || normalizeText(row.client_contact),
            clientAddress: customerCreditContext?.clientAddress || normalizeText(row.client_address),
            lookupType: customerCreditContext?.lookupType || normalizeText(row.client_lookup_type),
            lookupKey: customerCreditContext?.lookupKey || normalizeText(row.client_lookup_key),
            ...
            lastActivityDate: '',
            lastSourceLabel: '',
            lastRelatedOrderNumber: '',
            lastRelatedReceiptNumber: '',
            lastNote: '',
            lastBranch: '',
            lastAdminName: '',
            lastEntryId: 0,
            lastEntryAction: 'add',
            lastEntryAmount: 0,
            lastEntryEditable: false,
            searchMatched: false
        };

        existingGroup.availableCreditAmount = roundMoney(existingGroup.availableCreditAmount + amount);
        existingGroup.totalEntries += 1;
        if (amount > 0) {
            existingGroup.totalAddedAmount = roundMoney(existingGroup.totalAddedAmount + amount);
        } else if (amount < 0) {
            existingGroup.totalRemovedAmount = roundMoney(existingGroup.totalRemovedAmount + Math.abs(amount));
        }
        if (matchesSearch) {
            existingGroup.searchMatched = true;
        }
        if (!existingGroup.lastActivityAt || String(entry.createdAt) > String(existingGroup.lastActivityAt)) {
            existingGroup.lastActivityAt = entry.createdAt;
            existingGroup.lastActivityDate = entry.createdDate;
            existingGroup.lastSourceLabel = entry.sourceLabel;
            existingGroup.lastRelatedOrderNumber = entry.relatedOrderNumber;
            existingGroup.lastRelatedReceiptNumber = entry.relatedReceiptNumber;
            existingGroup.lastNote = entry.note;
            existingGroup.lastBranch = entry.branch;
            existingGroup.lastAdminName = entry.adminName;
            existingGroup.lastEntryId = entry.id;
            existingGroup.lastEntryAction = entry.action;
            existingGroup.lastEntryAmount = entry.amountAbsolute;
            existingGroup.lastEntryEditable = entry.editable;
        }
        groupedClients.set(contextKey, existingGroup);

        if (matchesSearch && matchesDate) {
            activityEntries.push(entry);
            matchingActivityClientKeys.add(contextKey);
        }
    });

    const matchingClients = Array.from(groupedClients.values())
        .filter((group) => !normalizedSearch || group.searchMatched)
        .filter((group) => {
            const availableCreditAmount = roundMoney(Number(group.availableCreditAmount || 0));
            if (statusFilter === 'zero_balance') {
                return availableCreditAmount <= 0.009;
            }
            if (statusFilter === 'all') {
                return true;
            }
        ...
        .sort((left, right) => {
            if (Number(right.availableCreditAmount || 0) !== Number(left.availableCreditAmount || 0)) {
                return Number(right.availableCreditAmount || 0) - Number(left.availableCreditAmount || 0);
            }
    ...
    const totalAvailableCreditAmount = roundMoney(
        matchingClients.reduce((sum, group) => sum + Math.max(0, Number(group.availableCreditAmount || 0)), 0)
    );
    const clientsWithBalanceCount = matchingClients.filter((group) => Number(group.availableCreditAmount || 0) > 0.009).length;
    const activityCreditAddedAmount = roundMoney(
        activityEntries.reduce((sum, entry) => sum + (entry.amount > 0 ? entry.amount : 0), 0)
    );
    const activityCreditRemovedAmount = roundMoney(
        activityEntries.reduce((sum, entry) => sum + (entry.amount < 0 ? Math.abs(entry.amount) : 0), 0)
    );
    ...
    return {
        ...
        summary: {
            ...
            totalAvailableCreditDisplay: formatMoney(totalAvailableCreditAmount),
            clientsWithBalanceCount,
            matchingClientCount: matchingClients.length,
            activityEntriesCount: activityEntries.length,
            activityClientCount: matchingActivityClientKeys.size,
            activityCreditAddedAmount,
            activityCreditAddedDisplay: formatMoney(activityCreditAddedAmount),
            activityCreditRemovedAmount,
            activityCreditRemovedDisplay: formatMoney(activityCreditRemovedAmount)
        },
        clients: matchingClients.slice(0, limitClients).map((group) => ({
            clientName: group.clientName,
            clientContact: group.clientContact,
            clientAddress: group.clientAddress,
            lookupType: group.lookupType,
            lookupKey: group.lookupKey,
            availableCreditAmount: roundMoney(group.availableCreditAmount),
            availableCreditDisplay: formatMoney(group.availableCreditAmount),
            totalAddedAmount: roundMoney(group.totalAddedAmount),
            totalAddedDisplay: formatMoney(group.totalAddedAmount),
            totalRemovedAmount: roundMoney(group.totalRemovedAmount),
            totalRemovedDisplay: formatMoney(group.totalRemovedAmount),
            totalEntries: group.totalEntries,
            lastActivityAt: group.lastActivityAt,
            lastActivityDate: group.lastActivityDate,
            lastSourceLabel: group.lastSourceLabel,
            lastRelatedOrderNumber: group.lastRelatedOrderNumber,
            lastRelatedReceiptNumber: group.lastRelatedReceiptNumber,
            lastNote: group.lastNote,
            lastBranch: group.lastBranch,
            lastAdminName: group.lastAdminName,
            lastEntryId: group.lastEntryId,
            lastEntryAction: group.lastEntryAction,
            lastEntryAmount: group.lastEntryAmount,
            lastEntryEditable: group.lastEntryEditable
        })),
        entries: activityEntries.slice(0, limitEntries)
    };
}
...
function normalizePaymentMethodBreakdown(value, fallbackPaymentMethods, totalAmount = 0) {
    ...
    rawEntries.forEach((entry) => {
        const rawMethod =
            ...
            || entry?.paymentMethod
            || entry?.label;
        const method = normalizeFlexibleOption(rawMethod, paymentOptions) || normalizeText(rawMethod);
        if (!method) {
            return;
        }
...
function buildAutoCashIncomeEntries(saleEntryData = {}) {
    ...
    if (normalizedCourier === 'LALAMOVE' && collectionAmount > 0) {
        entries.push({
            ...
            paymentMethod: 'Lalamove Collection',
    ...
    expectedEntries.forEach((entry) => {
        ...
        db.prepare(`
            INSERT INTO cash_income_entries (
                income_date, branch, about, amount, payment_method, note, source,
...
function shouldTrackVariantInventory(variant = {}) {
    const source = normalizeText(variant.source).toLowerCase();
    ...
    if (source !== VIRTUAL_T_VARIANT_SOURCE || itemSet !== 'T') {
        return true;
    }

    return containsKeyword(productName, PIECE_KEYWORDS);
}

function buildVirtualTVariant(baseVariant = null) {
    if (!baseVariant) {
        return null;
    }
...
function pickVirtualTTemplateVariant(rows = []) {
    return [...rows]
        .filter((row) => normalizeSet(row.item_set) !== 'T')
        .sort((left, right) => {
            const setCompare = getSetRank(left.item_set) - getSetRank(right.item_set);
            if (setCompare !== 0) {
                return setCompare;
            }
...
function findVirtualTTemplateVariant(db, { productName = '', itemCode = '' } = {}) {
    ...
    if (normalizedProductName) {
        const rows = db.prepare(`
            SELECT
                ...
                pc.price,
                pc.source,
                COALESCE(iv.cost_price, 0) AS cost_price
            FROM product_catalog pc
            LEFT JOIN inventory_variants iv
              ON LOWER(iv.product_name) = LOWER(pc.product_name)
             AND LOWER(iv.set_name) = LOWER(pc.item_set)
            WHERE LOWER(pc.product_name) = LOWER(?)
        `).all(normalizedProductName);
        const match = pickVirtualTTemplateVariant(rows);
        if (match) {
            return match;
        }
    }
    ...
    const rows = db.prepare(`
        ...
        WHERE pc.item_code = ?
    `).all(normalizedCode);
    return pickVirtualTTemplateVariant(rows);
}
...
function syncSalesEntries(db) {
    ...
    const insertRow = db.prepare(`
        INSERT INTO sales_entries (
            ...
            line_index, entry_unit, unit_cost_price, line_cost_total, line_profit
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'seed', ?, 1, ?, 0, 0, 0)
    `);
...
function listInventory({ branch = '', filter = '', limit = 500, offset = 0 } = {}) {
    const db = getDb();
...
function updateInventoryItem({
    ...
    const db = getDb();
    const normalizedId = Number(inventoryId);
    const normalizedUnit = normalizeText(inventoryUnit);
    ...
    const current = db.prepare(`
        SELECT id, item_name, inventory_unit, item_type
        FROM inventory_items
        WHERE id = ?
        LIMIT 1
    `).get(normalizedId);
    ...
    const conflict = db.prepare(`
        ...
        WHERE LOWER(item_name) = LOWER(?)
          ...
          AND id != ?
        LIMIT 1
    `).get(current.item_name, normalizedUnit, normalizedId);

    if (conflict) {
        throw new Error('Unit already exists for this item name.');
    }

    db.prepare(`
        UPDATE inventory_items
        SET inventory_unit = ?, item_type = ?
        WHERE id = ?
    `).run(normalizedUnit, normalizedType, normalizedId);

    return db.prepare(`
        SELECT id AS inventory_id, item_name, inventory_unit, item_type
        FROM inventory_items
        WHERE id = ?
        LIMIT 1
    `).get(normalizedId);
...
function listCompositeItems(filter = '', { limit = 500, offset = 0 } = {}) {
    const db = getDb();
    const query = normalizeText(filter);
    const params = [];
    let whereClause = '';
    ...
    const normalizedOffset = Math.max(0, Number(offset) || 0);
    const normalizedLimit = Math.min(2500, Math.max(1, Number(limit) || 500));
    return groupedRows.slice(normalizedOffset, normalizedOffset + normalizedLimit);
}

function syncCompositeVariantsToInventory() {
    const db = getDb();
    const rows = db.prepare(`
        SELECT DISTINCT product_name, item_code, item_set
        FROM composite_components
        WHERE TRIM(COALESCE(product_name, '')) != ''
          AND TRIM(COALESCE(item_code, '')) != ''
          AND TRIM(COALESCE(item_set, '')) != ''
        ORDER BY product_name COLLATE NOCASE, item_code COLLATE NOCASE, item_set COLLATE NOCASE
    `).all();

    rows.forEach((row) => {
        inventoryVariantStore.ensureInventoryVariant({
            product_name: row.product_name,
            item_code: row.item_code,
            set_name: row.item_set,
            price: 0,
            cost_price: 0
        }, {
            source: 'composite'
    ...
    return {
        totalCompositeVariants: rows.length
    };
}
...
function deleteCompositeItem(compositeKey = '') {
    ...
    inventoryDerivedState.markInventoryDerivedDataDirty(db);
...
function mapInventoryVariantRowToProductVariant(row) {
    if (!row) {
        return null;
    }

    const resolvedProductName = normalizeText(row.product_name || row.productName);
    const resolvedItemCode = normalizeText(row.item_code || row.itemCode);
    const resolvedItemSet = normalizeSet(row.set_name || row.setName || row.item_set || row.itemSet);

    if (!resolvedProductName || !resolvedItemSet) {
        return null;
    }
    ...
    return {
        ...
        item_set: resolvedItemSet,
        helper: normalizeText(row.helper) || [resolvedItemCode, resolvedItemSet].filter(Boolean).join(' | '),
        price: parseNumber(row.price),
        source: normalizeText(row.source),
        cost_price: parseNumber(row.cost_price ?? row.costPrice)
    };
}

function findInventoryVariantFallback(db, { productName = '', itemSet = '', itemCode = '' }) {
    const normalizedProductName = normalizeText(productName);
    const normalizedSet = normalizeSet(itemSet);
    const normalizedCode = normalizeText(itemCode);

    if (normalizedProductName && normalizedSet) {
        const productSetVariant = db.prepare(`
            SELECT product_name, item_code, set_name, price, cost_price, source
        ...
        if (productSetVariant) {
            return mapInventoryVariantRowToProductVariant(productSetVariant);
        }
    }

    if (normalizedCode && normalizedSet) {
        const codeSetVariant = db.prepare(`
            SELECT product_name, item_code, set_name, price, cost_price, source
            FROM inventory_variants
            WHERE item_code = ?
              AND LOWER(set_name) = LOWER(?)
        ...
        if (codeSetVariant) {
            return mapInventoryVariantRowToProductVariant(codeSetVariant);
        }
    }
    ...
    if (normalizedProductName) {
        ...
        if (productVariant) {
            return mapInventoryVariantRowToProductVariant(productVariant);
        }
    }
...
function findInventoryVirtualTTemplateVariant(db, { productName = '', itemCode = '' }) {
    const normalizedProductName = normalizeText(productName);
    const normalizedCode = normalizeText(itemCode);
    ...
    if (normalizedCode) {
        ...
        `).get(normalizedCode);
        if (codeVariant) {
            return mapInventoryVariantRowToProductVariant(codeVariant);
        }
    }
...
function getProductVariant(db, { helper = '', productName = '', itemSet = '', itemCode = '' }) {
    ...
    if (normalizedHelper) {
        ...
        if (helperVariant && (!normalizedSet || normalizeSet(helperVariant.item_set) === normalizedSet)) {
    ...
    if (normalizedProductName && normalizedSet) {
        const productSetVariant = db.prepare(`
            ...
            LEFT JOIN inventory_variants iv
             ...
             AND LOWER(iv.set_name) = LOWER(pc.item_set)
            WHERE LOWER(pc.product_name) = LOWER(?)
              AND LOWER(pc.item_set) = LOWER(?)
            LIMIT 1
        `).get(normalizedProductName, normalizedSet);
        if (productSetVariant) {
            return productSetVariant;
        }
    }
    ...
    if (normalizedCode && normalizedSet) {
        ...
        `).get(normalizedCode, normalizedSet);
        if (codeSetVariant) {
            return codeSetVariant;
        }
    }

    if (normalizedSet === 'T') {
        const virtualVariant = buildVirtualTVariant(findVirtualTTemplateVariant(db, {
            productName: normalizedProductName,
            itemCode: normalizedCode
        }) || findInventoryVirtualTTemplateVariant(db, {
            productName: normalizedProductName,
            itemCode: normalizedCode
        }));
        if (virtualVariant) {
    ...
    // Order Form rows are sourced from inventory_variants, so allow that table to
    // rescue saves when an older tenant is missing the mirrored product_catalog row.
    const inventoryVariantFallback = findInventoryVariantFallback(db, {
        productName: normalizedProductName,
        itemSet: normalizedSet,
        itemCode: normalizedCode
    });
    if (inventoryVariantFallback) {
        return inventoryVariantFallback;
    }
...
function applyInventoryForLine(db, branch, variant, quantity, direction = -1) {
    ...
    if (!shouldTrackVariantInventory(variant)) {
        return;
    }
...
function computePaymentSummary({ paymentType, baseTotal, amountPaid, deliveryFee, deliveryFeeToCollect, courier, appliedCreditAmount }) {
    ...
    const grossOrderTotal = normalizedBaseTotal + (includeDeliveryFee ? normalizedDeliveryFee : 0);
    const normalizedAppliedCreditAmount = Math.min(
        Math.max(0, parseNumber(appliedCreditAmount)),
        grossOrderTotal
    );

    const orderTotal = Math.max(grossOrderTotal - normalizedAppliedCreditAmount, 0);
    ...
    return {
        ...
        appliedCreditAmount: normalizedAppliedCreditAmount,
        grossOrderTotal,
        remainingAmount,
        orderTotal,
        collectionAmount,
...
function buildPromotionNote(promotionLabel = '', discountAmount = 0) {
    const normalizedLabel = normalizeText(promotionLabel);
    ...
    return normalizedLabel
        ? `${normalizedLabel} (-${formatMoney(normalizedDiscount)})`
        : `Discount Applied (-${formatMoney(normalizedDiscount)})`;
}

function mergeOrderNote(baseNote = '', promotionLabel = '', discountAmount = 0) {
    return [normalizeText(baseNote), buildPromotionNote(promotionLabel, discountAmount)]
        .filter(Boolean)
        .join(' | ');
}

function applyDiscountAcrossItems(items = [], requestedDiscountAmount = 0) {
    const originalItems = Array.isArray(items) ? items : [];
    const rawBaseTotal = roundMoney(originalItems.reduce((sum, item) => sum + Number(item.lineSubtotal || 0), 0));
    const discountAmount = Math.min(Math.max(0, roundMoney(requestedDiscountAmount)), rawBaseTotal);

    if (!discountAmount || !rawBaseTotal || !originalItems.length) {
        return {
            discountAmount: 0,
            items: originalItems.map((item) => ({
                ...
                lineProfit: roundMoney(Number(item.lineSubtotal || 0) - Number(item.lineCostTotal || 0))
            }))
        };
    }

    let remainingDiscount = discountAmount;
    const adjustedItems = originalItems.map((item, index) => {
        const isLastItem = index === originalItems.length - 1;
        const rawLineSubtotal = roundMoney(item.lineSubtotal);
        let lineDiscount = isLastItem
            ? remainingDiscount
            : roundMoney(discountAmount * (rawLineSubtotal / rawBaseTotal));

        lineDiscount = Math.min(rawLineSubtotal, Math.max(0, lineDiscount), remainingDiscount);
        remainingDiscount = roundMoney(remainingDiscount - lineDiscount);

        const adjustedLineSubtotal = roundMoney(rawLineSubtotal - lineDiscount);
        const adjustedUnitPrice = Number(
            (Math.max(0, Number(item.quantity || 0)) > 0
                ? adjustedLineSubtotal / Number(item.quantity || 0)
                : 0
            ).toFixed(4)
        );
        ...
        return {
            ...
            lineSubtotal: adjustedLineSubtotal,
            lineProfit: roundMoney(adjustedLineSubtotal - Number(item.lineCostTotal || 0))
        };
    });
    ...
    if (remainingDiscount > 0) {
        ...
        if (lastIndex >= 0) {
            ...
            const extraDiscount = Math.min(remainingDiscount, roundMoney(lastItem.lineSubtotal));
            adjustedItems[lastIndex] = {
                ...lastItem,
                lineDiscount: roundMoney(Number(lastItem.lineDiscount || 0) + extraDiscount),
                lineSubtotal: roundMoney(Number(lastItem.lineSubtotal || 0) - extraDiscount),
                unitPrice: Number(
                    (Math.max(0, Number(lastItem.quantity || 0)) > 0
                        ? roundMoney(Number(lastItem.lineSubtotal || 0) - extraDiscount) / Number(lastItem.quantity || 0)
                        : 0
                    ).toFixed(4)
                ),
                lineProfit: roundMoney(roundMoney(Number(lastItem.lineSubtotal || 0) - extraDiscount) - Number(lastItem.lineCostTotal || 0))
            };
        }
    }
...
function buildSaleEntryData(db, payload, existingOrder = null) {
    ...
    const branch = normalizeFlexibleOption(branchInput, BRANCHES);
    const cashBranch = cashBranchInput
        ? normalizeFlexibleOption(cashBranchInput, BRANCHES)
        : branch;
    const courier = normalizeFlexibleOption(courierInput, COURIER_OPTIONS);
    const configuredAdmins = getConfiguredOrderFormList('admins');
    const configuredSalesRepresentatives = getConfiguredOrderFormList('salesRepresentatives');
    const adminName = configuredAdmins.length
        ? normalizeFlexibleOption(adminInput, configuredAdmins)
        : normalizeText(adminInput);
    const salesRepresentative = configuredSalesRepresentatives.length
        ? normalizeFlexibleOption(salesRepresentativeInput, configuredSalesRepresentatives)
    ...
    const orderStatus = orderStatusInput
        ...
        : '';
    const source = normalizeText(payload.source ?? existingOrder?.source).toLowerCase() || 'manual';
    const discountAmount = roundMoney(payload.discountAmount ?? 0);
    const promotionLabel = normalizeText(payload.promotionLabel || '');
    const note = mergeOrderNote(payload.note ?? existingOrder?.note, promotionLabel, discountAmount);
    ...
    const normalizedItems = lineItems.map((item, index) => {
        ...
        const unitCostPrice = hasProvidedValue(item.unitCostPrice ?? item.costPrice)
            ? parseNumber(item.unitCostPrice ?? item.costPrice)
            : parseNumber(variant.cost_price);
        const lineSubtotal = roundMoney(quantity * unitPrice);
        const lineCostTotal = roundMoney(quantity * unitCostPrice);
        const lineProfit = roundMoney(lineSubtotal - lineCostTotal);
    ...
    const discountedItemsResult = applyDiscountAcrossItems(normalizedItems, discountAmount);
    const baseTotal = hasProvidedValue(payload.baseTotal)
        ? parseNumber(payload.baseTotal)
        : discountedItemsResult.items.reduce((sum, item) => sum + item.lineSubtotal, 0);
    const appliedCreditAmount = roundMoney(payload.appliedCreditAmount ?? payload.appliedCredit ?? existingOrder?.applied_credit_amount ?? 0);
    const availableCreditAmount = customerCreditContext
        ? getAvailableCustomerCreditBalance(db, customerCreditContext, {
            excludeOrderLookups: getCustomerCreditOrderLookups(existingOrder?.order_number, existingOrder?.receipt_number)
        })
        : 0;
    if (appliedCreditAmount > 0 && !customerCreditContext) {
        throw new Error('Applied credit requires the client contact number or exact client address.');
    }
    if (appliedCreditAmount > availableCreditAmount + 0.009) {
        throw new Error(`Applied credit exceeds the available customer credit of ${formatMoney(availableCreditAmount)}.`);
    }
    ...
    if (paymentSummary.paymentType === 'Full Paid' && paymentSummary.amountPaid > 0 && !paymentMethods.length) {
    ...
    const payloadOrderNumber = normalizeOrderIdentifier(payload.orderNumber);
    const existingOrderNumber = normalizeOrderIdentifier(existingOrder?.order_number);
    const existingReceiptNumber = normalizeOrderIdentifier(existingOrder?.receipt_number);
    if (isAutoOrderPlaceholder(payload.orderNumber) || isAutoOrderPlaceholder(existingOrder?.order_number)) {
        console.debug('[orders:debug] ignored placeholder order number while building sale entry', {
            payloadOrderNumber: normalizeText(payload.orderNumber),
            existingOrderNumber: normalizeText(existingOrder?.order_number),
            existingReceiptNumber
        });
    }

    const orderNumber = payloadOrderNumber
        || existingOrderNumber
        || existingReceiptNumber
        || buildOrderNumber(db, saleDate);
    const receiptNumber = normalizeOrderIdentifier(payload.receiptNumber)
        || existingReceiptNumber
        || orderNumber;
    ...
    return {
        ...
        orderStatus,
        note,
        normalizedItems: discountedItemsResult.items,
        paymentSummary,
        appliedCreditAmount: paymentSummary.appliedCreditAmount,
        availableCreditAmount,
        customerCreditContext,
        inventoryDeducted,
        source,
...
function insertSaleRows(db, saleEntryData) {
    const insertRow = db.prepare(`
        INSERT INTO sales_entries (
            ...
            delivery_fee_to_collect, applied_credit_amount, overpayment_amount, underpayment_amount, inventory_deducted,
            unit_cost_price, line_cost_total, line_profit
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    normalizedItems.forEach((item, index) => {
        const result = insertRow.run(
            ...
            note,
            source,
            buildImportKey(`${source || 'manual'}-sale`, [orderNumber, index + 1, item.itemCode, item.itemSet, item.quantity, item.unitPrice]),
            receiptNumber,
            index + 1,
            clientContact,
            clientAddress,
            ...
            index === 0 ? paymentSummary.appliedCreditAmount : 0,
...
function buildSaleResponse(db, saleEntryData, items) {
    const paymentMethod = saleEntryData.paymentMethod
        || (Number(saleEntryData.paymentSummary?.appliedCreditAmount || 0) > 0 ? 'Customer Credit' : '');
    const availableCreditAmount = saleEntryData.customerCreditContext
        ? getAvailableCustomerCreditBalance(db, saleEntryData.customerCreditContext)
        : 0;
    return {
        ...
        grossOrderTotal: saleEntryData.paymentSummary.grossOrderTotal,
        ...
        appliedCreditAmount: saleEntryData.paymentSummary.appliedCreditAmount,
        availableCreditAmount,
        creditLookupReady: Boolean(saleEntryData.customerCreditContext),
...
function getOrderRowsByLookup(db, orderLookup) {
    ...
    const receiptRows = db.prepare(`
        SELECT *
        FROM sales_entries
        WHERE LOWER(COALESCE(receipt_number, '')) = LOWER(?)
        ORDER BY line_index ASC, id ASC
    `).all(lookupValue);
    if (receiptRows.length) {
        console.debug('[orders:debug] database lookup matched receipt number', {
            requestedOrderLookup: lookupValue,
            rowCount: receiptRows.length,
            rowIds: receiptRows.slice(0, 10).map((row) => row.id)
        });
        return receiptRows;
    }

    if (isAutoOrderPlaceholder(lookupValue)) {
        console.debug('[orders:debug] blocked placeholder order lookup', {
            requestedOrderLookup: lookupValue
        });
        return [];
    }
    ...
    const orderRows = db.prepare(`
        ...
        ORDER BY line_index ASC, id ASC
    `).all(lookupValue);
    console.debug('[orders:debug] database lookup matched order number', {
        requestedOrderLookup: lookupValue,
        rowCount: orderRows.length,
        rowIds: orderRows.slice(0, 10).map((row) => row.id)
    });
    return orderRows;
}
...
function getSaleOrder(orderLookup) {
    ...
    const loadedOrderNumber = normalizeOrderIdentifier(header.order_number)
        || normalizeOrderIdentifier(header.receipt_number)
        || normalizeOrderIdentifier(orderLookup);
    const loadedReceiptNumber = normalizeOrderIdentifier(header.receipt_number)
        || loadedOrderNumber;
    console.debug('[orders:debug] loaded order from database', {
        requestedOrderLookup: normalizeText(orderLookup),
        loadedOrderNumber,
        loadedReceiptNumber,
        rowCount: rows.length,
        rowIds: rows.slice(0, 10).map((row) => row.id)
    });
    const settlementMap = buildConfirmedPendingSettlementMap(db, [{
        orderNumber: loadedOrderNumber,
        receiptNumber: loadedReceiptNumber
    }]);
    const effectivePayment = buildEffectivePaymentState(header, settlementMap);
    const customerCreditContext = resolveCustomerCreditContext({
    ...
    const availableCreditAmount = customerCreditContext
        ? getAvailableCustomerCreditBalance(db, customerCreditContext, {
            excludeOrderLookups: getCustomerCreditOrderLookups(loadedOrderNumber, loadedReceiptNumber)
        })
        : 0;
    ...
    return {
        ...
        clientContact: header.client_contact || '',
        clientAddress: header.client_address || '',
        paymentType: effectivePayment.paymentSummary.paymentType,
        paymentMethod: effectivePayment.paymentMethod,
        paymentMethods: effectivePayment.paymentMethodBreakdown.map((entry) => entry.method),
        paymentMethodBreakdown: effectivePayment.paymentMethodBreakdown,
        orderStatus: header.order_status || '',
        note: header.note || '',
        baseTotal: Number(header.base_total ?? header.order_total ?? effectivePayment.paymentSummary.baseTotal ?? 0),
        orderTotal: Number(header.order_total ?? header.base_total ?? effectivePayment.paymentSummary.orderTotal ?? 0),
        paymentAmount: effectivePayment.amountPaid,
        amountPaid: effectivePayment.amountPaid,
        remainingAmount: effectivePayment.paymentSummary.remainingAmount,
        collectionAmount: Number(effectivePayment.paymentSummary.collectionAmount ?? 0),
        deliveryFee: effectivePayment.paymentSummary.deliveryFee,
        deliveryFeeToCollect: effectivePayment.paymentSummary.deliveryFeeToCollect,
        grossOrderTotal: Number(effectivePayment.paymentSummary.grossOrderTotal ?? 0),
        appliedCreditAmount: Number(effectivePayment.paymentSummary.appliedCreditAmount ?? header.applied_credit_amount ?? 0),
        availableCreditAmount,
        creditLookupReady: Boolean(customerCreditContext),
        inventoryDeducted: parseBoolean(header.inventory_deducted ?? 1),
        overpaymentAmount: Number(effectivePayment.paymentSummary.overpaymentAmount ?? 0),
        underpaymentAmount: Number(effectivePayment.paymentSummary.underpaymentAmount ?? 0),
        ...
        items: rows.map((row) => ({
            ...
            itemCode: row.item_code || '',
                ...
                lineCostTotal: Number(row.line_cost_total ?? 0),
                lineProfit: Number(row.line_profit ?? (Number(row.line_subtotal ?? 0) - Number(row.line_cost_total ?? 0))),
...
function addSaleEntry(payload) {
    ...
    db.transaction(() => {
        ...
        syncCustomerCreditLedgerForSaleEntry(db, saleEntryData);
        persistClientProfileFromSaleEntry(saleEntryData);
    })();
...
function updateSaleOrder(orderLookup, payload) {
    ...
    const existingOrderLookup = normalizeOrderIdentifier(existingHeader.order_number)
        || normalizeOrderIdentifier(existingHeader.receipt_number)
        || normalizeOrderIdentifier(orderLookup);
    const existingReceiptLookup = normalizeOrderIdentifier(existingHeader.receipt_number)
        || existingOrderLookup;
    const saleEntryData = buildSaleEntryData(db, payload, existingHeader);
    assertCustomerCreditReplacementAllowed(db, existingHeader, saleEntryData);
    ...
    db.transaction(() => {
        ...
        deleteCustomerCreditLedgerEntriesForOrder(db, getCustomerCreditOrderLookups(
            existingOrderLookup,
...
function deleteSaleOrder(orderLookup) {
    ...
    const existingReceiptLookup = normalizeOrderIdentifier(existingHeader.receipt_number)
        || existingOrderLookup;
    assertCustomerCreditReplacementAllowed(db, existingHeader, null);
    const existingIds = existingRows.map((row) => Number(row.id)).filter(Boolean);
    const deleteRows = existingIds.length
        ? db.prepare(`DELETE FROM sales_entries WHERE id IN (${existingIds.map(() => '?').join(', ')})`)
        : null;
    ...
    return {
        orderNumber: existingOrderLookup,
        receiptNumber: existingReceiptLookup,
...
function normalizeFinancialPaymentMethod(value) {
    const input = normalizeText(value);
    if (!input) {
        return '';
    }

    const normalized = normalizeFlexibleOption(input, getConfiguredPaymentMethodOptions());
    if (!normalized) {
        throw new Error('Mode of payment must match an available payment option.');
    }

    return normalized;
}

function saveFinancialEntry({ tableName, dateField, dateValue, branch, about, amount, paymentMethod, confirmationStatus, note, entryLabel }) {
    const db = getDb();
    ...
    const normalizedPaymentMethod = normalizeFinancialPaymentMethod(paymentMethod);
    ...
    const result = isCashIncomeTable
        ? db.prepare(`
            INSERT INTO ${tableName} (${dateField}, branch, about, amount, payment_method, note, source, auto_generated, confirmation_status)
            VALUES (?, ?, ?, ?, ?, ?, 'manual', 0, ?)
        `).run(normalizedDate, normalizedBranch, normalizedAbout, normalizedAmount, normalizedPaymentMethod, normalizedNote, normalizedConfirmationStatus)
        : db.prepare(`
            INSERT INTO ${tableName} (${dateField}, branch, about, amount, payment_method, note, source)
            VALUES (?, ?, ?, ?, ?, ?, 'manual')
        `).run(normalizedDate, normalizedBranch, normalizedAbout, normalizedAmount, normalizedPaymentMethod, normalizedNote);
...
function updateFinancialEntry({ tableName, dateField, entryId, payload, entryLabel }) {
    ...
    const nextPaymentMethod = payload.paymentMethod ?? payload.payment_method ?? existing.payment_method;
    ...
    const normalizedPaymentMethod = normalizeFinancialPaymentMethod(nextPaymentMethod);
    ...
    if (tableName === 'cash_income_entries') {
        db.prepare(`
            UPDATE ${tableName}
            SET ${dateField} = ?, branch = ?, about = ?, amount = ?, payment_method = ?, note = ?, confirmation_status = ?
            WHERE id = ?
        `).run(normalizedDate, normalizedBranch, normalizedAbout, normalizedAmount, normalizedPaymentMethod, normalizedNote, nextConfirmationStatus, id);
    } else {
        db.prepare(`
            UPDATE ${tableName}
            SET ${dateField} = ?, branch = ?, about = ?, amount = ?, payment_method = ?, note = ?
            WHERE id = ?
        `).run(normalizedDate, normalizedBranch, normalizedAbout, normalizedAmount, normalizedPaymentMethod, normalizedNote, id);
    }
...
function listFinancialEntries({ tableName, dateField, filters = {}, db: targetDb = null }) {
...
function buildExpenseCashPositionSummary(filters = {}, targetDb = null) {
    const db = targetDb || getDb();
    const scopedFilters = {
    ...
    const cashIncomeResult = listFinancialEntries({
        tableName: 'cash_income_entries',
        dateField: 'income_date',
        filters: scopedFilters,
        db
    });
    const cashIncomeByBranch = BRANCHES.reduce((totals, branch) => {
        totals[branch] = 0;
        return totals;
    }, {});

    (cashIncomeResult.items || []).forEach((row) => {
        const summaryAmount = getFinancialSummaryAmount(row, 'cash_income_entries');
        if (summaryAmount <= 0) {
            return;
        }
        ...
        cashIncomeByBranch[branch] += summaryAmount;
    });

    const cashLeftByBranch = BRANCHES.reduce((totals, branch) => {
        totals[branch] = Number(cashIncomeByBranch[branch] || 0) - Number(expenseSummary.byBranch?.[branch] || 0);
        return totals;
    }, {});
    const cashIncomeAmount = Object.values(cashIncomeByBranch).reduce((sum, value) => sum + Number(value || 0), 0);
    const cashLeftAmount = cashIncomeAmount - Number(expenseSummary.totalAmount || 0);

    return {
        cashIncomeAmount,
        cashIncomeAmountDisplay: formatMoney(cashIncomeAmount),
        cashLeftAmount,
        cashLeftAmountDisplay: formatMoney(cashLeftAmount),
        cashIncomeByBranch,
        cashLeftBranchBreakdown: BRANCHES.map((branch) => {
            const cashIncome = Number(cashIncomeByBranch[branch] || 0);
            const expenses = Number(expenseSummary.byBranch?.[branch] || 0);
            const cashLeft = cashIncome - expenses;
            return {
                label: branch,
                value: cashLeft,
                displayValue: formatMoney(cashLeft),
                detailText: `${formatMoney(cashIncome)} cash income less ${formatMoney(expenses)} expenses`
            };
        })
    };
}
...
function buildSalesCostLookupKey(row = {}) {
    return [
        normalizeText(row.helper || ''),
        normalizeText(row.item_sold || ''),
        normalizeText(row.item_code || ''),
        normalizeSet(row.item_set || '')
    ].join('||').toLowerCase();
}

function resolveReportUnitCostPrice(db, row, lookupCache = new Map()) {
    const storedUnitCost = Number(row.unit_cost_price || 0);
    if (storedUnitCost > 0) {
        return storedUnitCost;
    }

    const quantity = Math.max(0, Number(row.quantity || 0));
    const storedLineCostTotal = Number(row.line_cost_total || 0);
    if (quantity > 0 && storedLineCostTotal > 0) {
        return storedLineCostTotal / quantity;
    }

    const cacheKey = buildSalesCostLookupKey(row);
    ...
    const resolvedCost = Number(variant?.cost_price || 0);
    lookupCache.set(cacheKey, resolvedCost);
    return resolvedCost;
}

function hydrateSalesRowsWithFinancials(db, rows = []) {
    const lookupCache = new Map();

    return rows.map((row) => {
        const quantity = Math.max(0, Number(row.quantity || 0));
        const unitCostPrice = resolveReportUnitCostPrice(db, row, lookupCache);
        const lineCostTotal = Number(row.line_cost_total || 0) > 0
            ? Number(row.line_cost_total || 0)
...
function listInventoryOutTracking(filters = {}) {
    const db = getDb();
    const clauses = ['COALESCE(inventory_deducted, 1) = 1', 'quantity > 0'];
    const params = [];
    ...
    const hasPositiveLimit = Number.isFinite(requestedLimit) && requestedLimit > 0;
    const hasOffset = Number.isFinite(requestedOffset) && requestedOffset > 0;
    const query = [`
        SELECT
            id,
            sale_date,
        ...
        FROM sales_entries
        ${whereClause}
        ORDER BY sale_date DESC, created_at DESC, receipt_number DESC, order_number DESC, line_index ASC, id DESC
    `];

    if (hasPositiveLimit) {
        query.push(`LIMIT ${Math.min(2500, requestedLimit)}`);
    } else if (hasOffset) {
        query.push('LIMIT -1');
    }

    if (hasOffset) {
        query.push(`OFFSET ${requestedOffset}`);
    }

    const rawRows = db.prepare(query.join(' ')).all(...params);
    const inventoryRows = db.prepare(`
        ...
        JOIN inventory_items ii ON ii.id = il.inventory_item_id
    `).all();
    const inventoryByBranchNameUnit = new Map();
    const inventoryByBranchName = new Map();

    inventoryRows.forEach((row) => {
        const branchName = normalizeText(row.branch);
        const itemName = normalizeText(row.item_name);
        const unit = normalizeText(row.inventory_unit);
        const quantity = Number(row.quantity || 0);
        const unitKey = `${branchName.toLowerCase()}::${itemName.toLowerCase()}::${unit.toLowerCase()}`;
        const nameKey = `${branchName.toLowerCase()}::${itemName.toLowerCase()}`;
        inventoryByBranchNameUnit.set(unitKey, quantity);
        inventoryByBranchName.set(nameKey, (inventoryByBranchName.get(nameKey) || 0) + quantity);
    });

    const resolveRemainingInventory = (row) => {
        const branchName = normalizeText(row.branch);
        const itemName = normalizeText(row.item_sold);
        const unit = normalizeText(row.entry_unit) || inferInventoryUnit(itemName, row.item_set);
        const unitKey = `${branchName.toLowerCase()}::${itemName.toLowerCase()}::${unit.toLowerCase()}`;
        const nameKey = `${branchName.toLowerCase()}::${itemName.toLowerCase()}`;
        if (inventoryByBranchNameUnit.has(unitKey)) {
            return Number(inventoryByBranchNameUnit.get(unitKey) || 0);
        }
        return Number(inventoryByBranchName.get(nameKey) || 0);
    };

    const items = rawRows.map((row) => {
        const quantityOut = Number(row.quantity || 0);
        const remainingInventory = resolveRemainingInventory(row);
        const invoiceNumber = normalizeOrderIdentifier(row.receipt_number) || normalizeOrderIdentifier(row.order_number) || `SALE-${row.id}`;
        const orderNumber = normalizeOrderIdentifier(row.order_number);
        const receiptNumber = normalizeOrderIdentifier(row.receipt_number);
        const entryUnit = normalizeText(row.entry_unit) || inferInventoryUnit(row.item_sold, row.item_set);
        return {
            id: row.id,
            salesRecordId: row.id,
            date: normalizeDate(row.sale_date),
            saleDate: normalizeDate(row.sale_date),
            branch: normalizeText(row.branch),
            cashBranch: normalizeText(row.cash_branch),
            product: normalizeText(row.item_sold) || 'Unspecified product',
            productName: normalizeText(row.item_sold) || 'Unspecified product',
            itemCode: normalizeText(row.item_code),
            itemSet: normalizeText(row.item_set),
            entryUnit,
            quantityOut,
            quantityOutDisplay: formatQuantity(quantityOut),
            invoiceNumber,
            orderNumber,
            receiptNumber,
            clientName: normalizeText(row.client_name),
            adminName: normalizeText(row.admin_name),
            salesRepresentative: normalizeText(row.sales_representative),
            remainingInventory,
            remainingInventoryDisplay: `${formatQuantity(remainingInventory)} ${entryUnit}`.trim(),
            createdAt: normalizeText(row.created_at),
    ...
    items.forEach((item) => {
        totalQuantityOut += Number(item.quantityOut || 0);
        if (item.invoiceNumber) {
            invoiceSet.add(item.invoiceNumber);
        }
        ...
        daily.quantityOut += Number(item.quantityOut || 0);
        daily.lineCount += 1;
        daily.products.add(`${item.productName.toLowerCase()}::${item.itemCode.toLowerCase()}::${item.itemSet.toLowerCase()}`);
        if (item.invoiceNumber) {
            daily.invoices.add(item.invoiceNumber);
        }

        const productKey = [item.date, item.branch, item.productName, item.itemCode, item.itemSet, item.entryUnit]
        ...
        if (!productMap.has(productKey)) {
            productMap.set(productKey, {
                ...
                remainingInventoryDisplay: item.remainingInventoryDisplay,
                invoices: new Set()
            });
        }
        const product = productMap.get(productKey);
        product.quantityOut += Number(item.quantityOut || 0);
        product.lineCount += 1;
        product.remainingInventory = item.remainingInventory;
        product.remainingInventoryDisplay = item.remainingInventoryDisplay;
        if (item.invoiceNumber) {
            product.invoices.add(item.invoiceNumber);
        }
    });

    const dailyTotals = Array.from(dailyMap.values()).map((entry) => ({
        date: entry.date,
        quantityOut: entry.quantityOut,
        quantityOutDisplay: formatQuantity(entry.quantityOut),
        invoiceCount: entry.invoices.size,
        productCount: entry.products.size,
        lineCount: entry.lineCount
    })).sort((left, right) => String(right.date).localeCompare(String(left.date)));

    const productTotals = Array.from(productMap.values()).map((entry) => ({
        date: entry.date,
        branch: entry.branch,
        ...
        invoiceCount: entry.invoices.size,
        lineCount: entry.lineCount,
        remainingInventory: entry.remainingInventory,
        remainingInventoryDisplay: entry.remainingInventoryDisplay
    })).sort((left, right) => {
        const dateCompare = String(right.date).localeCompare(String(left.date));
        if (dateCompare) return dateCompare;
        if (right.quantityOut !== left.quantityOut) return right.quantityOut - left.quantityOut;
        return String(left.productName).localeCompare(String(right.productName));
    });

    const topProduct = productTotals.reduce((current, row) => {
        if (!current || Number(row.quantityOut || 0) > Number(current.quantityOut || 0)) {
            return row;
        }
        return current;
    }, null);
    ...
    return {
        ...
        summary: {
            ...
            totalQuantityOut,
            totalQuantityOutDisplay: formatQuantity(totalQuantityOut),
...
function listSales(filters = {}) {
    ...
    const rawItems = db.prepare(query.join(' ')).all(...params);
    ...
    const items = hydrateSalesRowsWithFinancials(db, rawItems)
        .map((row) => {
    ...
    const totalCost = items.reduce((sum, row) => sum + Number(row.line_cost_total || 0), 0);
    ...
    const branchCostTotals = BRANCHES.reduce((totals, branch) => {
        totals[branch] = 0;
        return totals;
    }, {});
    ...
    items.forEach((row) => {
        ...
        if (branch) {
            ...
            branchCostTotals[branch] += Number(row.line_cost_total || 0);
        }
    });
    ...
    const netAmount = grossProfit + cashIncomeSummary.totalAmount - expenseSummary.totalAmount;
    const branchFinancialBreakdown = BRANCHES.map((branch) => {
        const sales = branchSalesTotals[branch] || 0;
        const cost = branchCostTotals[branch] || 0;
        const grossProfitByBranch = sales - cost;
        ...
        const net = grossProfitByBranch + cashIncome - expenses;
        ...
        return {
            ...
            cost,
            costDisplay: formatMoney(cost),
            grossProfit: grossProfitByBranch,
            grossProfitDisplay: formatMoney(grossProfitByBranch),
            expenses,
    ...
    return {
        ...
        summary: {
            ...
            totalCost,
            totalCostDisplay: formatMoney(totalCost),
            grossProfit,
            grossProfitDisplay: formatMoney(grossProfit),
            totalPayments,
            ...
            netProfitAmount: netAmount,
            netProfitAmountDisplay: formatMoney(netAmount),
            netAmount,
...
function buildPendingPaymentItem(row = {}, settlementMap = new Map()) {
    const courier = normalizeText(row.courier).toUpperCase();
    if (courier === 'LBC') {
        return null;
    }

    const effectivePayment = buildEffectivePaymentState(row, settlementMap, { excludeCollectionMethod: true });
    const amountPaid = effectivePayment.amountPaid;
    const paymentMethod = effectivePayment.paymentMethod;
    const remainingAmount = Math.max(0, Number(effectivePayment.paymentSummary.remainingAmount ?? 0));
    ...
    return {
        orderKey: getSalesOrderGroupKey(row),
        orderNumber: row.order_number || row.receipt_number || '',
        receiptNumber: row.receipt_number || row.order_number || '',
        saleDate: row.sale_date || '',
        branch: row.branch || '',
        cashBranch: row.cash_branch || row.branch || '',
        courier: row.courier || '',
        ...
        clientContact: row.client_contact || '',
        clientAddress: row.client_address || '',
        paymentType: amountPaid <= 0 ? 'Pending' : 'Partial',
        paymentMethod,
        paymentParts: effectivePayment.paymentParts,
        paymentMethodBreakdown: effectivePayment.paymentMethodBreakdown,
        orderStatus: row.order_status || '',
        note: row.note || '',
        source: row.source || 'manual',
        baseTotal: Number(row.base_total ?? effectivePayment.paymentSummary.baseTotal ?? 0),
        orderTotal: Number(row.order_total ?? effectivePayment.paymentSummary.orderTotal ?? 0),
        grossOrderTotal: Number(effectivePayment.paymentSummary.grossOrderTotal ?? 0),
        paymentAmount: amountPaid,
        amountPaid,
        remainingAmount,
        appliedCreditAmount: Number(effectivePayment.paymentSummary.appliedCreditAmount ?? row.applied_credit_amount ?? 0),
        overpaymentAmount: Number(effectivePayment.paymentSummary.overpaymentAmount ?? 0),
        underpaymentAmount: Number(effectivePayment.paymentSummary.underpaymentAmount ?? remainingAmount),
        statusKey: amountPaid <= 0 ? 'unpaid' : 'partial',
        statusLabel: amountPaid <= 0 ? 'Pending' : 'Partially Paid'
    };
}

function buildClientOrderHistoryItem(row = {}, filters = {}) {
    const paymentMethodBreakdown = normalizePaymentMethodBreakdown(
        row.payment_method_breakdown,
        row.payment_method || row.payment_option,
        row.payment_amount
    ...
    const paymentSummary = computePaymentSummary({
        baseTotal: Number(row.base_total ?? row.order_total ?? 0),
        amountPaid: Number(row.payment_amount ?? 0),
        deliveryFee: Number(row.delivery_fee ?? 0),
        deliveryFeeToCollect: Boolean(Number(row.delivery_fee_to_collect ?? 0)),
        courier: row.courier || '',
        appliedCreditAmount: Number(row.applied_credit_amount ?? 0)
    });
    const paymentParts = buildReportPaymentParts(row);
    const paymentMethod = paymentParts.length
        ? paymentParts.join(' + ')
        : (Number(paymentSummary.appliedCreditAmount || 0) > 0 ? 'Customer Credit' : 'Unspecified');
    const amountPaid = Math.max(0, Number(row.payment_amount ?? 0));
    const rawRemainingAmount = Math.max(
        Number(row.underpayment_amount ?? paymentSummary.underpaymentAmount ?? paymentSummary.remainingAmount ?? 0),
    ...
    const baseItem = {
        ...
        clientContact: row.client_contact || '',
        clientAddress: row.client_address || '',
        paymentType: paymentSummary.paymentType || (amountPaid <= 0 ? 'Pending' : 'Partial'),
        paymentMethod,
        paymentParts,
        paymentMethodBreakdown,
        orderStatus: row.order_status || '',
        note: row.note || '',
        source: row.source || 'manual',
        baseTotal: Number(row.base_total ?? paymentSummary.baseTotal ?? 0),
        orderTotal: Number(row.order_total ?? paymentSummary.orderTotal ?? 0),
        grossOrderTotal: Number(paymentSummary.grossOrderTotal ?? 0),
        paymentAmount: amountPaid,
        amountPaid,
        remainingAmount: rawRemainingAmount,
        rawRemainingAmount,
        collectionAmount: Number(row.collection_amount ?? paymentSummary.collectionAmount ?? 0),
        deliveryFee: paymentSummary.deliveryFee,
        deliveryFeeToCollect: paymentSummary.deliveryFeeToCollect,
        appliedCreditAmount: Number(row.applied_credit_amount ?? paymentSummary.appliedCreditAmount ?? 0),
        overpaymentAmount: Number(row.overpayment_amount ?? paymentSummary.overpaymentAmount ?? 0),
        underpaymentAmount: Number(row.underpayment_amount ?? paymentSummary.underpaymentAmount ?? rawRemainingAmount),
        inventoryDeducted: parseBoolean(row.inventory_deducted ?? 1)
    };
    const match = buildPendingClientMatch(baseItem, filters);
    return match ? { ...baseItem, ...match } : null;
}

function buildClientOrderHistoryProductSummary(rows = []) {
    const seen = new Set();
    const names = (Array.isArray(rows) ? rows : [])
        .slice()
        .sort((left, right) => {
            const leftLineIndex = Number(left.line_index || 0);
            const rightLineIndex = Number(right.line_index || 0);
            if (leftLineIndex !== rightLineIndex) {
                return leftLineIndex - rightLineIndex;
            }
            return Number(left.id || 0) - Number(right.id || 0);
        })
        .map((row) => String(row.item_sold || '').trim())
        .filter((name) => {
            const lookup = normalizeText(name).toLowerCase();
            if (!lookup || lookup === '-' || lookup === 'unspecified') {
                return false;
            }
            if (EXCLUDED_CLIENT_HISTORY_PRODUCT_LOOKUPS.has(lookup.replace(/\s+/g, ''))) {
                return false;
            }
            if (containsKeyword(name, SERVICE_KEYWORDS)) {
                return false;
            }
            if (seen.has(lookup)) {
                return false;
    ...
    return {
        primaryProduct: names[0],
        productSummary: names.length === 1 ? names[0] : `${names[0]} +${names.length - 1} more`
    };
}

function isMeaningfulClientHistoryItem(item = {}) {
    const hasProduct = Boolean(normalizeText(item.productSummary || item.primaryProduct || ''));
    const hasAmount = Number(item.orderTotal || 0) > 0
        || Number(item.amountPaid || 0) > 0
        || Number(item.remainingAmount || 0) > 0;
    const hasLookup = Boolean(normalizeText(item.orderNumber || item.receiptNumber || ''));

    return hasProduct || hasAmount || hasLookup;
}

function listClientOrderHistory(filters = {}) {
    const db = getDb();
    const clientName = normalizeText(filters.clientName);
    if (!clientName) {
        return [];
    }
    ...
    const rows = db.prepare(`
        SELECT *
        FROM sales_entries
        WHERE LOWER(TRIM(COALESCE(client_name, ''))) = LOWER(TRIM(?))
          AND (COALESCE(line_index, 1) = 1 OR COALESCE(order_total, 0) > 0 OR COALESCE(base_total, 0) > 0)
        ORDER BY sale_date DESC, receipt_number DESC, id DESC
    `).all(clientName);
    ...
    rows.forEach((row) => {
        ...
        const currentGroup = groupedHeaders.get(orderKey) || {
            header: null,
            rows: []
        };
        currentGroup.header = selectPreferredSalesOrderHeader(currentGroup.header, row);
        currentGroup.rows.push(row);
        groupedHeaders.set(orderKey, currentGroup);
    });

    const historyItems = Array.from(groupedHeaders.values())
        .map((group) => {
            const item = buildClientOrderHistoryItem(group.header, filters);
            if (!item) {
                return null;
            }
            ...
            return {
                ...item,
                ...buildClientOrderHistoryProductSummary(group.rows)
            };
        })
        .filter(Boolean);
    const settlementMap = buildConfirmedPendingSettlementMap(db, historyItems);

    return historyItems
        .map((item) => {
            const effectivePayment = buildEffectivePaymentState(item, settlementMap);
            const courier = normalizeText(item.courier).toUpperCase();
            const isLbcFlow = courier === 'LBC';
            const rawRemainingAmount = Math.max(0, Number(
                isLbcFlow
                    ? (item.rawRemainingAmount ?? item.remainingAmount ?? 0)
                    : effectivePayment.paymentSummary.remainingAmount
            ));
            const settledByCashIncome = !isLbcFlow
                && effectivePayment.settlementAmount > 0
                && rawRemainingAmount <= 0.009;
            const effectiveRemainingAmount = isLbcFlow ? 0 : rawRemainingAmount;
            const hasPending = effectiveRemainingAmount > 0;

            let statusKey = 'settled';
            let statusLabel = 'Settled';
            if (isLbcFlow) {
                statusKey = 'settled';
                statusLabel = 'LBC Flow';
            } else if (hasPending) {
                statusKey = Number(item.amountPaid || 0) <= 0 ? 'unpaid' : 'partial';
                statusLabel = Number(item.amountPaid || 0) <= 0 ? 'Pending' : 'Partially Paid';
            }

            return {
                ...item,
                paymentType: effectivePayment.paymentSummary.paymentType || item.paymentType,
                paymentMethod: effectivePayment.paymentMethod,
                paymentParts: effectivePayment.paymentParts,
                paymentMethodBreakdown: effectivePayment.paymentMethodBreakdown,
                paymentAmount: effectivePayment.amountPaid,
                ...
                rawRemainingAmount,
                collectionAmount: Number(isLbcFlow ? item.collectionAmount : effectivePayment.paymentSummary.collectionAmount),
                deliveryFee: effectivePayment.paymentSummary.deliveryFee,
                deliveryFeeToCollect: effectivePayment.paymentSummary.deliveryFeeToCollect,
                overpaymentAmount: Number(effectivePayment.paymentSummary.overpaymentAmount ?? 0),
                underpaymentAmount: Number(isLbcFlow ? item.underpaymentAmount : effectivePayment.paymentSummary.underpaymentAmount),
                hasPending,
                settledByCashIncome,
                statusKey,
                statusLabel
        ...
        .sort((left, right) => {
            const leftRank = left.hasPending ? 0 : 1;
            const rightRank = right.hasPending ? 0 : 1;
            ...
            if (String(right.receiptNumber || '') !== String(left.receiptNumber || '')) {
                return String(right.receiptNumber || '').localeCompare(String(left.receiptNumber || ''));
            }
            return String(left.clientName || '').localeCompare(String(right.clientName || ''));
        });
}
...
function buildConfirmedPendingSettlementMap(db, items = []) {
    const lookupValues = uniqueSorted(
        (items || []).flatMap((item) => [item?.orderNumber, item?.receiptNumber])
    ...
    const lookupValuesLower = lookupValues.map((value) => String(value || '').toLowerCase());
    const placeholders = lookupValues.map(() => '?').join(', ');
    const rows = db.prepare(`
        SELECT linked_order_number, linked_receipt_number, amount, about, note, auto_generated, confirmation_status, income_kind
        FROM cash_income_entries
        WHERE (
            COALESCE(auto_generated, 0) = 0
            OR (
                COALESCE(auto_generated, 0) = 1
                AND LOWER(COALESCE(income_kind, '')) = 'lalamove_cod'
                AND (
                    LOWER(COALESCE(linked_order_number, '')) IN (${placeholders})
                    OR LOWER(COALESCE(linked_receipt_number, '')) IN (${placeholders})
                )
            )
        )
    `).all(...lookupValuesLower, ...lookupValuesLower);
    const totals = new Map();
...
function isCashIncomeSettlementCandidate(row = {}) {
    ...
    const isAutoGenerated = parseBoolean(row.auto_generated ?? row.autoGenerated ?? 0);
    if (!isAutoGenerated) {
        return true;
    }
...
function collectCashIncomeSettlementLookups(row = {}, lookupSet = new Set()) {
    const safeLookupSet = lookupSet instanceof Set
        ? lookupSet
        : new Set(Array.isArray(lookupSet) ? lookupSet.map((value) => normalizeText(value)).filter(Boolean) : []);
    if (!safeLookupSet.size) {
        return [];
    }
    ...
    const isAutoGenerated = parseBoolean(row.auto_generated ?? row.autoGenerated ?? 0);
    if (!isAutoGenerated) {
        const searchText = normalizeText([row.about, row.note].filter(Boolean).join(' | ')).toLowerCase();
        if (searchText) {
            safeLookupSet.forEach((lookup) => {
...
function getConfirmedCashSettlementAmount(item = {}, settlementMap = new Map()) {
    if (!(settlementMap instanceof Map) || !settlementMap.size) {
        return 0;
    }
    ...
    return [
        ...
        .map((value) => normalizeText(value))
        .filter(Boolean)
        .reduce((highest, lookup) => Math.max(highest, Number(settlementMap.get(lookup) || 0)), 0);
}

function appendSettlementToPaymentBreakdown(entries = [], settlementAmount = 0, method = 'CASH') {
    const normalizedEntries = normalizePaymentMethodBreakdown(entries, [], 0).map((entry) => ({
        method: entry.method,
        amount: Math.max(0, parseNumber(entry.amount))
    }));
    const normalizedSettlementAmount = Math.max(0, parseNumber(settlementAmount));

    if (normalizedSettlementAmount <= 0) {
        return normalizedEntries;
    }

    const existingEntry = normalizedEntries.find((entry) => entry.method.toLowerCase() === normalizeText(method).toLowerCase());
    if (existingEntry) {
        existingEntry.amount += normalizedSettlementAmount;
        return normalizedEntries;
    }
...
function buildEffectivePaymentState(source = {}, settlementMap = new Map(), { excludeCollectionMethod = false } = {}) {
    const rawAmountPaid = Math.max(0, Number(
        source.payment_amount
        ?? source.paymentAmount
        ?? source.amountPaid
        ?? 0
    ));
    const settlementAmount = getConfirmedCashSettlementAmount(source, settlementMap);
    const basePaymentMethodBreakdown = normalizePaymentMethodBreakdown(
        source.payment_method_breakdown ?? source.paymentMethodBreakdown,
        source.payment_method ?? source.paymentMethod ?? source.payment_option ?? source.paymentOption,
        rawAmountPaid
    );
    const amountPaid = rawAmountPaid + settlementAmount;
    const paymentMethodBreakdown = appendSettlementToPaymentBreakdown(
        basePaymentMethodBreakdown,
        settlementAmount,
        'CASH'
    );
    const paymentSummary = computePaymentSummary({
        baseTotal: Number(source.base_total ?? source.baseTotal ?? source.order_total ?? source.orderTotal ?? 0),
        amountPaid,
        deliveryFee: Number(source.delivery_fee ?? source.deliveryFee ?? 0),
        deliveryFeeToCollect: parseBoolean(source.delivery_fee_to_collect ?? source.deliveryFeeToCollect ?? 0),
        courier: source.courier || '',
        appliedCreditAmount: Number(source.applied_credit_amount ?? source.appliedCreditAmount ?? 0)
    });
    const paymentContext = {
        ...
        paymentAmount: amountPaid,
        amountPaid,
        payment_method_breakdown: serializePaymentMethodBreakdown(paymentMethodBreakdown),
        paymentMethodBreakdown: paymentMethodBreakdown,
        base_total: Number(source.base_total ?? source.baseTotal ?? paymentSummary.baseTotal ?? 0),
        baseTotal: Number(source.baseTotal ?? source.base_total ?? paymentSummary.baseTotal ?? 0),
        applied_credit_amount: Number(source.applied_credit_amount ?? source.appliedCreditAmount ?? paymentSummary.appliedCreditAmount ?? 0),
        appliedCreditAmount: Number(source.appliedCreditAmount ?? source.applied_credit_amount ?? paymentSummary.appliedCreditAmount ?? 0),
        order_total: Number(source.order_total ?? source.orderTotal ?? paymentSummary.orderTotal ?? 0),
        orderTotal: Number(source.orderTotal ?? source.order_total ?? paymentSummary.orderTotal ?? 0),
        collection_amount: Number(paymentSummary.collectionAmount ?? 0),
        collectionAmount: Number(paymentSummary.collectionAmount ?? 0),
        overpayment_amount: Number(paymentSummary.overpaymentAmount ?? 0),
        overpaymentAmount: Number(paymentSummary.overpaymentAmount ?? 0),
        underpayment_amount: Number(paymentSummary.underpaymentAmount ?? 0),
        underpaymentAmount: Number(paymentSummary.underpaymentAmount ?? 0)
    };
    const paymentParts = buildReportPaymentParts(paymentContext)
        .filter((part) => !excludeCollectionMethod || normalizeText(part).toUpperCase() !== 'LBC COLLECTION');
    ...
    return {
        ...
        paymentSummary,
        paymentParts,
        paymentMethod: paymentParts.length
            ? paymentParts.join(' + ')
            : (Number(paymentSummary.appliedCreditAmount || 0) > 0 ? 'Customer Credit' : 'Unspecified')
    };
}

function isPendingPaymentSettledByConfirmedCashIncome(item = {}, settlementMap = new Map()) {
    const remainingAmount = Math.max(0, Number(item.remainingAmount || 0));
    if (remainingAmount <= 0 || !(settlementMap instanceof Map) || !settlementMap.size) {
        return false;
    }
...
function buildPendingClientMatch(item = {}, filters = {}) {
    const requestedName = normalizePendingClientLookup(filters.clientName || '');
    if (!requestedName) {
        return null;
    }

    const itemName = normalizePendingClientLookup(item.clientName || '');
    if (!itemName || itemName !== requestedName) {
        return null;
    }

    const requestedContact = normalizePendingClientContact(filters.clientContact || '');
    const itemContact = normalizePendingClientContact(item.clientContact || '');
    const requestedAddress = normalizePendingClientLookup(filters.clientAddress || '');
    const itemAddress = normalizePendingClientLookup(item.clientAddress || '');

    if (requestedContact && itemContact && requestedContact === itemContact) {
        return {
            matchLevel: 'strong',
            matchType: 'name_contact',
            matchLabel: 'Same name and contact number'
        };
    }

    if (requestedAddress && itemAddress && requestedAddress === itemAddress) {
        return {
            matchLevel: 'strong',
            matchType: 'name_address',
            matchLabel: 'Same name and address'
        };
    }
    ...
    return {
        matchLevel: 'possible',
        matchType: 'name_only',
        matchLabel: 'Same name only'
    };
}

function checkPendingPaymentsByClient(filters = {}) {
    const clientName = normalizeText(filters.clientName);
    const excludeLookup = normalizeText(
        filters.excludeOrderNumber
        || filters.excludeReceiptNumber
        || filters.excludeOrderLookup
    );
    const customerCreditContext = resolveCustomerCreditContext({
        clientName,
        clientContact: filters.clientContact || '',
        clientAddress: filters.clientAddress || ''
    });
    const availableCreditAmount = customerCreditContext
        ? getAvailableCustomerCreditBalance(getDb(), customerCreditContext, {
            excludeOrderLookups: excludeLookup ? [excludeLookup] : []
        })
        : 0;
    if (!clientName) {
        return {
    ...
    const historyItems = listClientOrderHistory(filters)
        .filter((item) => {
            if (!excludeLookup) {
                return true;
            }
            ...
            return ![item.orderNumber, item.receiptNumber]
                .map((value) => normalizeText(value))
                .includes(excludeLookup);
        })
        .sort((left, right) => {
            const leftRank = left.matchLevel === 'strong' ? 0 : 1;
            const rightRank = right.matchLevel === 'strong' ? 0 : 1;
            if (leftRank !== rightRank) {
                return leftRank - rightRank;
            }
            const leftPendingRank = left.hasPending ? 0 : 1;
            const rightPendingRank = right.hasPending ? 0 : 1;
    ...
    const items = historyItems.filter((item) => item.hasPending);

    const limit = Number.isFinite(Number(filters.limit)) && Number(filters.limit) > 0
        ? Math.min(20, Number(filters.limit))
        : 12;
    const pagedItems = items.slice(0, limit);
    const latestProductHistoryItem = historyItems
        .filter((item) => Boolean(normalizeText(item.productSummary || item.primaryProduct || '')))
        .sort((left, right) => {
            if (String(right.saleDate || '') !== String(left.saleDate || '')) {
                return String(right.saleDate || '').localeCompare(String(left.saleDate || ''));
            }
    ...
    const latestMeaningfulHistoryItem = latestProductHistoryItem || historyItems
        .filter((item) => isMeaningfulClientHistoryItem(item))
        .sort((left, right) => {
            if (String(right.saleDate || '') !== String(left.saleDate || '')) {
                return String(right.saleDate || '').localeCompare(String(left.saleDate || ''));
            }
            ...
            const rightRank = right.matchLevel === 'strong' ? 0 : 1;
            return leftRank - rightRank;
        })[0] || historyItems[0] || null;
    const pagedHistoryItems = items.length
        ? historyItems.slice(0, Math.max(limit, 8))
        : (latestMeaningfulHistoryItem ? [latestMeaningfulHistoryItem] : []);
    const strongCount = items.filter((item) => item.matchLevel === 'strong').length;
    const possibleCount = items.filter((item) => item.matchLevel === 'possible').length;
    const totalBalance = items.reduce((sum, item) => sum + Number(item.remainingAmount || 0), 0);
    const settledCount = historyItems.filter((item) => !item.hasPending).length;
    ...
    return {
        ...
        summary: {
            ...
            possibleCount,
            totalBalance,
            totalBalanceDisplay: formatMoney(totalBalance),
            historyCount: historyItems.length,
            pendingCount: items.length,
            settledCount,
            latestProduct: latestMeaningfulHistoryItem?.productSummary || '',
            latestSaleDate: latestMeaningfulHistoryItem?.saleDate || '',
            latestOrderNumber: latestMeaningfulHistoryItem?.orderNumber || latestMeaningfulHistoryItem?.receiptNumber || '',
            availableCreditAmount,
            availableCreditDisplay: formatMoney(availableCreditAmount),
            creditLookupReady: Boolean(customerCreditContext)
        }
...
function listPendingPayments(filters = {}) {
    const db = getDb();
    const clauses = [
        `UPPER(TRIM(COALESCE(courier, ''))) != 'LBC'`,
        `(COALESCE(line_index, 1) = 1 OR COALESCE(order_total, 0) > 0 OR COALESCE(base_total, 0) > 0)`
    ];
    const params = [];
    ...
    if (normalizeText(filters.clientNameExact)) {
        clauses.push(`LOWER(TRIM(COALESCE(client_name, ''))) = LOWER(TRIM(?))`);
        params.push(normalizeText(filters.clientNameExact));
    }
    ...
    if (normalizeText(filters.search)) {
        clauses.push(`
            (
                ...
                OR LOWER(COALESCE(receipt_number, '')) LIKE LOWER(?)
                OR LOWER(COALESCE(order_number, '')) LIKE LOWER(?)
                OR LOWER(COALESCE(client_contact, '')) LIKE LOWER(?)
                OR LOWER(COALESCE(client_address, '')) LIKE LOWER(?)
                OR LOWER(COALESCE(payment_method, '')) LIKE LOWER(?)
                OR LOWER(COALESCE(payment_option, '')) LIKE LOWER(?)
            )
        `);
        params.push(
            `%${filters.search}%`,
    ...
    const requestedPayment = normalizeText(filters.paymentOption);
    const requestedStatus = normalizeText(filters.status).toLowerCase();
    const groupedHeaders = new Map();
    ...
    rows.forEach((row) => {
        ...
        groupedHeaders.set(orderKey, selectPreferredSalesOrderHeader(currentHeader, row));
    });

    const groupedHeaderRows = Array.from(groupedHeaders.values());
    const settlementMap = buildConfirmedPendingSettlementMap(db, groupedHeaderRows.map((row) => ({
        orderNumber: row.order_number || row.receipt_number || '',
        receiptNumber: row.receipt_number || row.order_number || ''
    })));
    const pendingItems = groupedHeaderRows
        .map((row) => buildPendingPaymentItem(row, settlementMap))
        .filter(Boolean);
    const items = pendingItems
        .filter((item) => !isPendingPaymentSettledByConfirmedCashIncome(item, settlementMap))
        .filter((item) => {
            if (requestedPayment) {
                const matchesPayment = normalizeText(item.paymentMethod) === requestedPayment
                    || (item.paymentParts || []).some((part) => normalizeText(part) === requestedPayment);
                if (!matchesPayment) {
                    return false;
                }
            }
            ...
            if (requestedStatus === 'unpaid') {
                return item.statusKey === 'unpaid';
            }
            if (requestedStatus === 'partial') {
                return item.statusKey === 'partial';
            }
    ...
    const pendingBalance = items.reduce((sum, item) => sum + Number(item.remainingAmount || 0), 0);
    ...
    return {
        ...
        summary: {
            ...
            partialCount,
            pendingBalance,
            pendingBalanceDisplay: formatMoney(pendingBalance),
            totalPaid,
...
function selectPreferredSalesOrderHeader(currentHeader, candidateHeader) {
    if (!currentHeader) {
...
function selectPreferredLbcOrderHeader(currentHeader, candidateHeader) {
    return selectPreferredSalesOrderHeader(currentHeader, candidateHeader);
}
...
function listLbcOrderHeaders(db, { branch = '', dateFrom = '' } = {}) {
    ...
    const clauses = [
        ...
        `(COALESCE(line_index, 1) = 1 OR COALESCE(order_total, 0) > 0)`
    ];
...
function buildLbcTrackingItem({ orderRow, trackingRow = null, assignee = null } = {}) {
    ...
    const trackingSummary = summarizeLbcTrackingEntries(
        parseStoredLbcTrackingEntries(trackingRow?.tracking_entries_json || trackingRow?.trackingEntriesJson || '', orderRow, trackingRow),
        orderRow,
        trackingRow
    );
    const collectionState = deriveAggregateLbcCollectionState(trackingSummary.entries, trackingRow);
    const trackingNumber = normalizeText(trackingSummary.trackingNumber || '');
    ...
    const hasTrackingNumber = Boolean(trackingSummary.hasTrackingNumber);
    const deliveryStatus = hasTrackingNumber
        ? normalizeLbcTrackingStatus(trackingSummary.deliveryStatus, 'In Transit')
        : 'Pending';
    ...
    return {
        ...
        trackingNumber,
        trackingSummary: normalizeText(trackingSummary.trackingSummary || trackingNumber),
        trackingEntries: trackingSummary.entries.map((entry) => ({
            trackingEntryId: entry.id,
            trackingNumber: entry.trackingNumber,
            amountToCollect: entry.amountToCollect,
            amountToCollectDisplay: formatMoney(entry.amountToCollect),
            deliveryStatus: normalizeLbcTrackingStatus(entry.deliveryStatus, entry.trackingNumber ? 'In Transit' : 'Pending'),
            collectionStatus: normalizeLbcCollectionStatus(entry.collectionStatus, 'Pending'),
            collectionConfirmedBy: normalizeText(entry.collectionConfirmedBy || ''),
            collectionConfirmedAt: normalizeText(entry.collectionConfirmedAt || ''),
        ...
        trackingEntryCount: trackingSummary.trackingEntryCount,
        hasMultipleTrackingEntries: trackingSummary.trackingEntryCount > 1,
        hasTrackingNumber,
        missingTrackingAfterShipment,
        amountToCollect: trackingSummary.amountToCollect,
        amountToCollectDisplay: formatMoney(trackingSummary.amountToCollect),
        deliveryStatus,
        deliveryStatusSummary: normalizeText(trackingSummary.statusSummary || deliveryStatus),
        hasMixedTrackingStatuses: Boolean(trackingSummary.mixedStatus),
        collectionStatus: collectionState.collectionStatus,
        collectionConfirmedBy: collectionState.collectionConfirmedBy,
        collectionConfirmedAt: collectionState.collectionConfirmedAt,
        collectionActionRequired: collectionState.collectionActionRequired,
        dateMonitored: normalizeText(trackingSummary.dateMonitored || trackingRow?.date_monitored || trackingRow?.dateMonitored || ''),
        assignedTo,
        assignedUserId,
        adminName: normalizeText(orderRow.admin_name || ''),
        salesRepresentative: normalizeText(orderRow.sales_representative || ''),
        updatedBy: normalizeText(trackingSummary.updatedBy || trackingRow?.updated_by || trackingRow?.updatedBy || ''),
        updatedAt: normalizeText(trackingRow?.updated_at || trackingRow?.updatedAt || '')
    };
}

function getEffectiveLbcHistoryDateFrom(dateFrom = '') {
    const normalizedDateFrom = normalizeDate(dateFrom);
    if (!normalizedDateFrom) {
        return LBC_HISTORY_MIN_DATE;
    }
    return normalizedDateFrom >= LBC_HISTORY_MIN_DATE
        ? normalizedDateFrom
...
function listLbcTracking({ branch = '', search = '', deliveryStatus = '', quickFilter = 'all', dateFrom = '' } = {}) {
    ...
    const effectiveDateFrom = getEffectiveLbcHistoryDateFrom(dateFrom);
    ...
    const visibleItems = headers
        ...
        .filter((row) => {
            ...
            const haystack = [
                ...
                row.trackingNumber,
                row.trackingSummary,
                ...(Array.isArray(row.trackingEntries)
                    ? row.trackingEntries.flatMap((entry) => [
                        entry.trackingNumber,
                        entry.deliveryStatus,
                        String(entry.amountToCollect || '')
                    ])
                    : []),
                row.assignedTo,
                row.adminName,
...
function buildLbcCollectionItems(items = []) {
    return (Array.isArray(items) ? items : []).flatMap((row) => {
        const trackingEntries = Array.isArray(row.trackingEntries) ? row.trackingEntries : [];
        return trackingEntries
            .filter((entry) => normalizeText(entry.trackingNumber || '') && isLbcTrackingEntryDelivered(entry))
            .map((entry) => {
                const amountToCollect = Number(Math.max(0, Number(entry.amountToCollect || 0)).toFixed(2));
                return {
                    orderKey: row.orderKey,
                    orderNumber: normalizeText(row.orderNumber || row.receiptNumber || row.orderKey || ''),
                    receiptNumber: normalizeText(row.receiptNumber || row.orderNumber || row.orderKey || ''),
                    saleDate: normalizeText(row.saleDate || ''),
                    branch: normalizeText(row.branch || ''),
                    clientName: normalizeText(row.clientName || ''),
                    courier: normalizeText(row.courier || ''),
                    paymentMethod: normalizeText(row.paymentMethod || ''),
                    trackingEntryId: normalizeText(entry.trackingEntryId || entry.id || ''),
                    trackingNumber: normalizeText(entry.trackingNumber || ''),
                    trackingSummary: normalizeText(entry.trackingNumber || ''),
                    trackingEntryCount: 1,
                    amountToCollect,
                    ...
                    collectionConfirmedBy: normalizeText(entry.collectionConfirmedBy || ''),
                    collectionConfirmedAt: normalizeText(entry.collectionConfirmedAt || ''),
                    dateMonitored: normalizeText(entry.dateMonitored || row.dateMonitored || ''),
                    updatedBy: normalizeText(entry.updatedBy || row.updatedBy || ''),
                    updatedAt: normalizeText(entry.updatedAt || row.updatedAt || '')
                };
            });
    });
}

function listLbcCollections({ branch = '', search = '', status = 'all', dateFrom = '' } = {}) {
    const baseResult = listLbcTracking({
        branch,
        search,
        dateFrom
    });
    const allDelivered = buildLbcCollectionItems(baseResult.items || []);
    const requestedSearch = normalizeText(search).toLowerCase();
    const scopedItems = requestedSearch
        ? allDelivered.filter((row) => [
            row.orderNumber,
            row.receiptNumber,
            row.clientName,
            row.branch,
            ...
            row.collectionConfirmedBy,
            row.deliveryStatus,
            String(row.amountToCollect || '')
        ].join(' ').toLowerCase().includes(requestedSearch))
        : allDelivered;
    ...
    const summary = {
        totalDelivered: scopedItems.length,
...
function setLbcCollectionStatus(orderLookup, payload = {}) {
    ...
    return updateLbcTracking(orderLookup, {
        ...
        trackingEntryId: payload.trackingEntryId ?? payload.tracking_entry_id ?? payload.entryId ?? payload.entry_id ?? '',
...
function updateLbcTracking(orderLookup, payload = {}) {
    ...
    const existingTrackingRow = db.prepare(`
        ...
        LIMIT 1
    `).get(orderKey);
    const existingEntries = parseStoredLbcTrackingEntries(
        existingTrackingRow?.tracking_entries_json || existingTrackingRow?.trackingEntriesJson || '',
        orderHeader,
        existingTrackingRow
    );
    ...
    const actorBranchName = normalizeText(payload.requestBranchName || payload.branchName || '');
    const shouldEnforceAssignee = Boolean(payload.enforceAssignedUser);

    if (
        (actorRole || actorBranchName)
        && !canActorEditLbcTrackingBranch({
            actorRole,
            actorBranchName,
            orderBranch: orderHeader.branch || existingTrackingRow?.branch || ''
        })
    ) {
        throw new Error('You can update LBC tracking only for your assigned branch.');
    }
    ...
    const trackingEntryId = normalizeText(
        payload.trackingEntryId
        ?? payload.tracking_entry_id
        ?? payload.entryId
        ?? payload.entry_id
    );
    const shouldAppendTracking = Boolean(
        payload.appendTracking
        ?? payload.append_tracking
        ?? payload.addTracking
        ?? payload.add_tracking
    );
    const shouldDeleteTracking = Boolean(
        payload.deleteTracking
        ?? payload.delete_tracking
        ?? payload.removeTracking
        ?? payload.remove_tracking
    );

    if (shouldDeleteTracking && !trackingEntryId) {
        throw new Error('Tracking entry is missing for delete action.');
    }

    const targetEntryIndex = trackingEntryId
        ? existingEntries.findIndex((entry) => normalizeText(entry.id) === trackingEntryId)
        : -1;
    if (trackingEntryId && targetEntryIndex === -1) {
        throw new Error('Tracking entry not found for this order.');
    }
    ...
    const hasEditableUpdate = shouldDeleteTracking || hasTrackingNumberOverride || hasAmountOverride || hasStatusOverride;
    let entryUpdatedBy = normalizeText(actorName || actorId || payload.updatedBy || '');
    let nextEntries = existingEntries.slice();

    if (shouldDeleteTracking) {
        nextEntries.splice(targetEntryIndex, 1);
        if (!entryUpdatedBy) {
            entryUpdatedBy = normalizeText(existingTrackingRow?.updated_by || existingTrackingRow?.updatedBy || '');
        }
    } else {
        const targetEntry = trackingEntryId
            ? existingEntries[targetEntryIndex]
            : (shouldAppendTracking ? null : (existingEntries[0] || null));
        const isAdditionalTrackingTarget = Boolean(shouldAppendTracking || (trackingEntryId && targetEntryIndex > 0));
        const currentTrackingNumber = normalizeText(targetEntry?.trackingNumber || '');
        const trackingNumber = hasTrackingNumberOverride
            ? normalizeLbcTrackingNumber(payload.trackingNumber ?? payload.tracking_number, { allowEmpty: !isAdditionalTrackingTarget })
            : currentTrackingNumber;
        if (isAdditionalTrackingTarget && !trackingNumber) {
            throw new Error('Tracking number is required for additional tracking rows.');
        }

        const fallbackAmountToCollect = targetEntry
            ? Number(Math.max(0, parseNumber(targetEntry.amountToCollect)).toFixed(2))
            : (existingEntries.length ? 0 : Number(getLbcOrderCollectionAmount(orderHeader).toFixed(2)));
        const amountToCollect = hasAmountOverride
            ? normalizeLbcTrackingAmountToCollect(payload.amountToCollect ?? payload.amount_to_collect, fallbackAmountToCollect)
            : fallbackAmountToCollect;
        const hadTrackingNumberBefore = Boolean(currentTrackingNumber);
        const fallbackStatus = normalizeLbcTrackingStatus(
            targetEntry?.deliveryStatus,
        ...
        const dateMonitored = hasDateMonitoredOverride
            ...
            : (hasEditableUpdate ? nowIso : normalizeText(targetEntry?.dateMonitored || ''));
        entryUpdatedBy = normalizeText(entryUpdatedBy || targetEntry?.updatedBy || '');

        const nextEntry = normalizeLbcTrackingEntry({
            id: targetEntry?.id || buildLbcTrackingEntryId(),
            trackingNumber,
            amountToCollect,
            deliveryStatus,
            collectionStatus: targetEntry?.collectionStatus || existingCollectionStatus,
            collectionConfirmedBy: targetEntry?.collectionConfirmedBy || existingCollectionConfirmedBy,
            collectionConfirmedAt: targetEntry?.collectionConfirmedAt || existingCollectionConfirmedAt,
            dateMonitored,
            updatedBy: entryUpdatedBy,
            createdAt: normalizeText(targetEntry?.createdAt || targetEntry?.updatedAt || nowIso),
            updatedAt: hasEditableUpdate ? nowIso : normalizeText(targetEntry?.updatedAt || targetEntry?.createdAt || nowIso)
        }, {
            fallbackAmount: fallbackAmountToCollect,
            generateId: true
        });
        if (shouldAppendTracking) {
            nextEntries.push(nextEntry);
        } else if (targetEntryIndex >= 0) {
            if (hasMeaningfulLbcTrackingEntry(nextEntry)) {
                nextEntries.splice(targetEntryIndex, 1, nextEntry);
            } else {
                nextEntries.splice(targetEntryIndex, 1);
            }
        } else if (hasEditableUpdate && hasMeaningfulLbcTrackingEntry(nextEntry)) {
            nextEntries = [nextEntry, ...nextEntries];
        }
    }
    nextEntries = nextEntries
        .map((entry) => normalizeLbcTrackingEntry(entry, {
            fallbackAmount: 0,
            generateId: true,
            fallbackCollectionStatus: existingCollectionStatus,
            fallbackCollectionConfirmedBy: existingCollectionConfirmedBy,
            fallbackCollectionConfirmedAt: existingCollectionConfirmedAt
        }))
        .filter((entry) => entry.id && hasMeaningfulLbcTrackingEntry(entry));

    if (hasCollectionStatusOverride) {
        const collectionTargetEntry = trackingEntryId
            ? nextEntries.find((entry) => normalizeText(entry.id) === trackingEntryId)
            : null;
        if (trackingEntryId && !collectionTargetEntry) {
            throw new Error('Tracking entry not found for this order.');
        }
        if (requestedCollectionStatus === 'Confirmed') {
            if (trackingEntryId) {
                if (!isLbcTrackingEntryDelivered(collectionTargetEntry)) {
                    throw new Error('Collection can be confirmed only after delivery status is Delivered.');
                }
            } else if (!nextEntries.some((entry) => isLbcTrackingEntryDelivered(entry))) {
                throw new Error('Collection can be confirmed only after delivery status is Delivered.');
            }
        }
        ...
        nextEntries = nextEntries.map((entry) => {
            ...
            const deliveredEntry = isLbcTrackingEntryDelivered(entry);
            const targetMatches = trackingEntryId
                ? entryId === trackingEntryId
                : deliveredEntry;

            if (!targetMatches) {
                return normalizeLbcTrackingEntry(entry, {
                    fallbackAmount: 0,
                    generateId: true,
                    fallbackCollectionStatus: existingCollectionStatus,
                    fallbackCollectionConfirmedBy: existingCollectionConfirmedBy,
            ...
            if (!deliveredEntry) {
                return normalizeLbcTrackingEntry({
                    ...entry,
            ...
            const nextCollectionConfirmedBy = requestedCollectionStatus === 'Confirmed'
                ? normalizeText(
                    actorName
                    || actorId
                    || payload.collectionConfirmedBy
                    || entry.collectionConfirmedBy
                    || existingCollectionConfirmedBy
                )
                : '';
            const nextCollectionConfirmedAt = requestedCollectionStatus === 'Confirmed'
                ? normalizeText(
                    explicitCollectionConfirmedAt
                    || entry.collectionConfirmedAt
                    || existingCollectionConfirmedAt
                    || nowIso
                )
                : '';
            ...
            return normalizeLbcTrackingEntry({
                ...entry,
                collectionStatus: requestedCollectionStatus,
                collectionConfirmedBy: nextCollectionConfirmedBy,
                collectionConfirmedAt: nextCollectionConfirmedAt
            }, {
                fallbackAmount: 0,
                generateId: true,
                fallbackCollectionStatus: existingCollectionStatus,
    ...
    const trackingSummary = summarizeLbcTrackingEntries(nextEntries, orderHeader, existingTrackingRow);
    const nextOrderStatus = deriveNextSalesOrderStatusFromTracking({
        orderStatus: orderHeader.order_status || '',
        trackingNumber: normalizeText(trackingSummary.trackingNumber || ''),
        deliveryStatus: trackingSummary.deliveryStatus
    });
    const summaryDeliveryStatus = normalizeLbcTrackingStatus(trackingSummary.deliveryStatus, 'Pending');
    const aggregateCollectionState = deriveAggregateLbcCollectionState(nextEntries, existingTrackingRow);
    const collectionStatus = aggregateCollectionState.collectionStatus;
    const collectionConfirmedBy = aggregateCollectionState.collectionConfirmedBy;
    const collectionConfirmedAt = aggregateCollectionState.collectionConfirmedAt;
    ...
    db.transaction(() => {
        db.prepare(`
            INSERT INTO lbc_tracking_entries (
                ...
                date_monitored, updated_by, updated_at, tracking_entries_json
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?)
            ON CONFLICT(order_key) DO UPDATE SET
                ...
                updated_at = CURRENT_TIMESTAMP,
                tracking_entries_json = excluded.tracking_entries_json
        `).run(
            orderKey,
            ...
            assignedUserId,
            assignedTo,
            normalizeText(trackingSummary.trackingNumber || ''),
            trackingSummary.amountToCollect,
            summaryDeliveryStatus,
            collectionStatus,
            collectionConfirmedBy,
            collectionConfirmedAt,
            normalizeText(trackingSummary.dateMonitored || ''),
            normalizeText(trackingSummary.updatedBy || entryUpdatedBy),
            serializeStoredLbcTrackingEntries(nextEntries)
        );
    ...
    rawItems.forEach((rawItem, index) => {
        ...
        try {
            const updated = updateLbcTracking(targetLookup, {
                ...
                requestUserName: payload.requestUserName || payload.userName || '',
                requestRole: payload.requestRole || payload.role || '',
                requestBranchName: payload.requestBranchName || payload.branchName || '',
                enforceAssignedUser: Boolean(payload.enforceAssignedUser)
            });
            items.push(updated);
        } catch (error) {
...
function refreshLbcTrackingStatuses(payload = {}) {
    const requestedBranch = resolveLbcTrackingBranch(payload.branch || '');
    const hasDateFrom = Object.prototype.hasOwnProperty.call(payload, 'dateFrom')
        || Object.prototype.hasOwnProperty.call(payload, 'date_from');
    const effectiveDateFrom = hasDateFrom
        ? normalizeDate(payload.dateFrom ?? payload.date_from)
...
function listExpenses(filters = {}) {
    const db = getDb();
    const result = listFinancialEntries({
        tableName: 'expense_entries',
        dateField: 'expense_date',
        filters,
        db
    });
    const cashPositionSummary = buildExpenseCashPositionSummary(filters, db);
...
function getReferenceData() {
    ...
    return {
        branches,
        cashBranches,
        setOptions: getReferenceSetOptions(),
        inputAdmins: admins,
...
module.exports = {
    addExpenseEntry,
    addCashIncomeEntry,
    addManualCustomerCreditEntry,
    updateManualCustomerCreditEntry,
    applyCustomerCreditBackfill,
    addSaleEntry,
    bulkAssignLbcTracking,
    checkPendingPaymentsByClient,
    ...
    listExpenses,
    listInventory,
    listInventoryOutTracking,
    listProducts,
    listSales,
    syncCompositeVariantsToInventory,
    previewCustomerCreditBackfill,
