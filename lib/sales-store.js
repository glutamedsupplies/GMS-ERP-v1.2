...
function syncInventoryItems(db) {
    const existingKeys = new Set(db.prepare(`
        SELECT item_name, inventory_unit
        FROM inventory_items
    `).all().map((row) => getInventoryItemLookupKey(row.item_name, row.inventory_unit)));
    ...
    const insertRow = db.prepare(`
        ...
        VALUES (?, ?, ?, 'seed')
        ON CONFLICT (item_name, inventory_unit) DO NOTHING
    `);

    db.transaction((entries) => {
        entries.forEach((entry) => {
            runIgnoringUniqueConstraint(insertRow, entry.itemName, entry.inventoryUnit, entry.itemType);
        });
    })(items);
}
...
function getOrCreateInventoryItem(db, itemName, inventoryUnit, itemType) {
    ...
    runIgnoringUniqueConstraint(
        ...
        normalizeText(itemType) || 'component'
    );
