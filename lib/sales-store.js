...
function buildPendingPaymentItem(row = {}) {
    const courier = normalizeText(row.courier).toUpperCase();
    if (courier === 'LBC') {
        return null;
    }
    ...
    const paymentParts = buildReportPaymentParts(row).filter((part) => normalizeText(part).toUpperCase() !== 'LBC COLLECTION');
    ...
    return {
        ...
        underpaymentAmount: Number(row.underpayment_amount ?? paymentSummary.underpaymentAmount ?? remainingAmount),
...
function listClientOrderHistory(filters = {}) {
    ...
    return historyItems
        .map((item) => {
            ...
            const rawRemainingAmount = Math.max(0, Number(item.rawRemainingAmount ?? item.remainingAmount ?? 0));
            const settledByCashIncome = !isLbcFlow && isPendingPaymentSettledByConfirmedCashIncome({
                ...item,
                remainingAmount: rawRemainingAmount
            }, settlementMap);
            const effectiveRemainingAmount = isLbcFlow || settledByCashIncome ? 0 : rawRemainingAmount;
...
function buildConfirmedPendingSettlementMap(db, items = []) {
    ...
    const rows = db.prepare(`
        SELECT linked_order_number, linked_receipt_number, amount
        FROM cash_income_entries
        WHERE auto_generated = 1
          AND LOWER(COALESCE(income_kind, '')) = 'lalamove_cod'
          AND LOWER(COALESCE(confirmation_status, 'confirmed')) = 'confirmed'
          AND (
            LOWER(COALESCE(linked_order_number, '')) IN (${placeholders})
            OR LOWER(COALESCE(linked_receipt_number, '')) IN (${placeholders})
          )
    `).all(...lookupValues, ...lookupValues);
    const totals = new Map();
    ...
    rows.forEach((row) => {
        ...
        [row.linked_order_number, row.linked_receipt_number]
            .map((value) => normalizeText(value))
            .filter(Boolean)
            .forEach((lookup) => {
...
function listPendingPayments(filters = {}) {
    ...
    const pendingItems = Array.from(groupedHeaders.values())
        .map((row) => buildPendingPaymentItem(row))
        .filter(Boolean);
    const settlementMap = buildConfirmedPendingSettlementMap(db, pendingItems);
