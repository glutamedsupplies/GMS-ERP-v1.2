...
let autoCashIncomeBackfillComplete = false;
...
function backfillLinkedCashIncomeEntries(db) {
    if (autoCashIncomeBackfillComplete) {
        return;
    }
    ...
    autoCashIncomeBackfillComplete = true;
}
...
function syncInventoryCatalog(db) {
    ...
    const insertRow = db.prepare(`
        INSERT OR IGNORE INTO product_catalog (product_name, item_code, item_set, helper, price, source)
        VALUES (?, ?, ?, ?, ?, 'seed')
    `);

    db.transaction((entries) => {
        entries.forEach((entry) => {
            insertRow.run(entry.productName, entry.itemCode, entry.itemSet, entry.helper, entry.price);
        });
    })(rows);
}
...
function syncCompositeComponents(db) {
    ...
    const insertRow = db.prepare(`
        INSERT OR IGNORE INTO composite_components (
...
function syncInventoryItems(db) {
    ...
    catalogRows.forEach((row) => {
        ...
        const key = `${itemName}||${inventoryUnit}`;
        if (!seen.has(key)) {
            seen.add(key);
    ...
    const insertRow = db.prepare(`
        INSERT OR IGNORE INTO inventory_items (item_name, inventory_unit, item_type, source)
        VALUES (?, ?, ?, 'seed')
    `);

    db.transaction((entries) => {
        entries.forEach((entry) => {
            insertRow.run(entry.itemName, entry.inventoryUnit, entry.itemType);
        });
    })(items);
}
...
function syncInventoryLevels(db) {
    ...
    const insertRow = db.prepare(`
        INSERT OR IGNORE INTO inventory_levels (branch, inventory_item_id, quantity, source)
        VALUES (?, ?, 0, 'seed')
    `);
...
function syncSalesEntries(db) {
    ...
    const insertRow = db.prepare(`
        INSERT OR IGNORE INTO sales_entries (
...
function getOrCreateInventoryItem(db, itemName, inventoryUnit, itemType) {
    ...
    db.prepare(`
        ...
        VALUES (?, ?, ?, 'manual')
    `).run(normalizeText(itemName), normalizeText(inventoryUnit), normalizeText(itemType) || 'component');

    row = getInventoryItemByNameUnit(db, itemName, inventoryUnit);
    return row;
}
