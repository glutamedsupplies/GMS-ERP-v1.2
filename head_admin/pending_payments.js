...
async function loadPendingPayments() {
    ...
    try {
        ...
        const confirmedCashIncomeMap = buildConfirmedCashIncomeMap(cashIncomeRows);
...
function buildPendingItems(rows, confirmedCashIncomeMap = new Map()) {
    ...
    return Array.from(groupedRows.values())
        ...
        .filter(Boolean)
        .filter((item) => !isSettledViaConfirmedCashIncome(item, confirmedCashIncomeMap))
        .sort((left, right) => {
...
function buildConfirmedCashIncomeMap(rows = []) {
    const totals = new Map();

    rows.forEach((row) => {
        if (Number(row?.auto_generated || 0) !== 1) {
            return;
        }
        ...
        if (getCashIncomeKind(row) !== 'lalamove_cod') {
            return;
        }
