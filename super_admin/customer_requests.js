...
async function initialize() {
    ...
    companySelect.addEventListener('change', () => {
        state.selectedCompanyId = String(companySelect.value || '').trim();
        state.selectedCode = '';
...
async function loadCompanies() {
    setStatus('Loading companies...', false);
    try {
        const rows = await appClient.listSuperCompanies();
...
async function loadThread(requestCode, { silent = false } = {}) {
    if (!requestCode || !state.selectedCompanyId) {
        return;
    }
...
async function saveRequestUpdate() {
    if (!state.selectedCode || !state.selectedCompanyId) {
        setStatus('Select a request first.', true);
        return;
    }
